...
- F* proves useful, universal facts about selected deterministic Rust functions: exact PQXDH byte layouts and state transitions; construction of a symmetric-ratchet KDF request containing the exact core-selected 32-byte input and `SYM_RATCHET_INFO`; the exact 32-byte key, 32-byte next-chain, and 12-byte nonce partition; preservation of one executor in the concrete chain for the kernel lifetime; complementary role-bound `ConcreteRatchetKernel` construction from one agreed root; equality of beacon-send/server-receive and server-send/beacon-receive material at every sequence; concrete reachability preservation through both directions of public seal/open attempts; complete receive-rejection equality; non-vacuous exact future and cached preparation; conditional callback-success publication; replay neutrality; finite repeated-fixed-rejection retry equivalence; rejection capacity preservation; conditional exact maximum-gap publication; exact whole-entry swap-removal; conditional restoration; and the CTX transcript and pointwise collision-witness properties described below.
- Production stores the extracted `ConcreteRatchetKernel`, which specializes the generic `RefinedRatchet` to core-owned `RatchetChain` and `RatchetMaterial` values and seals one fixed KDF executor into both directional chains. The maintained high-level adapter makes the operational kernel and manager affine, returns only inert serialized ratchet views, gates runtime use on established state, and provides `PersistentServer` to fence activation and every accepted or otherwise state-changing result through a monotonic generation CAS while returning rejected receives without a successor or CAS. Its `SnapshotStore` is trusted for payload integrity and provenance as well as linearizable, durable, rollback-resistant head management; snapshots have no cryptographic authentication or encryption. These are reviewed Rust and deployment enforcement mechanisms, not F* conclusions: concrete HKDF semantics and totality, PQXDH root-derivation semantics, output noncollision, faithful libsodium conversions and callbacks, canonical serde behavior, trusted-store behavior, crash handling, deployment discipline, and compiler correspondence remain assumptions. The core secret-array wrappers implement zeroization on `Drop`, but F* does not prove physical erasure or compiler treatment of those writes.
...
### Ratchet module boundaries and measured dependency surface

The generated PQXDH parent implementation directly needs four root Ratchet declarations: `v_RATCHET_CHAIN_SIZE`, `v_SYM_RATCHET_INFO`, `t_RatchetChain`, and `impl_RatchetChain__from_bytes`. Generated `Pqxdh.Concrete` additionally needs `t_SymmetricRatchetKdfRequest` and `impl_SymmetricRatchetKdfRequest__new` from the root plus `t_ConcreteRatchetKernel` and `impl_ConcreteRatchetKernel__new` from `Ratchet.Concrete`. The maintained [`Beaconcrypt_core.Ratchet.fsti`](../beaconcrypt-core/proofs/fstar/Beaconcrypt_core.Ratchet.fsti) therefore exposes exactly six root value declarations, while [`Beaconcrypt_core.Ratchet.Concrete.fsti`](../beaconcrypt-core/proofs/fstar/Beaconcrypt_core.Ratchet.Concrete.fsti) exposes exactly the two concrete declarations. Ordinary PQXDH checking no longer sees the ratchet's logical state, generic refinement machinery, concrete record representation, or unrelated operations.

The source and generated modules now form a branched dependency graph. `Ratchet.Control` owns logical counters, cache admission, send/receive transitions, restoration, and peer-local helpers; `Ratchet.Refined` imports that logical layer and binds it to generic chain and material values; root `Ratchet` owns fixed-width KDF requests, outputs, and value types; and `Ratchet.Concrete` imports root Ratchet plus Refined to own executor-bearing chains, the concrete kernel, public seal/open operations, and restoration. Root `Pqxdh` imports only the narrow Ratchet value interface, while `Pqxdh.Concrete` imports Pqxdh, root Ratchet, and the narrow Ratchet.Concrete interface for initial executors and role/candidate kernel constructors. The Rust facades re-export the former flat public and crate-private names for compatibility, but hax follows each declaration's canonical definition path and therefore generates four distinct Ratchet modules and two distinct PQXDH modules.

Each generated Ratchet boundary has an `.fsti` that exposes only declarations needed by its generated consumers, and [`Beaconcrypt_core.Pqxdh.Concrete.fsti`](../beaconcrypt-core/proofs/fstar/Beaconcrypt_core.Pqxdh.Concrete.fsti) is an empty companion interface that keeps the concrete PQXDH implementation abstract to ordinary importers. Proof implementation visibility is deliberately narrower still: [`Beaconcrypt_core.Ratchet.Lemmas.fsti`](../beaconcrypt-core/proofs/fstar/Beaconcrypt_core.Ratchet.Lemmas.fsti) exports no proof declarations, while the PQXDH lemma implementation uses six explicit friend edges to the four Ratchet implementation modules, the Ratchet lemma implementation, and `Pqxdh.Concrete`. The Ratchet lemma implementation uses four friend edges to the root, Concrete, Control, and Refined representations, so representation-dependent reasoning remains available to these named proof implementations without becoming a general downstream proof API.
...
The earlier isolated interface prototype measured a modest recheck improvement for the former combined eight-declaration surface. The current six-plus-two interface split responds to the new module ownership rather than making a new performance claim: its primary benefit is preventing root Ratchet and Ratchet.Concrete implementation details from becoming PQXDH dependencies.
...
The proof-only `chain_after(initial, step, count)` recursively applies one fixed pure step `count` times, and `material_at(initial, step, sequence)` selects the material returned by the step that allocates that positive sequence. The generic `reachable` predicate strengthens `valid_refined` with canonical live-chain and cached-material derivations. `concrete_reachable` specializes it to `ConcreteRatchetKernel` and the sole core-selected `concrete_ratchet_step`; each private concrete chain contains its fixed-width bytes and executor, and `concrete_ratchet_step_preserves_executor` proves that every next chain carries the identical executor. The structural clauses still bound the cache at 50, require unique nonzero logical sequences no greater than the receive counter, require every active concrete tag to match its logical sequence, and require inactive slots to be empty ([ratchet lemmas](../beaconcrypt-core/proofs/fstar/Beaconcrypt_core.Ratchet.Lemmas.fst)). The empty-array proof normalizes the extracted 50-element literal once to a recursively defined all-`None` list, transports an arbitrary bounded index through sequence equality, and applies a generic recursive list-index lemma; the proof does not enumerate the 50 concrete indices.
...
- Four branch-heavy iterative scans—logical lookup, inactive-slot preflight, committed-prefix comparison, and staged-slot validation—have proof-only recursive Rust reference functions. Lookup has four branch lemmas plus its wrapper postcondition, and the three Refined scans each have a wrapper postcondition, for eight extracted postconditions and eight balanced local option scopes; four loop invariants equate each loop's current accumulator with its reference suffix. Each scope exposes the required unfolding at fuel 1 without admitting any query. The other three loops for future preparation, compatibility execution, and staged publication are proved against handwritten recursive semantic relations or direct inductions.
...
- The compiled future-preparation path owns one fixed-capacity staging array in `prepare_receive` and threads only a mutable reference to that array through an iterative bounded loop. The loop returns separate final-target metadata, and the caller moves the staging array into `PendingReceive` exactly once. Hax represents the loop with a transparent fuel-decreasing `Rust_primitives.Hax.while_loop` whose accumulator contains the current chain, logical control, remaining count, optional result, skipped count, and staging array. The handwritten recursive definitions are reference specifications and induction principles used to prove the iterative result exact; they do not describe compiled recursive frames or establish a compiler stack-layout bound. The constrained-stack Rust regression supplies compiled-code evidence for the intended reduction.
...
State neutrality trades unauthenticated cache exhaustion for bounded recomputation: the same invalid boundary-distance frame can force up to 50 private KDF steps on every attempt. Deployments still need external per-peer rate limits, admission quotas, or transport replay filtering for availability, and those controls must not encode rejection by mutating the ratchet. Neither proof system models those controls, timing, resource exhaustion, or denial-of-service resistance.
...
These facts remove the former production invariants between a copyable `RatchetState`, independently mutated receive slots, role-specific chain wrappers, and an adapter-reconstructed `RatchetStep`. `RatchetManager` now stores one `ConcreteRatchetKernel`; production message processing calls `concrete_seal_next` and `concrete_open_and_finish`, and PQXDH construction returns a concrete role kernel directly. The kernel owns the persistent chains and material, fixed executor, KDF-request construction, destination preflight, private sequence tags, exact callback arguments, and every logical/material mutation. Production's KDF boundary is executor-only: it reads the core-owned request and returns the fixed output, while seal/open callbacks make only ephemeral libsodium key/nonce values from the borrowed core material.

The proof gives the concrete lifetime derivation cryptographic inputs but not cryptographic semantics. F* proves that the core constructs each `SymmetricRatchetKdfRequest` with the exact old chain or agreed root and `SYM_RATCHET_INFO`, that the fixed-output executor result is partitioned correctly, and that the same per-step executor is carried through every concrete chain. Production must still provide total, faithful HKDF-SHA-512 executors; equal session roots additionally depend on faithful PQXDH root HKDF over the exact verified transcript and `PQXDH_INFO`. F* does not prove HKDF behavior, output noncollision, executor side effects or panics, compiler correspondence, or machine code.
...
The Rust send token and receive completion remain private implementation details of `ConcreteRatchetKernel`. F* proves that `concrete_seal_next` and `concrete_open_and_finish` route canonical core material, sequence, and context through their callbacks and preserve concrete reachability on every pure result, but it does not prove the callbacks' commitment or ChaCha20-Poly1305 semantics. The core's `RatchetChain`, `RatchetKey`, `RatchetNonce`, `RatchetMaterial`, and `SymmetricRatchetKdfRequest` implement `ZeroizeOnDrop`; production converts borrowed material to libsodium key/nonce values only inside a callback. Correct conversion bytes, libsodium behavior, callback result provenance, retained ephemeral copies, compiler treatment of zeroization, and physical erasure remain external. Production restoration canonicalizes the payload and fences activation by external generation CAS, while the restoration lemmas remain conditional and do not prove serde fidelity, trusted payload provenance, store correctness, rollback resistance, crash behavior, or replica coordination. No cryptographic snapshot authentication or encryption is provided.
...
| Maximum-gap success | In the separate capacity leg, sequence 51 succeeds from counter 1, retaining exactly sequences 2 through 50 and consuming sequence 51. |
| Capacity rejection and release | Sequence 53 is rejected because 49 cached entries plus distance 2 exceeds 50; cached sequence 50 is then consumed without moving the live counter or chain. |
| Forward progress | Sequence 53 then succeeds, publishes skipped sequence 52, consumes sequence 53, and ends with 49 cached entries. |

The private-state queries require all six named application values—consumed past, short-leg skipped and target, maximum-gap target, cached sequence 50, and after-release target—to remain secret. They preserve receive-to-send origin correspondence for every successful open. Twelve reachability queries witness both rejected attempts, every short-leg success and replay event, maximum-gap success, capacity rejection, cached consumption, after-release success, attacker-owned registration commit, and attacker recovery of its routed canary.
...
| “Cached receive material belongs to its sequence; send keys are one use; replay is rejected.” | `concrete_reachable` fixes each production kernel's initial chains and sole core-selected step. Fresh concrete construction establishes it; public concrete seal/open preserve it; successful receive removes the complete canonical target before replay lookup fails. For kernels derived from one agreed root, `concrete_directional_materials_agree` proves both cross-role material equalities at every sequence, while the two directional seal/open theorems preserve the paired reachability invariant. The maintained Rust API makes operational state affine and gates it on establishment, while `PersistentServer` provides generation/CAS restoration subject to its trusted-store contract. Concrete key/nonce no-reuse still requires output noncollision and correct external ownership/persistence; F* does not prove those premises, HKDF/AEAD semantics, serde/store correctness, trusted payload provenance, ephemeral conversion fidelity, external copies, or compiler correspondence. |
...
| “Authentication failure is state-neutral.” | For every normally returned receive rejection, F* proves complete refined-state equality across control, both live chains, and every material slot. An admitted future forgery may still cause up to 50 private KDF steps and one open callback before the candidate is dropped. The dedicated ProVerif trace reuses the same explicit state term across two failures, then witnesses later honest success, replay rejection, delayed cached delivery, maximum-gap capacity, and progress after cached consumption. Concrete primitive side effects, panics, timing, logs, allocation, and physical erasure remain outside the theorem. |
...
- concrete PQXDH root-HKDF and symmetric-ratchet HKDF semantics/totality, noncollision of canonical key/nonce outputs, ephemeral conversion of borrowed core key/nonce arrays into libsodium values, AEAD result provenance, executor or callback panic behavior, crash/concurrency atomicity, compiler correspondence, retained copies outside the authoritative concrete kernel, or rollback; F* proves the root bridge under one fixed pure function, core ownership of each exact input plus `SYM_RATCHET_INFO`, fixed output sizes and partitions, one executor carried through the kernel lifetime, complementary concrete role kernels, all-sequence directional equality, and concrete seal/open reachability preservation;
...
- Ed25519-to-X25519 conversion and all primitive calls have the documented
  semantics;
- HKDF is deterministic and supplies the needed PRF and one-way properties. Production must faithfully and totally derive the common PQXDH root from the exact verified root transcript with `PQXDH_INFO`; `authenticated_registration_derives_common_fixed_root` proves equality only for one fixed pure root function. For initial-chain and per-step expansion, the core constructs `SymmetricRatchetKdfRequest` with the exact root or old chain and `SYM_RATCHET_INFO`, and production only executes it. F* proves the 64-byte role split, 76-byte material/next-chain split, fixed lifetime executor, complementary concrete kernels, and both cross-role material equalities at every sequence. Concrete HKDF behavior, output noncollision, ephemeral libsodium conversion fidelity, callback correctness, and compiler correspondence remain obligations;
...
The extracted `ConcreteRatchetKernel` owns logical control, both fixed-width directional chains, the fixed executor carried in those chains, and fixed slots containing sealed core `RatchetMaterial` records. Its F* `concrete_reachable` invariant specializes the generic derivational invariant to the sole core-selected step. `concrete_initial_kernels_are_reachable` establishes that invariant for both role constructors, `concrete_directional_materials_agree` proves both cross-role material equalities for every sequence, and the two directional concrete-session theorems preserve reachability through public seal/open attempts. Whole-plan preflight and complete rejection neutrality have generic exact statements; strict existence theorems return exact valid future and cached preparations for every corresponding valid admitted target; and callback-success theorems conditionally establish exact publication, consumption, and whole-entry swap-removal. Public replay, finite repeated-fixed-rejection retry, rejection-capacity, and conditional fresh maximum-gap capstones compose those facts. The cached-preparation proof packages its target, last-record, and removal facts in one named witness relation and introduces the three witnesses together, avoiding repeated copies of the relation without changing the theorem. The generated function shape and immutable Rust preparation borrow establish non-publication during private work. Restoration establishes derivational reachability only from premises that imported chains and every imported material are canonical. The production connection still assumes:
...
- the fixed-output initial and per-step executors faithfully and totally implement HKDF-SHA-512 on each core-owned `SymmetricRatchetKdfRequest`; production does not select its input, `SYM_RATCHET_INFO`, partition, role direction, or next executor, but executor panic/side effects, HKDF semantics, output noncollision, and compiler correspondence remain external;
- production supplies `concrete_seal_next` and `concrete_open_and_finish` callbacks that correctly convert the borrowed core key and nonce arrays into ephemeral libsodium values and implement the intended commitment and ChaCha20-Poly1305 operations over the exact material, sequence, and frame context supplied by the kernel;
- no concrete key, chain, private lifecycle token, authoritative concrete kernel, or ephemeral callback conversion is independently retained or mutated around the callback-owned kernel calls; the maintained high-level implementation removes `Clone` from those operational values, but F* does not prove Rust visibility, ownership, unsafe-code behavior, FFI behavior, or absence of memory copies;
...
The extracted shared kernel supplies the machine-checked in-memory concrete specialization: core-owned request input and `SYM_RATCHET_INFO`, fixed output sizes and partitions, one executor carried through each kernel lifetime, direct role-bound kernel construction, complementary reachable origins, all-sequence cross-role material equality, and public concrete seal/open reachability preservation. The maintained production adapter adds affine operational state, establishment-gated ownership, inert update snapshots, canonical duplicate-rejecting decoding, and generation/head CAS with fencing. Private Rust fields, consuming APIs, `ZeroizeOnDrop`, compile-time assertions, and tests support these refinements but do not make primitive semantics/totality, callback/parser/codec fidelity, principal selection, trusted payload provenance, store behavior, output noncollision, crash handling, compiler behavior, physical erasure, or no-fork/no-rollback deployment guarantees into F* theorems. Binding checkpoint export/import is explicitly outside the durable one-owner premise. Snapshots have no cryptographic authentication or encryption.
...
The result also trusts the correctness of rustc for the extracted core, hax and its Rust/F* models, the F* checker, Z3, ProVerif, their libraries, the build scripts, and the reviewed extraction selection. Commitment, Ratchet, and PQXDH exact byte-layout proofs specifically rely on the specification-only `Rust_primitives.Slice.array_from_fn` contract supplied through `Core_models.Array.from_fn` by the pinned hax proof library; its pointwise result-index postcondition is trusted rather than proved in this repository. The seven generated ordinary `Rust_primitives.Hax.while_loop` calls instead use a transparent checked recursive definition with a supplied invariant and decreasing fuel; the assumed `while_loop_return` primitive is not emitted and remains rejected. The little-endian key-ID proof also relies on a specification-only right-shift lemma equating bounded shifts with division by the corresponding power of two ([inventory](../beaconcrypt-core/proofs/trusted-boundary.md#proof-library-and-tool-assumptions)).
...
4. `authenticated_registrations_establish_concrete_session` composes that common root with the beacon candidate free function and the generic server role constructor, proves both fresh kernels reachable under one fixed ratchet executor, and proves beacon-send/server-receive and server-send/beacon-receive material equality at an arbitrary sequence; universal quantification gives the result at every sequence. `candidate_ratchet_initializations_are_complementary` separately proves the two candidate direction plans complementary, but no handwritten theorem directly invokes the server candidate free-function wrapper.
5. F* separately proves the associated-data identities match and the two directional public seal/open pairs preserve the concrete-session reachability invariant for every pure callback outcome.
...
The defensible statement is therefore: **under primitive correctness and totality, faithful production execution, uncompromised identities, and the symbolic model assumptions, authenticated honest roles construct complementary reachable concrete kernels whose opposing directional materials agree at every sequence, while a committing honest beacon agrees with a unique earlier server commit on the modeled session data.** F* alone does not authenticate the network run or prove HKDF or AEAD semantics.
...
6. Separately, `symmetric_ratchet_kdf_request_is_exact` and `ratchet_step_uses_exact_chain_and_partition` prove that the core supplies the exact old chain and `SYM_RATCHET_INFO` to the executor and constructs exact fixed 32-byte key, 32-byte next-chain, and 12-byte nonce values from its output; `concrete_ratchet_step_preserves_executor` fixes the executor across the kernel lifetime.
7. `authenticated_registrations_establish_concrete_session` links authenticated equal root transcripts through one fixed pure root derivation to complementary reachable role kernels and both opposing material equalities at an arbitrary sequence, directly invoking the beacon candidate free function but using the generic server role constructor. Candidate-plan complementarity is separately proved, while direct handwritten coverage of the server candidate free-function wrapper remains outside this theorem. `concrete_seal_next_preserves_reachability`, `concrete_open_and_finish_preserves_reachability`, `beacon_seal_server_open_preserves_concrete_session`, and `server_seal_beacon_open_preserves_concrete_session` preserve the concrete derivational invariants through public operations on every callback outcome.
...
9. Production must faithfully and totally implement PQXDH root HKDF with `PQXDH_INFO` and each core-owned symmetric request with HKDF-SHA-512, and its callbacks must correctly convert borrowed core arrays into ephemeral libsodium values and implement commitment/AEAD semantics. Restoration additionally relies on the canonical codec and on `SnapshotStore` to provide payload integrity/provenance plus a linearizable, durable, rollback-resistant head; none is verified by F* or ProVerif, and no cryptographic snapshot authentication or encryption is provided.
...
The production commitment transcript helper and pointwise collision implication are F*-proved, while the probability/runtime lifting, caller, BLAKE2b, parsing, and field provenance remain computational, adapter, or primitive obligations. The layout proof uses the pinned pointwise `array_from_fn` postcondition, and the injectivity proof applies one embedded-segment equality lemma at each of the six transcript offsets instead of repeating the same sequence-extensionality argument for every field; this proof-structure adaptation does not alter the transcript layout or injectivity statement.
...
2. Production delegates planning, private preparation, opening, and success publication to `concrete_open_and_finish`, which selects the executor-bearing concrete step internally. The extracted definition checks admission and every required fixed-array destination before KDF work, while F* proves extensional independence of rejected admission from arbitrary pure executor/callback functions; concrete non-invocation and side-effect claims additionally rely on control-flow review, tests, and faithful compilation. The source-level preparation function borrows live state immutably. F* proves that every valid admitted nonzero future plan returns the exact bounded candidate with separate skipped and target material, canonical final chain, and post-consumption control, and that every valid cached zero-derivation target returns exact prevalidated removal metadata. The open callback receives the preparation-selected material, sequence, and frame context; whether it accepts remains an explicit premise of success theorems.
3. Callback `None` drops the cached metadata or future pending delta and returns the complete entry state, including control, send chain, receive chain, and every material slot. Conditional callback-`Some` lemmas identify cached success with exact whole-record swap-removal and future success with skipped-only movement, final receive-chain installation, and post-target-consumption control. Generated control flow has no normal failure branch after the first live mutation; concrete successful executions are witnessed by Rust tests and the finite ProVerif traces.
4. F* proves that every core-owned KDF request contains the exact old chain and `SYM_RATCHET_INFO`, that each output has the exact key/next-chain/nonce construction, and that the same executor persists through the concrete kernel lifetime. Restoration preserves derivational reachability only when trusted persistence premises supply canonical live chains and material for every imported sequence. Concrete HKDF and AEAD semantics, callback side effects or panics, serde/store correctness, trusted payload provenance, compiler and physical-erasure behavior, crash handling, and rollback resistance remain outside those theorems.
...
It enters the locked Nix environment, checks exact tool identities, regenerates both proof backends, enforces the local no-`assume`/no-`admit` policy, strictly checks generated and handwritten F*, and runs all ProVerif scenarios concurrently through separate targets. Each ProVerif scenario can also be checked directly with `make -C beaconcrypt-core check-proverif-<scenario>`, regenerated and checked in the locked shell with `verify-proverif-<scenario>`, or combined with its extraction-drift gate through `check-generated-proverif-<scenario>`. After intentional boundary diffs have been reviewed and their hashes refreshed, the separate `make -C beaconcrypt-core check-inventory` command checks the trust-boundary inventory. The reviewed tool bundle is recorded in the [Stage 8 document](impl/formal-verification-stage-8.md#locked-proof-bundle).
...
> For the symmetric ratchet specifically, production stores the extracted `ConcreteRatchetKernel`, whose core-owned fixed-width chains carry one executor for their lifetime and whose fixed cache stores private sequence-tagged material. F* proves exact core-owned request inputs and labels, output partitions, complementary reachable role kernels, all-sequence opposing material equality, and preservation of derivational invariants through public seal/open outcomes. The maintained high-level Rust adapter makes operational state affine, permits messaging only from establishment-gated state, returns inert update snapshots, and provides canonical snapshots plus generation/head CAS and loser fencing through `PersistentServer`. Its `SnapshotStore` is trusted for payload integrity and provenance, linearizable CAS, durability, and rollback resistance; snapshots have no cryptographic authentication or encryption. F* and ProVerif do not prove those ownership, codec, store, or deployment mechanisms. Real key/nonce no-reuse remains conditional on faithful HKDF execution and output noncollision, while physical/compiler erasure and absence of retained copies also remain outside the proof.
>
> A dedicated state-neutral receive trace rejects a forged future frame twice from the exact same receiver state, later accepts the authentic target while publishing only its skipped key, rejects replay, and accepts delayed delivery. A separate exact capacity leg accepts sequence 51 with 49 skipped keys, rejects sequence 53 because `49 + 2 > 50`, consumes one cached key, and then accepts sequence 53 while publishing only sequence 52. Explicit compromise after rejection discloses the unchanged live chain and empty cache: future material is exposed because it remains derivable from that chain, not because rejection retained anything. General full-state rejection equality and abstract derivational reachability come from F*. F* also proves non-vacuous exact preparation for valid admitted future and cached targets, conditional exact publication when the callback accepts, public replay neutrality, arbitrary-retry equivalence after finite repetitions of one fixed rejected operation, rejection capacity preservation, and the conditional 49-entry fresh maximum-gap result. The finite ProVerif traces supply concrete cryptographic and exact-schedule witnesses; byte-level parsing and arbitrary schedules remain outside them.
>
