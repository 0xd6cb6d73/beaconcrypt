...
- One canonical 182-fact production transcript interface consumed by every scenario and checked against compiled core builders, mutable covered adapter/commitment/schema sources, the exact Phase-1, Phase-2, and `CryptoFrame` schema ordinals and mappings, the covered Server and Beacon endpoint caller sites, and its ProVerif declaration, scoped process fixtures, active-quantum witness, and ideal frame-constructor sites.
