...
- A successful distance-50 receive from an empty cache ends with exactly 49 skipped entries. Capacity admission remains bounded at 50, and restoration still accepts structurally valid legacy version-2 snapshots with 50 cached entries.
...
- Reachability of two neutral rejected attempts from one exact entry-state term, later honest future delivery, skipped-key publication from success, replay rejection, delayed cached delivery, maximum-gap success with 49 skipped keys, capacity rejection, cached consumption, and forward progress after one slot is freed.
...
The state-neutral receive ProVerif process is one exact finite witness, not an unbounded receive API. It starts after successful symbolic frame construction and models attacker-selected frames that have passed the production parser, sender lookup/check, and minimum payload-length gate. Cap'n Proto parsing, serialized byte lengths, and the handling of malformed, truncated, or wrong-sender inputs remain production-review and regression-test obligations. The short leg rejects a forged sequence-3 frame twice with the exact same receiver-state term, then accepts the honest target, publishes only skipped sequence 2, rejects replay, and accepts delayed sequence 2. The capacity leg accepts sequence 51 from counter 1 with 49 skipped entries, rejects sequence 53 because `49 + 2 > 50`, consumes cached sequence 50, and then accepts sequence 53 while publishing skipped sequence 52. The extracted F* lemmas provide the general exact KDF-output partition, cached-tag invariant, fixed-origin derivational reachability, callback-`None` full-state equality, non-vacuous exact preparation for every valid admitted future plan and valid cached target, conditional callback-`Some` publication shape, replay neutrality, equality of an arbitrary retry before and after any finite repetition of one fixed rejected operation, rejection capacity preservation, and the exact 49-entry fresh maximum-gap success result when the canonical target callback accepts. The ProVerif traces and Rust tests additionally provide concrete cryptographic callback and exact-schedule witnesses. F* does not supply concrete HKDF semantics or output noncollision, cryptographic secrecy, or frame provenance.
...
The core now contains the full symmetric-ratchet control state and its production specialization. The generic layer treats chain and material types parametrically, while `ConcreteRatchetKernel` owns fixed-width `RatchetChain` and `RatchetMaterial` values and carries one executor in both private directional chains. `SymmetricRatchetKdfRequest` owns the exact selected 32-byte input and `SYM_RATCHET_INFO`; the initial and later helpers partition fixed executor outputs, and the concrete public operations select the executor-bearing step internally. `concrete_reachable` gives these values lifetime derivational meaning. Concrete PQXDH root and ratchet HKDF semantics, totality, output noncollision, ephemeral libsodium conversion and callback fidelity, ChaCha20-Poly1305, and the cryptographic meaning of secret bytes remain opaque.
...
The strict F* lemmas retain the generic foundation but add the exact concrete specialization. `symmetric_ratchet_kdf_request_is_exact`, `ratchet_step_uses_exact_chain_and_partition`, and `concrete_ratchet_step_preserves_executor` prove the core-owned input/label, exact `key[0..32] || next_chain[32..64] || nonce[64..76]` interpretation, and fixed lifetime executor. `concrete_kernel_new_is_reachable`, `concrete_seal_next_preserves_reachability`, and `concrete_open_and_finish_preserves_reachability` establish and preserve the concrete invariant through the public lifecycle. The generic public-open theorem proves exact rejection equality; admitted future and cached preparation have explicit existence theorems; callback success conditionally publishes the exact prevalidated result; replay after success is neutral; any finite repetition of one fixed rejected operation leaves an arbitrary later retry equal to its direct transition; rejection preserves cache capacity; and a fresh maximum-gap operation that accepts the canonical target publishes exactly 49 skipped entries. Low-level retention/retry/capacity lemmas describe only the compatibility logical API and restored cached state, while restoration reachability remains conditional.

The verified production trace remains restricted to high-level encryption and decryption from successfully established role state and, for persisted server execution, fresh activation from the trusted store with no rollback. Production enforces this runtime gate by storing a beacon ratchet only in `BeaconState::Established` and server ratchets only in `EstablishedRemote` entries; it makes operational ratchet state affine and keeps manual insertion, reset, associated-data mutation, and mutable ratchet access test-only or crate-private. This is reviewed runtime enforcement rather than a new generic compile-time typestate theorem. F* composes authenticated equal root-input transcripts, a fixed pure root derivation, core-owned ratchet requests, direct role-bound kernel construction, fixed lifetime executors, all-sequence cross-role material equality, and public seal/open reachability preservation. The remaining assumptions include correct and total HKDF semantics, output noncollision, faithful callbacks, complete establishment-gate refinement, canonical serde, trusted snapshot payload integrity/provenance, store atomicity/durability/rollback resistance, hax/Rust/compiler correspondence, crash behavior, and role-specific selection. Core secret arrays zeroize on `Drop`, but physical erasure, compiler treatment, allocator copies, and retained external values remain outside F*.
...
This ProVerif construction is an exact capacity-50 execution, whereas the F* lemmas quantify over every valid refined state satisfying their preconditions and prove whole-plan destination preflight, pure extensional independence of admission rejection from executor/callback choice, complete callback-failure equality, preservation of old associations, zero-step old lookup, and reachability preservation. Every valid admitted nonzero future plan and valid cached target has a non-vacuous exact preparation; callback success remains conditional on the callback returning plaintext and publishes the exact prevalidated state. F* also proves neutral replay after public success, equality of an arbitrary retry before and after any finite repetition of one fixed rejected operation, cache-capacity preservation on rejection, and exactly 49 published skipped entries for a fresh maximum-gap plan when the canonical target callback accepts. The finite ProVerif trace supplies the concrete cryptographic secrecy, compromise, and exact schedule. Concrete callback-free rejection additionally relies on generated/source control-flow review, faithful compilation, and Rust call-count tests. The ProVerif frame is already a symbolic `crypto_frame`: the model does not parse Cap'n Proto bytes or represent every serialized length. Production's pre-admission rejection of empty, unparsable, unknown/wrong-sender, and too-short frames remains an adapter/refinement claim supported by Rust tests, not a conclusion of the symbolic trace.
...
The completed work justifies statements about the implemented PQXDH and concrete symmetric-ratchet composition under explicit primitive and execution assumptions: the core owns exact ratchet KDF requests, direct role-bound kernels, the fixed lifetime executor, all-sequence opposing material equality, and public seal/open reachability preservation. Production adds affine establishment-gated operational state, inert update snapshots, canonical duplicate-rejecting state decoding, and generation/head CAS with fencing. The persistence claim additionally trusts `SnapshotStore` for payload integrity and provenance; snapshots have no cryptographic authentication or encryption. These adapter controls must not be presented as F* proofs of root or ratchet HKDF semantics/totality, libsodium, ML-KEM, callbacks, output noncollision, serialization, trusted-store correctness, store atomicity/durability/rollback resistance, bindings, compiler behavior, physical erasure, RNG behavior, or computational reductions. Core arrays implement zeroization on `Drop`, but that implementation fact does not prove physical/compiler erasure.
