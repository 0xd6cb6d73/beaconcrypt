...
- Successful restoration returns the old logical cache length as its concrete append slot and seals the supplied logical sequence beside the supplied material in that same slot; the cryptographic provenance of an imported pair remains external.
...
Persistence is a separate boundary. The checked builder re-establishes the structural tag and cache invariants for supplied pairs, but serde fidelity and the claim that imported material was cryptographically derived for its supplied sequence remain external.
...
The core now contains the full symmetric-ratchet control state rather than only a send counter. It treats stored chain and material values parametrically while sealing each receive material beside its logical sequence, and its pure splitter exposes the exact interpretation of a fixed 76-byte ratchet KDF result. HKDF semantics, ChaCha20-Poly1305, and the cryptographic meaning of secret bytes remain opaque.
...
The sole opaque ratchet operation is a callback with the shape `fn(&Chain, &[u8]) -> RatchetStep<Chain, Material>`. Production specializes it by invoking HKDF-SHA-512, passing its fixed `[u8; 76]` result to extracted `split_ratchet_kdf_output`, and wrapping the helper's borrowed key, next-chain, and nonce views in production secret types while the complete buffer remains under its zeroizing owner. The kernel delegates receive-until directly to `refined_advance_receive_until`, decides admission, and preflights every destination in its fixed material array before the first callback. Every reported rejection is therefore callback-free and complete-state neutral. A valid admitted future plan then executes the exact bounded consecutive sequence/slot/material trace in callback chain order, seals each result with its sequence, preserves every old tagged association, and reaches the target counter without a reported intermediate failure that could publish only a prefix; an old or current target performs zero steps before lookup. Sequence lookup returns material only after validating the cached tag. Authentication failure retains the post-admission entries, while successful authentication checks the target and old-last tags and performs logical and whole-entry swap-removal internally; a mismatch returns `Missing` without mutation. On send, the kernel returns a non-clonable `RefinedSendKey<Material>` that keeps one allocated sequence paired with its material until consuming `refined_finish_send`.

Persistence keeps the five-field `RatchetManager` format: `send_key`, `recv_key`, `send_ctr`, `recv_past`, and `recv_ctr`. Concrete send-message material is never serialized, and legacy six-field objects containing `send_past` are rejected rather than migrated. Serialization obtains each active `(sequence, material)` pair through the tag-checking kernel accessor. During import, the adapter sorts persisted pairs, calls `start_refined_restore` with both chains and counters, supplies each pair atomically through `refined_restore_receive_key`, and obtains the complete kernel from `finish_refined_restore`. The builder seals each supplied sequence with its material and rejects states with more than 50 outstanding receive keys, but the cryptographic provenance of each persisted pair remains an external adapter and storage obligation.

The strict F* lemmas are parametric in the chain types, material type, and callback. They prove the exact `key[0..32] || next_chain[32..64] || nonce[64..76]` partition, require every active cached tag to equal its logical slot sequence, reject mismatched-tag lookup, and cover whole-plan destination preflight, callback-free complete-state rejection neutrality, exact bounded ordered receive execution to the target counter for admitted future plans, preservation of existing tagged cache associations, zero-step old lookup, failed-authentication retention, successful internal whole-entry swap-removal, sealed restoration, and the refined send-token lifecycle for normal returned transitions. The extracted completion code additionally validates both target and old-last tags before mutation and rejects a mismatch without changing state. The older logical APIs and lemmas remain as compatibility surfaces, but production calls the refined kernel.

The verified production trace remains restricted to high-level `Beacon::{encrypt_message,decrypt_message}` and `Server::{encrypt_message,decrypt_message}` operations starting from fresh or successfully validated role state without rollback. The callback is arbitrary and pure in the theorem, so concrete HKDF behavior, the intended old-chain input and `SYM_RATCHET_INFO` label, faithful passage of the requested 76 bytes to the extracted splitter, faithful typed wrapping of its named fields, callback panic behavior, authentication-result provenance, serde translation and persisted-pair cryptographic provenance, hax/Rust/compiler correspondence, crash atomicity, zeroization and physical erasure, concrete AEAD/hash behavior, role-specific principal or peer selection, and one authoritative non-rollback owner remain assumptions. The byte offsets of the per-step output split are proof conclusions rather than adapter assumptions. The non-clonable refined send token removes the former adapter assumption that a copyable logical capability stays paired with the correct material, but conditional cloning or persistence rollback of the complete ratchet can still fork state.
...
The strict handwritten PQXDH module proves exact tagged-key construction and validation, exact semantic registration IDs, the six-segment root transcript and zero-DH rejection, exact associated data, complementary role ratchets, preservation of the expected server binding, response-key mismatch rejection, authenticated server-sender and assigned-ID checks, successful-acceptance agreement with both fields of the accepting server candidate, checked nonwrapping allocation, binding and collision rejection, and commit/abort state shape. A composed post-validation honest-run theorem relates both role transitions through their authenticated and committed core peer IDs. The concrete HKDF result, production application of the core-provided role offsets to the initial 64-byte two-chain result, and atomic peer-map publication remain adapter obligations; the later per-step 76-byte key/next-chain/nonce offsets are checked separately by the ratchet splitter proof.
