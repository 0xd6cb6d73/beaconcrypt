...
The receive cache is a packed fixed array of at most 50 logical key sequences. Its operations are implemented directly rather than through an assumed `Vec` model, so F* can see allocation, lookup, and exact-key removal. The production adapter stores concrete receive keys in a parallel fixed array indexed by the core's verified slots and maintains this refinement invariant:

```text
concrete_receive_keys[slot].is_some() == (slot < core_state.receive_cache_len())
```

The core now owns every receive sequence-to-slot decision used by production. `advance_receive` returns the append slot for a newly derived key, `lookup_receive_key` returns the current slot for an existing sequence, `finish_receive_with_removal` returns the exact target/last swap-removal plan, and `restore_receive_key_with_slot` returns the append slot used while rebuilding persisted state. The compatibility functions `finish_receive` and `restore_receive_key` are projections of the detailed operations, so there is only one implementation of each logical transition.

The F* lemmas characterize verified lookup in both directions, show that missing and retained finishes return no removal and preserve state, identify the exact old-last slot and moved logical entry on successful removal, prove compatibility-wrapper equivalence, and connect successful restoration to its returned append slot. These theorems prove the pure logical-array decisions; they do not prove that production stored the right secret bytes in those slots.

The high-level beaconcrypt API delegates these ratchet control decisions to this crate. For each admitted logical receive step, the adapter performs exactly one opaque KDF operation and stores the resulting concrete key in the returned append slot. It obtains the current slot again by sequence before authentication completion, applies the exact returned swap/take plan only on successful authentication, and retains both receive representations on failure. For send, the adapter keeps the capability returned by `advance_send` and its concrete KDF output together in one private stack-local transaction, passes the capability to `finish_send` after the single encryption attempt on every recoverable outcome, and never stores either value in `RatchetManager`.

The persistence adapter uses a five-field ratchet format containing `send_key`, `recv_key`, `send_ctr`, `recv_past`, and `recv_ctr`. It serializes each active concrete receive-array slot under the corresponding logical sequence from the core state, rejects missing active material and populated inactive slots, and reconstructs the packed receive array from sorted persisted map entries by placing each concrete key in the slot returned by `restore_receive_key_with_slot`. Imports with more than 50 outstanding receive keys are rejected, as are legacy six-field objects containing `send_past`; this is an intentional compatibility break. See Step 3 of the [formal verification plan](../../doc/formal-verification.md) and the [Stage 3 implementation record](../../doc/formal-verification-stage-3.md).

The Stage 3 correspondence claim covers high-level encryption and decryption traces without state rollback. Concrete HKDF behavior and output splitting, association of returned slots with the correct `KeyMaterial`, serde behavior, authentication-result provenance, crash atomicity, physical erasure, direct low-level receive-ratchet calls, pre-send state forks, and the production peer-map lookup remain explicit adapter obligations or exclusions. The core's `SendKey` is a copyable logical availability value rather than an affine Rust type, so the high-level one-use claim additionally relies on the private adapter transaction not persisting the capability or reusing a copy, leaving no reusable copy after the call, and using one authoritative non-rollback state.
...
The checked ratchet properties cover send and receive counter monotonicity and exhaustion, receive-gap and cache bounds, lookup soundness and completeness, exact append-slot reporting, retry retention, explicit target/last removal shape, detailed/compatibility wrapper equivalence, restore-slot correspondence, exact key consumption and replay rejection, one-use send keys, and non-selected peer isolation. The PQXDH properties cover exact type/role encodings and validation, the semantic registration ID, root and associated-data transcripts, conditional honest-role agreement, complementary ratchet initialization, authenticated key-ID correspondence, replay-status handling, and checked server transactions.
