...
This is not a consequence of collision resistance and is intentionally separate from the binding result. The SSProve model establishes the finite game hop and bad-event shape. The later Lean `CtxComputationalPrivacy.lean` development instantiates the production-width 256-bit prefix, composes the bad event with distinct modified nonce-AEAD IND$ reductions, and obtains `qH / 2^256` specialized or `qH / 2^128` conventional guessing terms. Neither development proves concrete ChaCha20-Poly1305 or BLAKE2b security, a QROM result, asymptotic PPT preservation, adapter correspondence, or protocol-level multi-key composition.
...
Lean now also proves the local honest-seal sampling premise. Before any public key-prefix query, an unused nonce makes the next honest outer input fresh from the complete shared cache: provenance excludes every public origin, and nonce rejection plus injective protocol-width serialization excludes every earlier seal origin. The corresponding lazy-ROM seal transition is exactly direct uniform 512-bit sampling followed by cache installation. The direct local transition still constructs the key-prefixed address, but `CtxSplitCache.lean` proves an exact canonical-cache projection into a non-prefix complete-input cache and a key-free suffix cache indexed by `N ‖ AD ‖ T ‖ LE64(seq) ‖ LE64(sid)`. `CtxIndependentTags.lean` lifts that projection through the full adaptive handler, routes every adversary ROM query only through the public cache and every honest tag only through the suffix cache, and proves `Δ(split canonical, key-free independent tags) ≤ Pr[public hidden-key-prefix query]` with exactly one bad-event charge. `CtxHonestTagSampling.lean` then maintains marked-nonce, pairwise-unique-nonce, and suffix-provenance invariants, proves that an unused nonce makes the next key-free suffix a cache miss, and replaces the full lazy-suffix execution exactly by one explicit independent uniform 512-bit draw per successful seal. Reused nonces reject before sampling, so the hop has zero loss. `CtxNonceAeadIntCtxt.lean` constructs the retained-base primitive forger, proves exact adaptive state/history projection, and bounds every fresh retained-base acceptance by modified nonce-AEAD INT-CTXT advantage with factor one; each fresh CTX seal makes exactly one primitive seal call and reused nonces make none. `CtxNonceAeadIndDollar.lean` identifies the canonical secret-prefix event exactly with a bounded key-probe experiment and proves `Pr[prefix] ≤ Adv_IND$-probe + qH / 2^256`. `CtxNonceAeadIndDollarValidation.lean` uses one extra encryption at a provably unused nonce to derive the conventional bound `Pr[prefix] ≤ Adv_IND$ + qH / 2^128` whenever `qE < 2^96`. `CtxComputationalSecurity.lean` composes those results for the actual typed full-fresh one-key game, charges the prefix event once, and proves `Pr[forge] ≤ Adv_INT-CTXT + ofReal(Adv_IND$) + qH / 2^128 + 2^-512`; it also retains the tighter specialized key-probe alternative. Its machine-checked accounting gives the validation reduction `qE + 1` primitive seals and `64qH + 65qE + 1` total primitive-interface calls, and gives the complete direct game `96 + 64(qH + qE)` uniform-byte calls. Raw-payload normalization, multi-key and production composition, and asymptotic PPT preservation remain open.

`CtxComputationalPrivacy.lean` now composes the confidentiality side for a bounded Boolean adversary with adaptive public-ROM access and nonce-respecting chosen-plaintext sealing. The real game samples one hidden 256-bit key and returns actual `C || T || T*`; the ideal game preserves length and repeated-nonce leakage but independently samples each fresh body, 16-byte base tag, and 64-byte outer commitment while retaining a consistent public lazy ROM and its natural collisions. The canonical shared-ROM-to-independent-tag hop costs the public hidden-key-prefix event once, direct honest-tag sampling is exact, and the Boolean modified nonce-AEAD view reduction has exact real/direct and random/ideal experiment equalities. The structural theorem is `ofReal(Adv_CTX-privacy) ≤ ofReal(Adv_IND$(R_view)) + Pr[prefix]`. Its specialized form adds the distinct key-probe IND$ advantage and `qH / 2^256`; its conventional form, for `qE < 2^96`, adds the distinct validation IND$ advantage and `qH / 2^128`. There is no INT-CTXT, collision-resistance, birthday, `2^-512`, or honest-commitment guessing term in these privacy bounds. The fixed query accounting covers source and reduction-side calls but excludes challenger-private length-matched bodies and 16-byte tags because no total plaintext-byte budget is assumed.

This component theorem gives a one-key confidentiality bound for the modeled nonce-AEAD IND$ and classical-ROM interface. Using it to justify the protocol's `seal_frame`/`open_frame` abstraction additionally needs the key-distribution and execution composition described in [the composition ledger](composition-assumptions.md). It does not prove privacy for concrete BLAKE2b or ChaCha20-Poly1305, normalize raw wire values, connect Rust and adapters, compose ratchet/HKDF or multiple keys, handle the QROM, formalize PPT cost, or mechanically convert the complete ProVerif result into an end-to-end computational theorem.
...
## Raw-input acceptance boundary

`Computational/RawCtxReceive.lean` adds a separate receive endpoint whose adversarial input is only raw bytes. An explicit external decoder returns sender, sequence and payload or rejects; runtime predicates check the sender, nonempty encrypted body and exact AD width. The wrapper invokes the extracted receive driver with a fixed modeled CTX interpreter, using request-owned material and sequence. For a structurally valid entry kernel (`ValidRefined entry.refined`), its success theorem derives record well-formedness and exact inner payload decoding, verifies the commitment and returns the actual base-AEAD plaintext; replay after success rejects without changing the poststate under that same entry premise. Failure restores the full entry kernel without a validity premise, and wrapper totality is unconditional. No adversary-supplied claimed plaintext or well-formed target is a premise of this endpoint.

Production record receive now rejects trailing bytes and concatenated Cap'n Proto `CryptoFrame` messages, prepares shared tag/commitment/ciphertext slices before taking ratchet ownership, and consumes the core open request inside the executor that checks commitment and calls AEAD. Focused raw-input, malformed-field, AD/cross-session, replay and rollback regressions check those behaviors. This is stronger execution enforcement and a checked modeled acceptance relation, not a proof of the Cap'n Proto decoder or libsodium semantics. The existing typed one-key probability bound has not been lifted to this raw interface or an adaptive multi-query decryption game; that missing reduction and the KDF/multi-key composition remain explicit obligations.

The single-frame check applies to the record receiver and server sender lookup. It does not establish canonical Cap'n Proto encodings or tighten the separate outer Phase-1/Phase-2 registration-container parsers.
