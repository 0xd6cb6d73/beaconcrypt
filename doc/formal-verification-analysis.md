...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations.
  Lean now mechanizes factor-one probability reductions from the direct one-shot equal-seal modified-CTX commitment game, the stronger general raw-payload misattribution event, and separately named wrong-sequence, wrong-sender, and cross-session record-opening games to BLAKE2b collision-resistance advantage. It proves a factor-one fixed-context retained-tag projection and, over an arbitrary well-formed designated history, an exhaustive same-view union bound that separates every accepted full-fresh CTX forgery into a fresh accepted base `C || T` projection or a context-alias replay under different outer fields. Lean also proves that the alias branch uses genuinely distinct serialized outer-hash inputs, extracts the exact opening from protocol admission, proves reordered-record and foreign-session transition bounds plus exact-zero wrong-sender admission, and equates the current generated 229-byte commitment builder with the ideal CTX preimage. The authenticity classification is not a standard hidden-key nonce-AEAD game, and the result is not yet the paper's complete adaptive CAE oracle/history game; PPT/runtime preservation plus the adapter hash, field-provenance, Hax/Rust, and compiler bridges remain unmechanized, and BLAKE2b-512 collision resistance remains assumed.
