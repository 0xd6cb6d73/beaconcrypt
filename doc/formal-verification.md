...
- One canonical 220-fact production transcript interface consumed by every scenario and checked against compiled core builders, mutable covered adapter/commitment/schema/concrete-ratchet sources, the exact Phase-1, Phase-2, and `CryptoFrame` schema ordinals and mappings, the covered Server and Beacon endpoint caller sites, the two normal-return ratchet message helpers, named checked Lean interpretation anchors, and its ProVerif declaration, scoped process fixtures, active-quantum witness, and ideal frame-constructor sites.
