...
ProVerif establishes these results only in the stated ideal symbolic model. The Phase-2 response controls reuse the exact modeled root, associated-data, ratchet, AEAD, and CTX construction but do not parse Cap'n Proto bytes or prove the Rust adapter equal to that process. The negative control deliberately changes one protocol acceptance gate and does not weaken a cryptographic primitive. The CI synchronization gate establishes the enumerated literal, size, order, offset, use-site, and symbolic-interface facts, but it does not establish concrete HKDF or ML-KEM security, derive ML-KEM public-key binding from IND-CCA, prove complete implementation or model fidelity, or provide computational security for the protocol or primitives.
