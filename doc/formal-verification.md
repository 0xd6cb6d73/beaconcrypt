...
For extraction, replace `HashMap` in the core with a bounded, packed sequence whose uniqueness and size are visible to the prover. Restoration can require sorted input even though successful receive uses swap-removal and does not preserve entry order. Production now uses a parallel fixed array: the core returns the append, current lookup, removal, and restoration slots, while the remaining refinement obligation is that the adapter associates the correct concrete key and nonce bytes with those proved logical positions.
...
For an existing sequence, production obtains the current physical slot only through `lookup_receive_key`; it does not maintain a second index or scan the logical cache itself. Authentication failure returns no removal and retains the same logical and concrete receive key. Successful authentication returns the exact target/old-last plan from `finish_receive_with_removal`, and production applies that swap/take before publishing the returned core state. On send, each allocated logical capability and its concrete message key exist only in one private stack-local transaction; the adapter passes the capability to `finish_send` and drops the concrete key after the single encryption attempt, including recoverable failure paths.

Persistence uses a five-field `RatchetManager` format: `send_key`, `recv_key`, `send_ctr`, `recv_past`, and `recv_ctr`. Concrete send-message keys and logical send capabilities are never serialized, and legacy six-field objects containing `send_past` are rejected rather than migrated. The counters are serialized from the authoritative core state; no duplicate core representation or physical slot number is written. Serialization maps active receive slots back to their logical sequences and rejects both missing active concrete material and populated inactive slots. During import, the adapter sorts receive sequence/material pairs and rebuilds the core exclusively through `start_restore`, `restore_receive_key_with_slot`, and `finish_restore`, placing each material value directly in its returned slot. States with more than 50 outstanding receive keys are rejected rather than admitted outside the verified state space.

The strict F* lemmas prove that lookup returns a slot exactly when the unique active logical cache contains the sequence, that detailed completion has the same state and disposition as `finish_receive`, that missing and retained outcomes return no removal and preserve state, that successful completion reports the precise target and previous-last slots and mirrors the logical moved-entry shape, and that detailed restoration returns the append slot while projecting exactly to `restore_receive_key`. This mechanizes the control decisions that production formerly reconstructed, but it does not verify the concrete HKDF result, `KeyMaterial` association, Rust array mutation, serde implementation, authentication flag, compiler, crash atomicity, or physical erasure.

The verified production trace for this stage is restricted to `BeaconCryptPqxdh` operations through the high-level `encrypt_message` and `decrypt_message` methods, starting from a fresh or successfully validated state without rollback. Direct calls to the low-level receive-ratchet and key helpers are outside that trace. `SendKey` is a copyable logical availability marker rather than an affine Rust type: the proof covers sequential use of the unavailable value returned by `finish_send`, but does not prevent copying the original available value or forking pre-send ratchet state. The high-level one-use claim therefore relies on the private adapter transaction keeping the capability and concrete key local, not persisting the capability or reusing a copy, leaving no reusable copy after the call, finishing and dropping the local values after one attempt, and operating on one authoritative non-rollback state. Selection from the production peer map is also an adapter refinement obligation: the core proves the pointwise peer frame rule, while unique peer lookup and the unchanged-state behavior of non-selected map entries remain covered by implementation validation and protocol regression tests rather than extraction. The adapter must also perform exactly one correct HKDF step after each successful receive advance, store and move the corresponding key and nonce in the returned slots, pass truthful authentication results, and publish logical and concrete mutations in the documented order.
