...
### Native Lean computational reductions for PQXDH and modified CTX

The imported [`PqxdhJointKdf.lean`](../beaconcrypt-core/proofs/lean/BeaconcryptCore/Computational/PqxdhJointKdf.lean) module gives the source-fixed HKDF-SHA-512/no-salt interface one canonical 76-byte stream at each exact `(info, input)` address. Its typed public projection interface admits exactly the production `INFO_PQ` and `INFO_R` domains and exposes their common `first32`, `second32`, and `final12` projections, so the 32-byte root, 64-byte initial-ratchet result, and 76-byte record step are prefixes of the same per-address value instead of independent output-length calls. The checked equations give exact widths and domain/address separation, equate the initial left/right halves with the step key/next chain for equal `INFO_R` input, and connect the projections to `rootSecret`, `rootChains`, `ratchetOut`, `nextChain`, and `msgMaterial`. The separate `ProductionHkdfPrefixConsistent` predicate records a functional fact of the production wrapper; it is neither a cryptographic assumption about alternate primitives nor an attempt to prove HKDF, HMAC, or SHA-512 internals.

[`PqxdhJointKdfGame.lean`](../beaconcrypt-core/proofs/lean/BeaconcryptCore/Computational/PqxdhJointKdfGame.lean) turns that surface into a bounded adaptive Boolean game. Every public projection query under either fixed domain is forwarded to one complete-stream primitive query; the lazy random implementation samples exactly 76 bytes only on a cache miss and reuses the stored stream on every hit. The underlying named primitive source is expressed over raw `(info, input)` addresses, but a checked invariant proves that the constructed public-view reduction emits only the two typed production-domain images. The reduction preserves `qU` adversary-uniform queries and `qKdf` logical KDF queries without multiplicative loss, and its real- and random-world equations terminate at a specifically named fixed BeaconCrypt HKDF-SHA-512/no-salt joint-stream advantage. That endpoint is intentionally not called an ordinary fixed-secret-key PRF advantage: the public labels and inputs vary, no secret PRF key is part of this game, and primitive security remains an external assumption.

[`PqxdhHiddenRoot.lean`](../beaconcrypt-core/proofs/lean/BeaconcryptCore/Computational/PqxdhHiddenRoot.lean) proves the next one-hidden-coordinate root hop. Starting with four fixed 32-byte coordinates and one independent uniform 32-byte final coordinate, it uses the exact source input `FF^32 ‖ DH1 ‖ DH2 ‖ DH3 ‖ DH4 ‖ hidden32`, proves its 192-byte length and injectivity, and leaves every opaque public-context parameter outside the KDF address. A full 76-byte stream is programmed at the hidden address before comparing the shared lazy stream to an independent honest root, preserving consistency for later `first32`, `second32`, and `final12` projections. The candidate vector allocates one optional slot for every logical adversarial root-domain query, whether the lazy handler hits or misses; a slot contains a hidden-coordinate candidate exactly when the queried bytes have the fixed root layout, and the sticky bad event fires when that candidate is the hidden coordinate. Symmetric-domain logical queries, the reduction's honest root lookup, and handler-internal sampling on a miss allocate no candidate slot, so the bad event is at most `qRoot / 2^256`. A separate checked invariant covers the lifted hidden-coordinate sampler, the reduction-owned honest `INFO_PQ` request, and every forwarded adversarial request and proves that the packaged primitive distinguisher emits no stream address outside the exact `INFO_PQ` and `INFO_R` images. The capstone proves `Adv(root real, independent root) ≤ Adv_fixed-HKDF-SHA-512-joint-stream(reduction) + qRoot / 2^256`, with reduction bounds `qU + 32` uniform-byte queries, `qRoot + 1` root queries, `qSym` symmetric queries, and `qRoot + qSym + 1` total stream queries. It begins after the final coordinate is uniform and therefore neither proves nor assumes a generic ML-KEM IND-CCA-to-uniform bridge, public-key binding, output noncollision, multi-session composition, QROM security, or an end-to-end computational interpretation of the concurrent ProVerif model.
