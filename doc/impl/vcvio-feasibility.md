...
The checked relabelling specialization uses an honest `sealRecord` as the source payload and proves that acceptance under a distinct target context has the same factor-one reduction. Its pointwise wrong-sequence, wrong-sender, and cross-session corollaries return the exact two CTX transcripts that collide if a record is accepted under any of those changed fields. Separately named wrong-sequence and wrong-sender record-opening games now let an adversary choose an honest source explanation, independent target material and plaintext, and a distinct well-formed target field; `ctxWrongSequenceAdvantage_le_blake2b_cr` and `ctxWrongSenderAdvantage_le_blake2b_cr` map those adversaries through the relabelling reduction and preserve the factor-one bound. The sender result is deliberately scoped to `openRecord`: `beaconFinish` rejects a mismatched pinned sender before invoking CTX, so its eventual protocol-transition theorem should have advantage zero rather than charge that rejection to BLAKE2b.

The native theorem closure is admission-free. `#print axioms` reports only `propext` and `Quot.sound` for collision extraction, the three pointwise field specializations, and the field-game pointwise implications, and additionally `Classical.choice` for the four probability inequalities; no audited theorem reports `sorryAx`.
...
2. Continue the field-specific and protocol-transition games: the wrong-sequence and wrong-sender record-opening advantages are complete, while the ideal `beaconFinish` wrong-sender precheck still needs its separate zero-advantage statement, and cross-session still needs its separately named record-opening advantage and transition lift.
