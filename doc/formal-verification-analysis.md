...
The extracted ratchet has a logical control layer, a generic refined proof layer, and the production-facing `ConcreteRatchetKernel`. The generic `RefinedRatchet<SendChain, ReceiveChain, Material>` owns control, both live chain values, and a fixed `[Option<CachedReceiveKey<Material>>; 50]` array; the concrete kernel specializes those parameters to private executor-bearing chains and core `RatchetMaterial`. Each private cached record holds both `sequence` and `material`; derivation and restoration seal them together, lookup compares the tag with the requested logical sequence, and completion checks the target and old-last tags before mutation. For example, after sequence 3 arrives first, the cache holds the canonical receive-step results for sequences 2 and 3; successful authentication of sequence 3 removes its complete record while the canonical sequence-2 record remains for delayed delivery.
...
- The production-facing `concrete_open_and_finish` selects `concrete_ratchet_step` internally and delegates its complete receive plan to the refined kernel. Before its first KDF request, the helper preflights every fixed-array slot to which the plan could append. Every reported rejection therefore preserves all control, chain, and material fields, invokes no executor or AEAD callback, and cannot return after publishing only a prefix of the plan.
...
- The kernel-private lookup returns only the canonical material for the requested sequence in a reachable state and rejects a populated slot whose sealed tag differs from that sequence. `concrete_open_and_finish` passes only that selected core material, its sequence, and the supplied frame context to the opaque open callback. Callback `None` retains the admitted state, callback `Some(plaintext)` returns that plaintext only with same-sequence consumption, and `concrete_open_and_finish_preserves_reachability` covers rejection, retention, and consumption. The extracted completion checks both target and old-last tags before mutation and returns `Missing` without mutation on a mismatch; successful completion moves the complete old-last canonical record into a non-last target, clears the old last slot, and publishes the matching logical removal.
...
This ProVerif result is one exact, unrolled capacity-50 execution under ideal cryptography. It is not a quantification over every gap, cache arrangement, retry count, or interleaving. The general state statements come from the extracted F* ratchet lemmas, which prove exact canonical live-chain iterations and cached-material derivations, whole-plan preflight, callback-free neutral rejection, exact bounded execution, zero-step old lookup, derived-material-only access, retention, consumption, capacity, and replay. `concrete_reachable` specializes those facts to the core chain/material types and fixed lifetime executor, while the PQXDH lemmas prove both cross-role material equalities at every sequence. F* still cannot establish concrete root or ratchet HKDF semantics/totality, output noncollision, ephemeral libsodium conversion and AEAD behavior, cryptographic secrecy, compromise, or forgery conclusions of the symbolic trace.
...
2. Production delegates the whole plan and AEAD attempt to `concrete_open_and_finish`, which selects the executor-bearing concrete step internally. F* proves that the extracted operation checks admission and every planned fixed-array destination before the first KDF request, so every reported rejection is callback-free, preserves the complete concrete state, and cannot publish only an executed prefix. From a `concrete_reachable` state, a valid admitted future plan executes the fixed receive step in exact bounded order, advances the live chain and counter to their canonical target iteration, caches canonical material for every intervening sequence, and supplies only the exact selected core material, sequence, and frame context to the open callback. Callback `None` retains that admitted reachable state. F* does not prove executor or callback side effects or panics, crash behavior, role-specific principal or peer selection, or concrete HKDF and AEAD semantics.
3. F* proves that every core-owned KDF request contains the exact old chain and `SYM_RATCHET_INFO`, that each output has the exact key/next-chain/nonce construction, and that the same executor persists through the concrete kernel lifetime. It also proves canonical cached-material derivation, mismatched-tag rejection, callback-`None` retention, callback-`Some` same-sequence consumption, exact whole-record swap-removal, and concrete reachability preservation. Restoration preserves derivational reachability only when trusted persistence premises supply canonical live chains and material for every imported sequence. The production adapter supplies a canonical envelope plus fresh activation CAS, but PQXDH root and ratchet HKDF semantics/totality, output noncollision, ephemeral conversion and callback fidelity, serde/store correctness, trusted payload provenance, compiler and physical-erasure behavior, retained copies, crash handling, and rollback resistance remain outside those theorems.
