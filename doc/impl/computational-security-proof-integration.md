...
Status on 2026-08-27: EasyCrypt is the selected primary language for further computational-game development after assigning zero weight to migration cost and existing infrastructure. It now has checked counterparts for 30 selected bounded capstone areas in the maintained SSProve tree while using substantially less state and package plumbing; SSProve remains an independent Rocq-kernel-rechecked cross-check rather than the main expansion path. This is reviewed coverage of current finite abstractions, not mechanically proved cross-assistant proposition equivalence or completion of the production computational proof. ProVerif runs 17 scenarios and checks exact reachability for every passive canary, passive-classical and modeled-quantum observational equivalence, six symbolic CTX-constructor field-dependency tripwires, and a paired ML-KEM opacity/recovery control. Neither computational prover establishes a positive passive-QPT or QROM theorem. See the [EasyCrypt computational-proof evaluation](easycrypt-computational-security-proof-plan.md) for the clean-slate comparison and the maintained [SSProve cross-check plan](computational-security-proof-plan.md) for the older production-completion gates.
...
The integration has four deliberately different evidence layers:

- ProVerif checks symbolic reachability, secrecy, and correspondence under ideal constructor and destructor equations.
- EasyCrypt is the primary language for probabilistic games, reductions, adaptive adversary interfaces, and concrete advantage accounting under explicit contracts.
- SSProve retains independently kernel-rechecked counterparts for the bounded capstones and remains useful as a cross-check of model and reduction structure.
...
| Passive classical | Transcript observation without network modification or injection over one bounded honest-only session. | The same ideal classical primitives as the active-classical model. | All five secrecy queries and the five-challenge observational-equivalence check are true, and a shared active-scheduler control proves registration commit and all five record deliveries reachable in the same process. | The selected transcript does not reveal or distinguish replacements of the initial, cached, advanced, future, or beacon-to-server record canary. This is a bounded symbolic threat-model check and is logically weaker than the active-classical result. |
| Passive quantum | Transcript observation without network modification or injection over one bounded honest-only session. | Public symbolic recovery of secrets behind Ed25519 and X25519 public keys, while ML-KEM remains opaque. | All five secrecy queries and the modeled-quantum observational-equivalence check are true; progress, classical-key recovery, ML-KEM opacity, and ML-KEM-reveal secrecy failure are independently checked. | The selected hybrid transcript continues to hide and not distinguish replacements of the five canaries after the modeled classical-key recovery capability is enabled while the ML-KEM shared secret remains unavailable. This checks dependency structure only; it is not a computational quantum proof. |
...
The maintained entry points are [`baseline.pv`](../../beaconcrypt-core/proofs/pro-verif/baseline.pv) for active classical, [`passive.pv`](../../beaconcrypt-core/proofs/pro-verif/passive.pv) for both passive secrecy scenarios, [`passive-strong-secrecy.pv`](../../beaconcrypt-core/proofs/pro-verif/passive-strong-secrecy.pv) for the passive-classical and modeled-quantum observational-equivalence checks, and [`active-quantum.pv`](../../beaconcrypt-core/proofs/pro-verif/active-quantum.pv) with [`active-quantum-witness.pvl`](../../beaconcrypt-core/proofs/pro-verif/active-quantum-witness.pvl) for the bounded break. The aggregate runs 17 scenarios, and [`check-results.awk`](../../beaconcrypt-core/proofs/pro-verif/check-results.awk) fixes each expected classification, including the equivalence, reachability, capability, and negative-control results.
...
The passive scenarios use `passive.pv`, which removes replicated malicious endpoints and runs one bounded honest beacon, one server, and the private state sink. ProVerif does not classify positive reachability in passive-attacker mode, so both passive targets depend on an active-scheduler run of the same honest-only process that witnesses `BeaconCommitted` and all five exact `MessageReceived` deliveries. The separate `passive-strong-secrecy.pv` biprocess proves observational equivalence for replacements of the five challenge plaintexts in both the classical theory and the modeled-quantum theory. The passive-quantum target also depends on controls that expose Ed25519 and X25519 recovery canaries, keep the honest ML-KEM contribution opaque, and then reveal the ML-KEM secret in a paired control where transcript secrecy must fail. These are symbolic dependency checks, not a passive-QPT or QROM theorem.
...
The active-classical symbolic result is the specification target for a classical computational protocol proof. The passive-classical result should follow as a restricted-adversary corollary rather than receive an unrelated reduction. The checked EasyCrypt and SSProve passive-quantum capability games expose the classical secret-recovery consequences while keeping honest ML-KEM decapsulation opaque, but a genuine passive-quantum result still requires a QPT or carefully justified post-quantum lifting theorem over classical transcripts. Both current computational developments use classical probabilistic programs and classical oracle calls; neither models quantum state, superposition queries, or QROM access.
...
The checked EasyCrypt active-quantum artifact is narrower than the ProVerif witness: under two abstract recovery contracts and an explicit game-world validation module, it proves that an adversary can recover abstract classical secrets, submit attacker-selected material through the registration API, and make that modeled registration accept. It is a substitution/acceptance break in that game world, not a full beaconcrypt execution or an extraction-linked proof that the production task ciphertext is decrypted.

The EasyCrypt protocol artifact is an abstract sequential multi-session skeleton. It does not reproduce the production receive path's skipped-key cache lookup, cache consumption and filtering, maximum-gap behavior, future-key staging, rollback on failed opens, or complete receive-side ratchet updates; it also omits arbitrary scheduler interleavings and exact transcript/serialization behavior. Its active `deliver` API is an unrestricted plaintext-returning decryption oracle without a challenge-exclusion security game, while its corruption result is a non-evolving establishment secret rather than production ratchet state. Separate EasyCrypt files now prove bounded CTX collision extraction, full six-coordinate Boolean adaptive classical-ROM privacy, and record-integrity classification/collision capstones, but these remain uncomposed with an AEAD game and conditional on pending implementation-representation bridges.

## EasyCrypt bounded capstone coverage

[`theorem-parity.tsv`](../../beaconcrypt-core/proofs/easycrypt/theorem-parity.tsv) maps 30 selected SSProve foundation and security areas to exact EasyCrypt declaration heads. The policy checker rejects missing declarations, duplicate areas, unsupported relation labels, assumption drift, proof-bypass commands, or a change in the reviewed coverage surface. This is a manually reviewed coverage map: the checker does not compare propositions across proof assistants, and several rows intentionally record different bounded encodings or incomparable adversary interfaces. It is not proof-script equivalence, implementation refinement, or a completed beaconcrypt theorem.

The EasyCrypt coverage surface includes the closed active-classical, passive-classical, passive-quantum-capability, and expected active-quantum advantage-one games; arbitrary lossless bounded classical-ROM adversaries for the protocol and each of the five ordered hybrid coordinates; one erased ratchet hop with the full public view; CTX binding extraction with verifier-query accounting; a full six-Boolean-coordinate CTX privacy game with an internally sampled hidden key, arbitrary off-secret table behavior, exact trace/bad bookkeeping, a true-real/programmed-real distribution equality, and a hidden-query bound inside that finite game; and randomized record-integrity classification, cross-context/cross-sequence collision extraction, trace accounting, and an exact one-bit fresh-tag result.

The EasyCrypt development uses named losslessness contracts for abstract higher-order adversaries, two recovery equations for the active-quantum capability witness, and one pending abstract CTX encoding-injectivity bridge. The build inventories the complete normalized declarations rather than only their names. Why3 and CVC5 are part of the EasyCrypt trusted replay boundary; SSProve's corresponding benefit is a Rocq proof term rechecked by `coqchk`. The latter remains valuable independent assurance, but under the user's zero-migration-cost premise it no longer outweighs EasyCrypt's smaller and more direct stateful game proofs for the primary development path.

## Bounded SSProve PQXDH and ratchet cross-checks
...
2. Generalize the finite CTX fields to production-width types, complete the Lean/F*/EasyCrypt representation bridge, and connect the extracted collision to a named primitive collision game with exact hash-query and reduction-overhead accounting.
...
7. Retain the checked paired ML-KEM opacity/recovery controls, and use a quantum-aware framework or reviewed external lifting theorem before promoting the passive-quantum capability result to a QPT or QROM claim.
...
9. Maintain the EasyCrypt exact assumption and bounded-capstone coverage policy for the primary games, and retain the SSProve exact assumption reports, out-of-tree objects, and `coqchk` gate for the independent cross-check.

Completion means checked games and reductions with named assumptions and losses, not merely four passing labels in a fixed ideal game. The supported summary is now: beaconcrypt has a four-way symbolic suite and two independently checked finite computational developments; the EasyCrypt tree has reviewed counterparts in 30 selected bounded SSProve coverage areas, including CTX binding/privacy, record-integrity classification, hybrid establishment, one-hop ratchet, three positive scenario bounds, and the expected active-quantum advantage-one break. Its numerical production-width bound, arbitrary-execution composition, implementation refinement, primitive reductions, and quantum-computational lifting remain work in progress.
...
make -C beaconcrypt-core check-easycrypt
```
