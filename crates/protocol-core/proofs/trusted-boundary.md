...
| AR-06 | Persistence import checks sequence bounds, materializes send/receive maps, sorts receive sequence/material pairs, and rebuilds only through `start_restore`/`restore_receive_key_with_slot`/`finish_restore`, placing material at each returned slot and rejecting oversized, duplicate-slot, or structurally invalid logical state. Serialization preserves the external six-field sequence-keyed schema, emits no physical slots, and rejects both missing active material and populated inactive slots. Duplicate raw JSON map keys may be resolved by ordinary serde `HashMap` parsing before these checks. Serde correctness, persistence confidentiality, storage atomicity, and rollback prevention remain external. |
