...
Lean now machine-checks the displayed [computational advantage inequality](ctx-commitment.md) with factor one for the corresponding ideal `openRecord` misattribution event.
PPT/runtime preservation, the bridge from the F* fixed-width extracted witness to the ideal Lean transcript, BLAKE2b-512 collision resistance, and correct production use of the proved helper remain explicit complexity, primitive, and adapter obligations.
...
Lean now machine-checks the corresponding factor-one probability reduction for the ideal `openRecord` event, while PPT/runtime preservation, the extracted-to-ideal transcript and adapter bridge, and BLAKE2b collision resistance remain unproved or assumed.
