...
The core now contains the full symmetric-ratchet control state rather than only a send counter. It treats stored chain and material values parametrically while sealing each receive material beside its logical sequence, and its concrete adapter passes the exact old chain to a fixed `32 -> 76` opaque primitive before constructing the exact fixed-width interpretation of its result. The initial adapter analogously passes the exact root to a fixed `32 -> 64` primitive and constructs complementary role chains. HKDF semantics, final external-type conversions, ChaCha20-Poly1305, and the cryptographic meaning of secret bytes remain opaque.
...
The verified production trace remains restricted to high-level `Beacon::{encrypt_message,decrypt_message}` and `Server::{encrypt_message,decrypt_message}` operations starting from fresh or successfully validated role state without rollback. F* now proves the opaque primitive receives the exact old chain, fixes both output lengths by type, owns the per-step and initial directional partitions, and constructs fixed-width result values. The remaining KDF assumption is the cryptographic correctness and totality of the two private HKDF-SHA-512 primitives. Their reviewed implementation unconditionally selects the private `SYM_RATCHET_INFO` constant, so label selection is no longer an adapter argument or separate assumption. Final conversion into external libsodium types, primitive panic behavior, authentication-result provenance, serde translation and persisted-pair cryptographic provenance, hax/Rust/compiler correspondence, crash atomicity, zeroization and physical erasure, concrete AEAD/hash behavior, role-specific principal or peer selection, and one authoritative non-rollback owner remain assumptions. The non-clonable refined send token removes the former adapter assumption that a copyable logical capability stays paired with the correct material, but conditional cloning or persistence rollback of the complete ratchet can still fork state.
