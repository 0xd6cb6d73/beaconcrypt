...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations.
  Lean now mechanizes a factor-one probability reduction from the ideal-model CTX misattribution event to BLAKE2b collision-resistance advantage, but PPT/runtime preservation and the extracted-production refinement remain unmechanized, and BLAKE2b-512 collision resistance remains assumed.
...
4. Lean machine-checks a factor-one reduction from the corresponding ideal-model misattribution advantage to BLAKE2b-512 collision advantage; PPT/runtime preservation, collision resistance itself, and the extracted-to-ideal production bridge are not mechanized.
...
The production commitment transcript helper and fixed-width pointwise collision implication are F*-proved, and Lean proves the factor-one probability reduction on the ideal-model surface; PPT/runtime preservation, their representation bridge, the caller, BLAKE2b, parsing, and field provenance remain computational, adapter, or primitive obligations. The injectivity proof now applies one embedded-segment equality lemma at each of the six transcript offsets instead of repeating the same sequence-extensionality argument for every field; this is a proof-structure refactor and does not alter the transcript layout or injectivity statement.
...
> Lean machine-checks the corresponding factor-one ideal-model probability reduction to BLAKE2b-512 collision advantage, while PPT/runtime preservation and the extracted-production bridge remain unmechanized.
