...
Status on 2026-09-01: SSProve can express the composition needed for beaconcrypt, and the state-separating proof pattern already exists in the literature, but the repository's standalone security inequalities still cannot be combined merely by adding them. The viable architecture treats PQXDH as a keying package, treats the symmetric ratchet plus record layer as a keyed package, connects them through a private key interface, and proves contextual replacements through one shared end-to-end experiment. [`StateSeparatedComposition.v`](../../beaconcrypt-core/proofs/ssprove/StateSeparatedComposition.v) implements the first single-run structural slice and a security-shaped optional-ciphertext public view with internal finite sampling, [`StateSeparatedPackageSemantics.v`](../../beaconcrypt-core/proofs/ssprove/StateSeparatedPackageSemantics.v) certifies the exact authenticated and rejected executions of that linked operation for one call and equates its public finite view and advantage with the direct calculation, and [`StateSeparatedIndexedSessions.v`](../../beaconcrypt-core/proofs/ssprove/StateSeparatedIndexedSessions.v) proves bounded two-handle repeated-call equality with a stateful reference even when the first public observation selects the second handle. The indexed result uses one globally cached complete finite sample and ghost handle routing with no new KDF label; it is not production registration-replay semantics, arbitrary multi-user composition, or an unrestricted stock SSProve contextual theorem.
...
This investigation adds [`ProtocolLabels.v`](../../beaconcrypt-core/proofs/ssprove/ProtocolLabels.v). That module records the two exact production ASCII labels, proves their lengths and inequality, gives the existing Boolean ROM tags a proved-injective interpretation, records the 32/64/76-byte uses, and proves that initial and per-record ratchet derivation share one symmetric domain. `StateSeparatedComposition.v` consumes those exact typed uses without adding a role, session, direction, sequence, or initialization-versus-step KDF tag, `StateSeparatedPackageSemantics.v` executes the exact linked bodies rather than introducing a second KDF vocabulary, and `StateSeparatedIndexedSessions.v` uses its Boolean handles only to select ghost locations and static protocol parameters. The indexed module intentionally does not treat the handle as a production session identifier or add it to the PQXDH root input, shared symmetric input, direction, sequence, or domain label. None of these modules proves that the handwritten Rocq literals equal extracted Rust arrays; that representation bridge remains explicit.

No end-to-end computational-security theorem is claimed by this document. The results are structural state-machine, single-run key-correctness, exact one-call semantics, and bounded two-handle adaptive-context milestones, not an unrestricted package-interpreter contextual equivalence, four-corner advantage bound, confidentiality theorem, authenticity theorem, agreement theorem, production record proof, or implementation refinement. The present SSProve results remain bounded finite-game milestones, and production primitive security, implementation refinement, numerical losses, arbitrary schedules and multi-user sessions, and quantum semantics remain open or assumed as stated below.
...
| [`StateSeparatedPackageSemantics.v`](../../beaconcrypt-core/proofs/ssprove/StateSeparatedPackageSemantics.v) | A proof-carrying certificate for the exact linked `RUN` raw code from the empty heap with one indexed value for its sole joint sampler. Authenticated execution returns the direct optional ciphertext and two `Taken` tombstones; rejected provenance returns `None` before sampling or core execution and two `Empty` slots. The authenticated public marginal, uniform finite-source view probability, and distinguishing advantage equal the direct state-separated definitions, while executing the same exact-label KDF bodies. | This evaluator and certificate are intentionally one-call and assumption-safe, not stock `Pr`/`Pr_op` or unrestricted contextual semantics. Its stateless comparison is distinguishable on a second call; the following indexed module supplies the bounded stateful relation, while active scheduling, primitive reductions, production widths, and implementation refinement remain open. |
| [`StateSeparatedIndexedSessions.v`](../../beaconcrypt-core/proofs/ssprove/StateSeparatedIndexedSessions.v) | Four private CKEY slots separate two Boolean handle/role pairs, while one outer cache shares the complete finite ROM sample across them. Exact stream semantics and a stateful reference cover first fresh use, fresh opposite-handle use without another sample, false authentication, same-handle reuse, both handle orders, and a deterministic context whose first public observation selects the second handle. | This is a two-handle theorem over the one-global-sample/two-bucket Boolean ROM abstraction. Handles are ghost routing indices rather than production session identifiers or KDF labels, and the theorem does not model registration replay, active scheduling, arbitrary multi-user sessions, primitive reductions, production widths, stock unrestricted contextual advantage, or implementation refinement. |
...
The bounded indexed extension duplicates the role-local slots for two Boolean handles and places one complete finite ROM sample in an outer shared cache. The first fresh authenticated handle samples and consumes its pair, the other fresh handle consumes its disjoint pair using the cached sample, and false authentication or reuse of a consumed handle is exactly state- and sampler-neutral. A deterministic two-call context may choose the second handle from the first optional ciphertext, and the package execution agrees with a stateful reference on both observations and the residual sample stream. This closes the stateless-harness mismatch for that bounded context while retaining destructive per-handle transfer; it neither implements the paper's administrative interface nor assigns production replay meaning to handle reuse.
...
The indexed experiment keeps the same exact KDF vocabulary. Its Boolean handle routes only to one of four private CKEY locations and selects one of two static authentication, challenge, and root-input parameters; it is absent from the PQXDH root query and shared symmetric query and does not become a role, direction, sequence, or initialization-versus-step domain tag. Both handles share one cached complete sample because a fresh sample per handle could give inconsistent answers to equal KDF inputs and conceal cross-handle collisions in the two Boolean root/symmetric buckets.

The assumption boundary remains narrower than stock SSProve contextual reasoning. In pinned release 0.2.4, retaining generic package/raw sampler syntax in an assumption-reported capstone makes `Print Assumptions` traverse a generic distribution proof with the repository-rejected infinite-sum interchange dependency; package validity, `Parable`, association, interchange, `Pr`, `Pr_op`, and stock contextual-advantage paths are therefore not accepted capstones here. The modules instead kernel-check exact raw executions and expose assumption-admissible proof-erased or direct finite statements. This discharges the one-call package semantics and the bounded two-handle adaptive-context relation, but not production registration replay, an active adversarial network experiment, arbitrary multi-user sessions, unrestricted package contextual advantage, or the computational four-corner hybrid.
...
### Structural lifecycle milestone: one-call certificate and bounded two-handle relation

`StateSeparatedComposition.v` implements the affine key-transfer and production-orientation nucleus needed by Phase 2. It constructs typed keying and keyed siblings around a private two-slot CKEY provider, checks one role- and session-bound `PUT`/destructive `TAKE` path with cleared tombstones, requires authenticated provenance on seal and open, steps the server's left initial half at sequence one, and proves a pure single-run equality to the monolithic key calculation. Its first public-experiment redesign makes `RUN` take only `unit`, samples the exact joint finite source internally, returns an optional ciphertext rather than the tape or opened plaintext, and defines a challenge-indexed game pair plus direct optional-ciphertext view. `StateSeparatedPackageSemantics.v` certifies the exact linked authenticated and rejected one-call executions, including consumed-versus-empty private slots, and proves equality of the authenticated public marginal, uniform finite view probability, and distinguishing advantage with those direct definitions. `StateSeparatedIndexedSessions.v` then allocates two disjoint role-slot pairs, caches one complete finite sample globally, proves fresh/reuse/rejection and both-handle normal forms, and establishes equality with a stateful reference when the first public observation chooses the second handle. It does so without introducing a session-derived KDF label or relying on the pinned stock package-syntax probability path that exposes the rejected interchange dependency. Separate closed snapshots record replay consumption before construction, failed construction without server publication, dropped-response asymmetry, accepted counters, and rejection that terminally aborts a pending beacon while leaving prior server state unchanged. This milestone does not prove production registration replay, arbitrary multi-user or unrestricted contextual composition, Phase 1's production-width HKDF interface, the production-left-half confidentiality reduction, or Phase 2's active adversary experiment and computational hybrid.
...
SSProve is technically suitable for composing PQXDH with beaconcrypt's ratchet because its package algebra can represent the keying-plus-keyed structure, and the checked slices now demonstrate a private consuming CKEY seam with production-correct role orientation, lifecycle snapshots, an assumption-admissible one-call execution certificate, and a bounded two-handle adaptive-context relation to a stateful reference. The latter closes the specific stateless second-call mismatch under one globally cached finite sample, while its Boolean handles remain ghost routing metadata and add no KDF label. It does not satisfy the stock paper theorem's repeatable administrative CKEY, prove production registration replay, or establish unrestricted package-level contextual advantage reasoning. The missing work is not a final triangle inequality: it is the common production-shaped active adversary experiment, the four-corner hybrid, a joint exact-label and prefix-consistent HKDF interface, contextual equivalences for the standalone security hops, arbitrary multi-user and bounded-ratchet state, AEAD/CTX composition, production-width bounds, and deterministic representation bridges.
