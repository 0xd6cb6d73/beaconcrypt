...
This document explains the formal-verification results in plain language. It audits the checked F*, Lean, ProVerif, SSProve, and CryptoVerif sources under [`beaconcrypt-core/proofs`](../beaconcrypt-core/proofs), distinguishes generated translation coverage from behavioral refinement, and records which concrete F* theorems still describe the predecessor executor/callback source snapshot.
...
- Seven CryptoVerif 2.12 models now exercise automated computational protocol proofs: active registration agreement across polynomially many honest beacons and server calls, registration-task confidentiality across polynomially many calls, bidirectional established-channel confidentiality and injective record-origin correspondences across polynomially many sessions and active deliveries, an assigned-ID binding negative control, and three focused passive-PQ, one-step ratchet, and record-integrity probes. CryptoVerif automatically reports the multi-user primitive losses and collision terms. These are handwritten abstractions with no Rust correspondence, not a full beaconcrypt theorem.
...
- The proof does **not** establish security of the primitive implementations or a general computational theorem for the complete protocol. The SSProve games use finite Boolean atoms and ideal primitives; the CryptoVerif results are conditional on named signature, PRF-ODH, KDF, KEM, and record-transform assumptions. The active registration root currently represents one authenticated static-DH contribution, registration confidentiality excludes the active finish oracle, and the established-channel abstraction starts from fresh directional keys instead of the exact ratchet. Full registration/channel/ratchet composition, production-width bad-query bounds, model-to-implementation correspondence, primitive proofs, and BLAKE2b-512 assumptions remain open. The SSProve passive-quantum theorem is explicitly a classical-query capability result, while the separate CryptoVerif passive hybrid experiment is post-quantum sound under its ML-KEM, hybrid-root PRF, shared symmetric-PRF, and AEAD assumptions. The CryptoVerif symmetric-channel abstraction is active-network and PQ-capable only after authenticated keys are assumed; current classical authentication still makes active post-quantum registration/key establishment and end-to-end security false as expected.
...
| CryptoVerif result | A computational game query follows up to the displayed sum of named primitive advantages and collision terms for the handwritten oracle process. | Primitive security, model-to-Rust correspondence, or protocol behavior omitted from the process. |
...
- **CryptoVerif** is an automated verifier for computational protocol games. The current experiments use its oracle frontend with its classical default or post-quantum assumption library as appropriate; they do not use its implementation-generation features.
...
### CryptoVerif computational feasibility models

The [`crypto-verif`](../beaconcrypt-core/proofs/crypto-verif/) directory contains seven oracle-front-end models checked with CryptoVerif 2.12. The active-classical registration models load the pinned `default.ocvl`, while the established-channel model and focused post-quantum-capable probes load `pq.ocvl`. The policy gate rejects locally declared equivalences, equations, collision statements, and security statements, while typed `[data]` functions remain explicit assumptions that modeled transcript and context encodings are injective. The detailed [CryptoVerif evaluation](impl/cryptoverif-evaluation.md) compares this automated protocol role with SSProve's fundamentals-oriented role and records the theorem boundaries and next increments.

#### Developer-facing interpretation

For developers and library consumers, the checked functional statements are:

- A modeled honest beacon that finishes registration has accepted the exact assigned ID, initial task, protected frame, and handshake fields committed by one unique matching modeled honest-server registration. An attacker may prevent completion by dropping the response, so a server commit does not prove that the beacon finished. The model consumes the registration identifier before returning Phase 2 and proves neither retry nor recovery after response loss.
- An honest-server commit for an exact bundle in the model's private honest set has one unique matching beacon initiation. This private-set membership is proof instrumentation rather than a theorem that the server rejects or deauthorizes other valid self-signed registrations.
- A valid self-signed registration is not proof that the application authorized that identity. The model's private honest-bundle classification exists to state its queries; enrollment, routing, tenant separation, and access-control policy remain external.
- A registration response by itself hides which of two attacker-chosen equal-length initial tasks it contains across many scheduled registrations. This privacy result does not let the attacker feed modified responses into a beacon and observe whether they were accepted.
- Once authenticated fresh directional channel keys are assumed, every accepted record in either direction has one unique matching send with the same session, endpoints, direction, non-encrypted context, sequence, plaintext, and frame. For each session and receiving direction, replay of an already accepted sequence and moving a frame to a different bound context do not produce another acceptance except by defeating the assumed tamper resistance of the record protection.
- Under the same established-channel abstraction, an active network attacker cannot distinguish which of two equal-length application messages was protected across many sessions and sends, except with the displayed probability of breaking the assumed record transform. Length and public routing metadata remain visible.
- The passive post-quantum probe keeps one selected equal-length registration plaintext confidential when every X25519 contribution is treated as known, and the one-step ratchet probe keeps one earlier equal-length plaintext confidential after the next chain state is disclosed when the previous chain state was actually discarded. Neither statement proves active post-quantum registration, general forward secrecy, cached-key secrecy, or recovery after compromise.

These are machine-checked properties of handwritten protocol abstractions, not yet guarantees attached to the Rust API. It is accurate to say that the beaconcrypt protocol design has executable computational evidence for the listed behaviors under named primitive assumptions. It is not accurate to say that the shipped library is end-to-end formally proved secure. The [developer-oriented table in the CryptoVerif evaluation](impl/cryptoverif-evaluation.md#what-the-checked-results-mean-for-developers) gives the operational question, answer, and limitation for each result.

[`automated-registration-first-record.ocv`](../beaconcrypt-core/proofs/crypto-verif/automated-registration-first-record.ocv) gives an active adversary polynomially many honest beacon initializations and server registration calls, arbitrary Phase-2 delivery, separately signed and role-tagged Phase-1 fields, pinned server identity, consumed-registration state, unique assigned IDs, and a terminal beacon finish. CryptoVerif proves that every `BeaconCommitted` event has a unique matching `HonestServerCommitted` event under signature EUF-CMA, multi-session static-DH PRF-ODH, output-splitting, and registration-record integrity assumptions. It also proves that an honest server commit selected through the private honest-bundle table has a unique beacon-initiation origin; this second result is primarily a ghost-table and one-use-state invariant rather than an independent cryptographic authentication theorem.

The registration agreement model is a signed-static-DH abstraction, not the exact beaconcrypt PQXDH computation. Only one static-DH contribution supplies modeled secrecy/authentication; the other three DH fields and a KEM ciphertext field are public transcript placeholders. Ed25519-to-X25519 conversion, ML-KEM encapsulation and decapsulation, the KEM shared secret, the exact hybrid HKDF input, ratchet derivation, nonce derivation, and CTX are absent. Valid registrations outside the private honest-bundle table return an attacker-supplied frame. This exposes arbitrary frame values but deletes the real server DH/root/encryption path and its chosen-key/oracle effects, so no simulation establishes that the abstraction preserves every honest-agreement or confidentiality behavior.

[`automated-registration-confidentiality.ocv`](../beaconcrypt-core/proofs/crypto-verif/automated-registration-confidentiality.ocv) proves one global left/right task bit secret across polynomially many honest Phase-1 publications and server registration responses. Its bound automatically accumulates multi-user signature, static-DH PRF-ODH, and registration-record IND-CPA losses. It deliberately has no beacon-finish/decryption oracle. The active agreement theorem and this CPA-only confidentiality theorem are separate and are not claimed to compose into active chosen-ciphertext registration security; CryptoVerif did not close that combined game using the selected predefined AEAD transformations.

[`automated-established-channel.ocv`](../beaconcrypt-core/proofs/crypto-verif/automated-established-channel.ocv) models polynomially many attacker-parameterized abstract sessions with chosen-message sends, arbitrary delivery, modification, replay, and cross-context routing in both directions. From two independent fresh directional traffic keys and an assumed context-aware record transform, CryptoVerif proves a global left/right challenge bit secret and both injective acceptance-to-send correspondences. The protected context directly binds session, peers, direction, associated data, and sequence. This is a PQ-capable symmetric-channel abstraction, not a model of authenticated establishment, the shared initial KDF, sequential ratchet, derived nonces, key/nonce non-reuse, skipped-key cache, compromise, state lineage, or the deployed AEAD-plus-CTX layout.

[`assigned-id-binding-negative-control.ocv`](../beaconcrypt-core/proofs/crypto-verif/assigned-id-binding-negative-control.ocv) changes only the check equating the encrypted assigned ID with the clear outer assigned ID. CryptoVerif then reports that it could not prove beacon-to-server commit agreement while the honest-origin invariant still proves. The concrete modeled mismatch is a relabeling trace: the adversary forwards a valid protected response but substitutes a distinct outer ID, after which the weakened beacon commits event data under that outer ID while the server committed under the encrypted ID. The tool's `Could not prove` output alone is retained as an expected regression classification, not presented as a general attack theorem.

[`passive-post-quantum-one-record.ocv`](../beaconcrypt-core/proofs/crypto-verif/passive-post-quantum-one-record.ocv) gives the adversary all four ordered X25519 contributions and challenges it on two equal-length plaintexts under one honest ML-KEM encapsulation. CryptoVerif proves challenge-bit secrecy with a bound consisting of twice the ML-KEM IND-CCA2 advantage, twice the hybrid-root PRF advantage, two separately charged applications of the same symmetric-KDF PRF, and twice the nonce-based AEAD privacy advantage. This is a conservative passive harvest-now/decrypt-later experiment because it assumes no secrecy from X25519, but it omits signatures, active registration, CTX, multiple sessions, and decapsulation behavior.

[`ratchet-forward-secrecy.ocv`](../beaconcrypt-core/proofs/crypto-verif/ratchet-forward-secrecy.ocv) encrypts one equal-length challenge under material split from a hidden predecessor chain and then exposes the successor chain through a later oracle. CryptoVerif proves challenge-bit secrecy up to twice the ratchet-PRF advantage plus twice the AEAD privacy advantage. The predecessor is erased only because the process no longer returns it; cached keys, retained copies, multi-step compromise, persistence, and physical erasure are not modeled.

[`record-authentication-replay.ocv`](../beaconcrypt-core/proofs/crypto-verif/record-authentication-replay.ocv) permits repeated adversarial receive attempts for one sent frame, binds associated data, sequence, and sender into one injective protected context, and records an accepted tuple only once. CryptoVerif proves the injective acceptance-to-send correspondence up to the assumed integrity advantage of the composed AEAD-plus-CTX transform. This result demonstrates correspondence and replay-state automation but deliberately assumes, rather than proves, that the deployed AEAD and CTX construction jointly realize that transform.

Together these results establish that CryptoVerif can automate useful multi-session protocol slices and reduction accounting, while SSProve remains better suited to the project's custom finite bad-event and state arguments. A meaningful next capstone is registration through one authenticated beacon-to-server confirmation record, followed by the exact hybrid root and ordered ratchet. Passive post-quantum hybrid-registration security must remain a separate theorem with X25519 public. The symmetric-channel abstraction already handles an active network under PQ-capable assumptions once authenticated fresh keys are given, but active post-quantum registration/key establishment and end-to-end authentication are not positive targets while Ed25519 and X25519 remain in the protocol.
...
The modified CTX justification has four deliberately separated layers. F* machine-checks `ctx_distinct_openings_imply_hash_collision`, which constructs a pointwise collision witness from two distinct accepted explanations of one fixed production-format `C || T || U` payload for arbitrary pure hash and AEAD-open functions. SSProve runs a bounded adaptive hidden-ROM verifier and extracts a valid collision from every successful abstract double explanation, with a `q + 2` trace bound. A separate SSProve programming hop bounds the absolute true-real/fresh-ideal CTX decision gap by a secret-transcript query, conditional on a hidden uniform key. ProVerif supplies an explicit ideal-hash negative control in which the base AEAD deliberately multi-opens and removing CTX reverses the query result. None of these layers proves BLAKE2b, the libsodium call, production field provenance, the F*/SSProve representation and production-width bridge, a numerical hidden-query or collision bound, full transformed-AEAD composition, hax/compiler correctness, or compiled machine code.
...
- every CryptoVerif result line exactly matches its reviewed ordered baseline; all six positive models end with `All queries proved.`, the multi-session registration and established-channel models retain their complete multi-user reductions, the three focused probes retain their named primitive terms, and the assigned-ID control retains one proved origin invariant and one expected unproved agreement query without an all-proved marker.

On 27 August 2026, `make -C beaconcrypt-core verify-cryptoverif`, `make -C beaconcrypt-core check-inventory`, the shell syntax checks, and `git diff --check` completed successfully in the dedicated `cryptoverif` worktree. The standalone target uses a minimal pinned CryptoVerif shell and neither builds nor invokes the unrelated F*/Lean implementation-correctness backends.
...
The generated check runs the extraction-backed suite and rejects tracked or untracked extraction drift; the SSProve target compiles every finite game module in the locked Rocq/SSProve shell, exact-checks each assumption report, and runs `coqchk`; the CryptoVerif target checks the exact tool/library and exact-matches each complete reviewed computational bound; and the standalone inventory command checks monitored trust-boundary membership and fingerprints. The dedicated formal-verification workflow distributes the equivalent F*, SSProve, CryptoVerif, per-scenario ProVerif, and inventory checks across independent matrix jobs, then reports the established aggregate status only when every component passes. Together, those commands make the proof reproducible and prevent a query, monitored trust-boundary file, or generated file from silently disappearing or changing without an explicit baseline update. They do not prove that a handwritten model is faithful to production or that the reviewed assumptions hold; those conclusions still require substantive review beyond the mechanical Stage 9 gate.
...
> SSProve supplies a finite hidden-ROM CTX adversary and verifier, extracts a collision from every accepted distinct pair within `q + 2` trace entries, and separately bounds the absolute true-real/fresh-ideal privacy decision gap by a secret-transcript query. The production-width F*/SSProve representation bridge, numerical ROM and BLAKE2b-512 bounds, ordinary-integrity preservation, and complete AEAD privacy composition remain open or assumed.
