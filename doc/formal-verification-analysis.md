...
- Production stores the extracted `ConcreteRatchetKernel`, which specializes the generic `RefinedRatchet` to core-owned `RatchetChain` and `RatchetMaterial` values and seals one fixed KDF executor into both directional chains. The core constructs every KDF request, selects `concrete_ratchet_step` internally in `concrete_seal_next` and `concrete_open_and_finish`, and constructs role kernels directly from the agreed root, so production executes the request and supplies ephemeral AEAD callbacks without persistent role/libsodium conversions or an adapter-reconstructed `RatchetStep`. Concrete HKDF semantics and totality, PQXDH root-derivation semantics, output noncollision, faithful libsodium conversions and callback results, authenticated persistence, crash/concurrency atomicity, rollback prevention, external copies, and compiler correspondence remain assumptions. The core secret-array wrappers implement zeroization on `Drop`, but F* does not prove physical erasure or compiler treatment of those writes.
...
- no concrete key, chain, private lifecycle token, authoritative concrete kernel, or ephemeral callback conversion is independently retained or mutated around the callback-owned kernel calls;
...
- restoration requires the exact five-field ratchet schema, rejects legacy objects containing `send_past`, validates bounds and uniqueness, sorts the exact imported receive sequence/material pairs, rejects malformed or oversized state, binds the fixed executor through `start_concrete_restore`, passes each pair to `concrete_restore_receive_key`, publishes through `finish_concrete_restore`, serializes only entries from a reachable authoritative state, and authenticates snapshot provenance strongly enough to establish that imported live chains equal the canonical iterations at their counters and imported materials equal `material_at` for their sequences; the current persistence format does not provide that authentication; and
- server state and replay history have one authoritative owner, are not forked, and are not rolled back.

The extracted shared kernel now supplies the machine-checked in-memory concrete specialization: core-owned request input and `SYM_RATCHET_INFO`, fixed output sizes and partitions, one executor carried through each kernel lifetime, direct role-bound kernel construction, complementary reachable origins, all-sequence cross-role material equality, and public concrete seal/open reachability preservation. Production no longer performs persistent role/libsodium conversions or reconstructs a `RatchetStep`; its remaining boundary work is root and ratchet HKDF execution plus ephemeral libsodium conversion and callbacks. Private Rust fields, consuming APIs, `ZeroizeOnDrop` on core secret arrays, compile-time size assertions, and regression tests support but do not discharge primitive semantics/totality, callback and parser fidelity, principal/peer selection, snapshot authentication, compiler and physical-erasure behavior, output noncollision, crash/concurrency, and no-fork/no-rollback assumptions.
...
9. Production must faithfully and totally implement PQXDH root HKDF with `PQXDH_INFO` and each core-owned symmetric request with HKDF-SHA-512, and its callbacks must correctly convert borrowed core arrays into ephemeral libsodium values and implement commitment/AEAD semantics. Restoration additionally needs authenticated canonical-chain and canonical-material snapshot provenance; current persistence is not authenticated.
...
3. F* proves that every core-owned KDF request contains the exact old chain and `SYM_RATCHET_INFO`, that each output has the exact key/next-chain/nonce construction, and that the same executor persists through the concrete kernel lifetime. It also proves canonical cached-material derivation, mismatched-tag rejection, callback-`None` retention, callback-`Some` same-sequence consumption, exact whole-record swap-removal, and concrete reachability preservation. Restoration preserves derivational reachability only when authenticated snapshot premises supply canonical live chains and material for every imported sequence. PQXDH root and ratchet HKDF semantics/totality, output noncollision, ephemeral libsodium conversion and seal/open callback fidelity, current unauthenticated persistence, serde correctness, compiler and physical-erasure behavior, retained external copies, crash/concurrency atomicity, and rollback remain outside those theorems.
...
> For the symmetric ratchet specifically, production stores the extracted `ConcreteRatchetKernel`, whose core-owned fixed-width chains carry one executor for their lifetime and whose fixed cache stores private sequence-tagged core material. F* proves that every `SymmetricRatchetKdfRequest` contains the exact root or old chain and `SYM_RATCHET_INFO`, proves the exact 64-byte initial split and 76-byte key/next-chain/nonce split, constructs complementary reachable role-bound kernels, and proves beacon-send/server-receive plus server-send/beacon-receive material equality at every sequence. `concrete_seal_next_preserves_reachability`, `concrete_open_and_finish_preserves_reachability`, `beacon_seal_server_open_preserves_concrete_session`, and `server_seal_beacon_open_preserves_concrete_session` preserve the concrete derivational invariants on every pure callback outcome. Production only executes core-owned KDF requests and performs ephemeral libsodium conversion inside callbacks; it no longer maintains persistent external role/material conversions or reconstructs a `RatchetStep`. PQXDH root and ratchet HKDF semantics/totality, output noncollision, ephemeral conversion and callback fidelity, compiler correspondence, authenticated persistence provenance, crash/concurrency atomicity, and one authoritative no-fork/no-rollback owner remain assumptions. Core secret arrays zeroize on `Drop`, but F* does not prove physical/compiler erasure or absence of retained copies.
>
