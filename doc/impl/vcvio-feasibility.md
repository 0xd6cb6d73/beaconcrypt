...
2. Complete the remaining `beaconFinish` games through `BeaconRecordAdmitted`: the reordered-record factor-one bound is complete, while foreign-session admission still needs its bound and the earlier pinned-sender precheck needs its separate zero-advantage result.
