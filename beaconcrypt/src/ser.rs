...
struct DirectionalRatchetChainRef<'a, Role> {
	chain: &'a beaconcrypt_core::ratchet::RatchetChain,
	_role: PhantomData<Role>,
}

impl<'a, Role> DirectionalRatchetChainRef<'a, Role> {
	fn new(chain: &'a beaconcrypt_core::ratchet::RatchetChain) -> Self {
		Self {
			chain,
			_role: PhantomData,
		}
...
struct SerializableConsumedRegistrations<'a>(
	&'a HashSet<[u8; beaconcrypt_core::pqxdh::REGISTRATION_ID_SIZE]>,
);
...
struct SerializableServerState<'a> {
	...
	consumed_registrations: &'a HashSet<[u8; beaconcrypt_core::pqxdh::REGISTRATION_ID_SIZE]>,
}
...
pub(crate) fn serialize_server_state(
	...
	consumed_registrations: &HashSet<[u8; beaconcrypt_core::pqxdh::REGISTRATION_ID_SIZE]>,
) -> Option<String> {
