...
# Historical formal-verification design and implementation record

This record preserves the earlier F*-based plan and milestone descriptions. Current commands, proof scope, and boundaries are maintained in [Formal verification](../formal-verification.md); historical backend claims below apply to their recorded source snapshots.
...
Lean machine-checks the displayed [computational advantage inequality](../ctx-commitment.md) with factor one for the corresponding ideal `openRecord` misattribution event and checks equality between the current generated transcript builder and the ideal preimage.

SSProve independently machine-checks a bounded hidden-ROM CTX verifier, collision extractor, `q + 2` trace bound, separate true-real/fresh-ideal privacy hop, ideal PQXDH-plus-ratchet confidentiality games, a root-derived masked-bit hybrid-combiner reduction, a one-step erasure-conditioned ratchet hop, combined-authenticator record event/collision reductions, and an exact `1/2` fresh-guess bound for its one-bit ideal table.

PPT/runtime preservation, the cross-prover and production-width representation bridges, production-width collision, guessing, and hidden-query bounds, primitive instantiation, composition, BLAKE2b-512 collision resistance, and correct production use of the proved helper remain explicit complexity, primitive, and adapter obligations.
...
- ML-KEM opacity under the modeled classical-key recovery capabilities and a paired total ML-KEM secret-key-recovery canary; those controls distinguish opacity from total key recovery, while a separate same-identity multi-epoch differential tests cross-key re-encapsulation.
- An explicit production-theory ML-KEM public-key-binding or strong cross-key shared-secret collision-resistance assumption, plus a deliberately weakened theory in which learning one shared secret permits public re-encapsulation under another key; generic IND-CCA is not claimed to imply the production assumption.
- Exact non-vacuity of the unsupported multi-epoch path, old-key compromise, attempted ciphertext substitution, and ordinary one-shot registration; the strong theory preserves the new-session canary and public-key/ciphertext agreement, while the weak theory exposes the accepted root and canary and breaks agreement.
...
- One canonical 397-fact production transcript interface consumed by every scenario and checked against compiled core builders, mutable covered adapter/commitment/schema/concrete-ratchet sources, the exact Phase-1, Phase-2, and `CryptoFrame` schema ordinals and mappings, the covered Server and Beacon endpoint caller sites, the two normal-return ratchet message helpers, the finite receive-state fixtures and exact queries, registration lifecycle and replay-key scope, the source-shaped initial-ratchet handoff, named checked Lean interpretation anchors, and its ProVerif declaration, scoped process fixtures, active-quantum witness, and ideal frame-constructor sites.
- An exact Phase-1 `InitKex` boundary mapping `identityKey @0`, `preKey @1`, `oneTimeKey @2`, and `pqKey @3` to the encoded Ed25519 identity and three attached Ed25519 signatures over the separately tagged prekey, one-time, and ML-KEM-768 values; Server verification returns those tagged values under the decoded identity and passes identity, prekey, one-time, PQ to core tag/role validation.
- Fidelity mutations for both label bytes, domain aliasing, separate initialization/step domains, associated-data identity order, X25519 roles, both KEM agreement fields, every individual CTX field, every Phase-2 response manifest/schema/process/source mapping, the separately counted 163-case Phase-1 matrix, the separately counted 223-case `CryptoFrame` schema/seal/open/core/symbolic matrix, the separately counted 242-case endpoint caller/source/scoped-fixture matrix, the separately counted 154-case normal-return ratchet-driver/core/Lean-anchor/ProVerif matrix, the separately counted 1,230-case finite receive-state/core/query matrix, the separately counted 177-case registration-lifecycle/replay-scope matrix, and the separately counted 223-case initial-ratchet source/core/adapter/Lean-anchor/scoped-ProVerif matrix, for 2,452 exact deterministic drift cases in total.
- An exact initial-ratchet source seam from the caller-supplied Server `RegistrationOutput` and Beacon candidate root through the 32-byte `KexDerivedSecret` accessor, role-typed pending request, 64-byte response, `0..32`/`32..64` split, zero-counter complementary kernels, the exact PQXDH-root binding in each of three scoped ProVerif roles, and the five scoped chain uses and their material bindings. This establishes deterministic text synchronization only. Complementarity requires equal endpoint roots and equal outputs from the same faithful deterministic response function; it neither proves the roots equal nor proves HKDF-SHA-512, response provenance by type, Rust-array/ProVerif-term equality, or semantic source-to-model refinement.
- Exact production endpoint mapping for ordinary Server and Beacon send/receive plus both sides of the initial registration frame: local target metadata remains distinct from the serialized sender, associated data is server-first, each role selects the stated peer or single ratchet, and registration publishes only the post-initial-frame peer/Beacon state. The exact ProVerif call and event arrays are finite main-honest and malicious-registration fixture locks, not a universal production scheduling theorem.
- A focused Phase-2 response-binding control in which a genuine response commits, three independently reachable responses with a wrong outer identity, relabeled outer assigned ID, or wrong authenticated inner sender never commit, the wrong-identity and wrong-sender variants reach their internal comparison gates, and the accepted markers immediately preceding the sole commit correspond injectively to the pinned identity, opened assigned-ID prefix, and pinned sender.
- A paired negative control that reuses the exact response and finish process while weakening only the equality gate between the authenticated eight-byte prefix and outer `keyId`; the same relabeled response then commits and exposes a canary, only the assigned-prefix correspondence fails, and the unchanged identity and sender gates continue to reject their one-field mutations.
...
The canonical-interface mutations are fidelity controls: they reject deterministic production/model drift even when the modified model could still prove secrecy. The deliberately weakened HKDF, encoding, CTX, and ML-KEM theories are security controls instead and must expose their reviewed attack or correspondence-failure witnesses.
...
ProVerif establishes these results only in the stated ideal symbolic model. The Phase-2 response controls reuse the exact modeled root, associated-data, ratchet, AEAD, and CTX construction but do not parse Cap'n Proto bytes or prove the Rust adapter equal to that process. The negative control deliberately changes one protocol acceptance gate and does not weaken a cryptographic primitive. The Phase-1 fidelity check records that Rust evaluates its pure signature getters PQ, prekey, one-time while ProVerif evaluates its pure gates prekey, one-time, PQ; their semantic field mapping is exact, but evaluation-order identity is neither claimed nor security relevant. The CI synchronization gate establishes the enumerated literal, size, order, offset, use-site, and symbolic-interface facts, but it does not establish concrete Ed25519, HKDF, or ML-KEM security, derive ML-KEM public-key binding from IND-CCA, prove Cap'n Proto parser or compiler correctness, prove complete implementation or model fidelity, or provide computational security for the protocol or primitives.

The supported `Beacon` API invokes its identity, prekey, and ML-KEM keypair generators together, and `get_registration_bundle` can return `Some` at most once per live object: it accepts `Fresh` or `FreshWithCoins`, and only successful serialization advances to `InitSent`, while every covered later transition remains ineligible. Pre-transition failure can retry without a byte-equality guarantee. Public secret-key accessors permit caller-constructed signed bytes outside that method, and key uniqueness is not proved. Production has no same-identity ML-KEM rotation operation, and its exact identity-plus-one-time replay ID means HB-49's PQ-only second bundle is consumed after a first successful `get_shared_secret`. The multi-epoch re-encapsulation fixture is therefore a non-production regression control for a hypothetical extension, not a claim that the current Beacon API performs rotation. Such an extension would need to preserve a justified public-key-binding property or cryptographically bind the selected KEM key into the authenticated handshake transcript, together with explicit authenticated epoch and replay semantics.
...
9. **Complete:** maintain a reviewed inventory of every opaque Rust function, assumed primitive law, adapter refinement, proof-library assumption, generated-code exception, and handwritten backend fragment, with a standalone CI drift gate.
10. **Complete:** extract and prove the production CTX transcript order and injectivity, machine-check the pointwise collision-witness theorem and bounded hidden-ROM SSProve binding/privacy games, add a deliberately multi-opening ProVerif negative control with an exact no-CTX failure witness, and document the remaining production-width, cross-prover representation, and primitive-reduction lifting.
11. **Complete for the stated deterministic surface:** add the handwritten ideal modified-PQXDH model and checked refinement from Aeneas-generated byte encodings and crypto-free registration transitions, with concrete primitive semantics and record-layer adapter composition remaining explicit.
12. **Complete for ideal-model CTX binding:** define the raw-payload double-opening and honest-record relabelling games in Lean and prove factor-one reductions to VCVio BLAKE2b collision-resistance advantage, with PPT/runtime preservation and the extracted-transcript/adapter bridge remaining open.
13. **Complete for the enumerated ProVerif transcript boundary:** add one canonical machine-readable interface, check it against compiled core builders and adapter wiring, load it in every scenario, and require all reviewed fidelity mutations to fail without calling the result extraction, refinement, or computational security.
...
Lean machine-checks the corresponding factor-one probability reduction for the ideal `openRecord` event and equality between the current generated transcript builder and the ideal preimage. SSProve independently runs a bounded adaptive hidden-ROM CTX game, verifies both candidate explanations with exactly two additional queries, extracts an unequal-input equal-output collision, and proves a `q + 2` trace bound. A separate SSProve table-programming game proves the true-real/programmed-real representation and bounds the absolute deterministic true-real/fresh-ideal decision gap by the probability of querying the secret transcript. PPT/runtime preservation, the cross-prover and production-width representation bridges, numerical collision and hidden-query bounds, the adapter bridge, complete AEAD privacy/integrity composition, and BLAKE2b collision resistance remain unmechanized or assumed.
...
`make verify` now regenerates or checks every configured backend in the revision-pinned proof shell. Its ProVerif result parser rejects timeouts, missing or substituted queries, unexpected true/false classifications, and every unproved or inconclusive security query. The aggregate ProVerif check runs its 27 `check-proverif-<scenario>` targets concurrently, and the corresponding `verify-proverif-<scenario>` and `check-generated-proverif-<scenario>` targets support isolated locked-shell and generated-drift checks. `make check-generated` covers the generated F*, ProVerif, and Lean artifacts, while SSProve objects are checked separately under `target/formal-verification/ssprove`.
