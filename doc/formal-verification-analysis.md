...
[`StateSeparatedComposition.v`](../beaconcrypt-core/proofs/ssprove/StateSeparatedComposition.v) is the first structural PQXDH-to-ratchet seam. Its typed SSProve packages link state-free keying and keyed siblings to a private CKEY provider with distinct role slots; each slot permits one role/session-checked `PUT`, one destructive `TAKE`, and a cleared tombstone, while the pure semantics also prove overwrite, retake, cross-role, and cross-session rejection. The server receives `(send=L, receive=R)`, the beacon receives `(send=R, receive=L)`, both authenticated endpoints use `L` for the first response, and the response is explicitly sequence one. Closed capstones require the transported authenticated-provenance bit, reject false provenance, reuse the exact root and shared symmetric KDF labels and 32/64/76-byte sizes, and prove a pointwise single-run equality to the monolithic calculation. Separate scenario snapshots capture replay consumption before construction, failed construction without server publication, a published-but-dropped response, complementary accepted counters, and terminal abort of a pending beacon that preserves any prior server snapshot.

This new module is not an end-to-end security theorem. The response is a one-bit XOR key-correctness harness with no nonce, associated data, AEAD, CTX, identifier binding, serialization, or attacker-facing network interface; the accepted root input and Boolean provenance are parameters rather than an intended-peer agreement proof; and the lifecycle snapshots are not reachability results from the executable package. The CKEY interface is a custom consuming `PUT`/`TAKE` store rather than the state-separation paper's repeatable `GET`/`HON` interface. The reported equality concerns the pure single-call bodies, not the package interpreter, and the one-shot package is distinguishable from the stateless monolithic harness under a second call. Package-level validity and stock interchange/advantage results expose assumptions outside the current allowlist, so the module assumption-reports only its closed pure capstones and leaves the contextual bridge and four-corner bound open.
...
The closed protocol capstones use a direct finite pushforward rather than SSProve's high-level general package advantage bridge because the pinned bridge pulls in an unaccepted infinite-sum interchange dependency. The executable packages remain useful as bounded operational presentations, but their assumption-safe equality theorems are presently stated for direct deterministic calculations rather than the package interpreter. All attacker-facing extensions are sound reduction shapes rather than negligible production claims: their one-bit domains permit exhaustive queries or non-negligible guesses, and no production-width bad-event bounds are supplied. The original closed confidentiality models fix one session, one abstract sequence-zero record, public metadata, and a statically selected action; the hybrid, one-step erasure, and integrity files remain standalone hops. The new one-shot CKEY module adds a structural sequence-one lifecycle seam but still does not compose those advantages. The [end-to-end composition investigation](impl/ssprove-end-to-end-composition.md) records that milestone and specifies the remaining repeatable private key interface, four-corner state-separating hybrid, shared prefix-consistent exact-label HKDF package, and production and cross-prover obligations needed to turn the hops into a common experiment.
