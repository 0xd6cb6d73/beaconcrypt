...
/// Check that the key ID outside `appCipherText` is the same value carried in
/// its authenticated plaintext prefix. Only the resulting typestate can be
/// committed.
...
mod tests {
	...
	fn beacon_candidate_commits_or_aborts_explicitly() {
		...
		assert!(
			authenticate_registration_key_id_binding(candidate_for_binding, 12u64.to_le_bytes())
				.is_ok()
		);
		assert!(matches!(
			authenticate_registration_key_id_binding(candidate(), 13u64.to_le_bytes()),
			Err(RegistrationError::KeyIdMismatch)
		));
		let authenticated =
			authenticate_registration_key_id_binding(candidate(), 12u64.to_le_bytes()).unwrap();
		assert_eq!(beacon_commit(authenticated).assigned_key_id(), 12);
