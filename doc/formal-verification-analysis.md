...
- For every valid admitted future plan, the extracted executor performs exactly the bounded derivation count in order, appends exactly the consecutive sequences from the old counter plus one through the target to exactly the consecutive slots beginning at the old cache length, places each abstract callback result's material in its corresponding slot, preserves every old active sequence/material association, and finishes with the receive counter equal to the target. An old or current target performs zero steps and leaves the complete refined state unchanged before ordinary key lookup determines whether material is still available.
...
- concrete HKDF call labels, HKDF implementation behavior and output slicing, AEAD result provenance, callback panic behavior, crash/concurrency atomicity, compiler correspondence, retained copies outside the authoritative refined state, or rollback; the extracted kernel now proves callback-free reported rejection plus exact normal-return whole-plan association and ordering for the chain and material values returned by its opaque callback;
...
The extracted `RefinedRatchet` owns the logical control value, both live chains, and the fixed concrete-material slots. Its F* invariant proves that every active logical sequence is paired with the material returned for that step in the same unique slot and every inactive slot is empty. Its whole-plan theorem proves destination preflight before callback application, callback-free complete-state neutrality for every reported rejection, the exact bounded ordered sequence/slot/material trace and target counter for a valid admitted future plan, preservation of every old active association, and a zero-step unchanged result for an old or current target before lookup. Receive lookup uses the same pairing, missing and retained completion preserve the complete state, successful completion performs the exact material swap-removal with its logical update, and restoration appends each supplied sequence/material pair atomically. The production connection still assumes:
...
- the adapter passes the exact F*-verified root input and labels to HKDF, uses
  ...
  offsets correctly;
- the production ratchet-step callback passes the intended old chain and `SYM_RATCHET_INFO` to HKDF-SHA-512, splits the exact output into the returned next chain, key, and nonce, and has no relevant stateful effect beyond returning those values; F* proves callback-independent reported rejection and exact whole-plan normal-return association, while the extracted definition preflights all destinations and applies each pure callback in the admitted sequence order with the matching counter and slot; callback panic and crash behavior remain external;
...
> For the receive cache specifically, the extracted `RefinedRatchet` owns the logical cache and fixed concrete-material slots, and production delegates directly to its `refined_advance_receive_until` operation. F* proves that all planned destinations are preflighted before the first abstract KDF call; every reported rejection is callback-free, preserves the complete state, and cannot publish a prefix; every valid admitted future plan produces the exact bounded ordered sequence/slot/material trace, preserves old associations, and reaches the target counter; and an old or current target performs zero steps before lookup. It also proves full-state retention on missing or failed authentication, exact sequence-and-material swap-removal on success, and atomic restoration of each supplied pair. Production still has to implement the callback with the intended HKDF label and split, supply truthful authentication and persisted pairs, serialize faithfully, preserve one authoritative non-rollback state, and rely on correct hax/compiler translation; callback panics, crash/concurrency atomicity, and physical erasure are not proved.
>
