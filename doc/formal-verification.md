...
Persistence is a separate boundary. `start_concrete_restore`, `concrete_restore_receive_key`, and `finish_concrete_restore` bind the fixed executor and re-establish structural tag/cache invariants for supplied core values, while the generic F* builder lemmas preserve derivational reachability only under canonical-chain and canonical-material premises. Current serde fidelity, snapshot authentication, rollback prevention, and the claim that imported material was cryptographically derived for its supplied sequence remain external, so importing arbitrary structurally valid state does not establish `concrete_reachable`.
...
The core constructs `SymmetricRatchetKdfRequest` with the exact root or old chain and fixed `SYM_RATCHET_INFO`; production's `initial_ratchet_hkdf` and `ratchet_hkdf` functions only execute the request and return the statically sized output. `derive_initial_ratchet_kernel` constructs `ConcreteRatchetKernel` directly, and each concrete chain carries `ratchet_hkdf` unchanged into the next step, so production no longer rebuilds a `RatchetStep`. Public `concrete_open_and_finish` and `concrete_seal_next` select the concrete step internally, lend the exact core material and sequence plus frame context to one callback, and preserve the same retention/consumption semantics as the generic proof. The callbacks convert borrowed key and nonce arrays to libsodium types only for the operation; those external values are not persistent ratchet storage.

Persistence keeps the byte-identical five-field `RatchetManager` format: `send_key`, `recv_key`, `send_ctr`, `recv_past`, and `recv_ctr`. Concrete send-message material is never serialized, and legacy six-field objects containing `send_past` are rejected rather than migrated. Serialization obtains each active `(sequence, RatchetMaterial)` pair through the tag-checking kernel accessor. Import sorts persisted pairs, calls `start_concrete_restore` with both chains, counters, and the fixed executor, supplies each pair through `concrete_restore_receive_key`, and publishes through `finish_concrete_restore`. The builder seals each sequence with its material and rejects more than 50 outstanding receive keys. F* proves conditional reachability only when live chains and appended materials have authenticated canonical provenance; the current format does not discharge those premises, so persisted-pair provenance, snapshot authentication, forks, and rollback remain external storage obligations.

The strict F* lemmas retain the generic foundation but add the exact concrete specialization. `symmetric_ratchet_kdf_request_is_exact`, `ratchet_step_uses_exact_chain_and_partition`, and `concrete_ratchet_step_preserves_executor` prove the core-owned input/label, exact `key[0..32] || next_chain[32..64] || nonce[64..76]` interpretation, and fixed lifetime executor. `concrete_kernel_new_is_reachable`, `concrete_seal_next_preserves_reachability`, and `concrete_open_and_finish_preserves_reachability` establish and preserve the concrete invariant through the public lifecycle, while the generic lemmas retain exact cache, rejection, retention, consumption, and conditional restoration results.

The verified production trace remains restricted to high-level `Beacon::{encrypt_message,decrypt_message}` and `Server::{encrypt_message,decrypt_message}` operations starting from fresh or successfully validated role state without rollback. F* now composes authenticated equal root-input transcripts, one fixed pure root derivation, core-owned ratchet requests, direct role-bound concrete kernel construction, a fixed executor carried for each kernel lifetime, all-sequence cross-role material equality, and concrete public seal/open reachability preservation. Production only executes the KDF requests and supplies callbacks; persistent conversions into role/libsodium types and adapter-reconstructed `RatchetStep` values have been removed. The remaining assumptions include correct and total PQXDH root and ratchet HKDF-SHA-512 semantics, relevant output noncollision, faithful ephemeral libsodium conversion and commitment/AEAD callbacks, serde and authenticated persistence provenance, snapshot/fork/rollback prevention, hax/Rust/compiler correspondence, crash atomicity, and role-specific principal or peer selection. Core `RatchetChain`, `RatchetKey`, `RatchetNonce`, `RatchetMaterial`, and request values zeroize on `Drop`, but physical erasure, compiler treatment, allocator copies, and retained external values remain outside F*.
...
The completed work justifies statements about the implemented PQXDH and concrete symmetric-ratchet composition under explicit primitive and execution assumptions: the core owns exact ratchet KDF requests, direct role-bound kernels, the fixed lifetime executor, all-sequence opposing material equality, and public seal/open reachability preservation. Production only executes those requests and performs ephemeral libsodium conversion inside callbacks; it no longer owns persistent role/material conversions or reconstructs ratchet steps. This must not be presented as a proof of root or ratchet HKDF semantics/totality, libsodium, ML-KEM, callback correctness, output noncollision, serialization or authenticated persistence provenance, rollback prevention, bindings, the compiler, physical erasure, the operating-system RNG, or computational reductions. Core arrays implement zeroization on `Drop`, but that implementation fact does not prove physical/compiler erasure.
