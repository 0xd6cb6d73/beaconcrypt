...
The native theorem closure is admission-free. `#print axioms` reports only `propext` and `Quot.sound` for collision extraction, the three pointwise field specializations, and the field-game pointwise implications, and additionally `Classical.choice` for the five probability inequalities; no audited theorem reports `sorryAx`.
...
2. Lift the completed wrong-sequence, wrong-sender, and cross-session record-opening games to the ideal `beaconFinish` transition: first expose the exact post-CTX admission event, then prove factor-one bounds for reordered and foreign-session records and a separate zero-advantage result for the earlier pinned-sender precheck.
