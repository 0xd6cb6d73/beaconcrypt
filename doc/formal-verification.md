...
For a plain-language audit of the properties actually checked by the current proof corpus, including a scenario-by-scenario developer explanation of the computational results, see [what beaconcrypt's formal verification proves](formal-verification-analysis.md#what-the-computational-proofs-mean-in-application-terms).
...
- EasyCrypt as the primary environment for new computational game development: probabilistic programs, arbitrary adversary modules, relational hops, exact finite bounds, and stateful protocol abstractions under explicit contracts.
- SSProve as an independently implemented, Rocq-kernel-rechecked cross-check of the bounded computational capstones.

F* and Lean remain the implementation-correctness extraction targets. The hax ProVerif backend is experimental, so its models and generated output need closer review, but it addresses a different class of questions than functional verification. Direct hax-to-EasyCrypt extraction is currently rejected because the pinned experimental backend does not emit usable production artifacts, while direct hax-to-SSProve extraction remains rejected by its unsafe-prelude gate. EasyCrypt therefore consumes a reviewed contract boundary backed by named Lean/F* theorems rather than claiming direct proof transfer. Under a zero-migration-cost comparison, EasyCrypt is the better primary language for the remaining computational games because its native imperative modules and relational probabilistic logic match the stateful protocol reductions; SSProve remains valuable as a separately encoded kernel-rechecked cross-check. None of these layers is expected to prove the cryptographic primitives themselves.
...
EasyCrypt and SSProve now independently check bounded capstones at comparable finite abstractions. The EasyCrypt suite proves the closed active-classical, passive-classical, and passive-quantum classical-interface confidentiality games, and an explicit active-quantum recovery/substitution game with advantage one; adaptive classical-ROM bounds for the protocol and its ordered five-coordinate DH1/DH2/DH3/DH4/ML-KEM hybrid; a one-hop erasure-conditioned ratchet bound; CTX collision extraction with exact trace accounting and full six-Boolean adaptive true-real/programmed/fresh privacy; and randomized record-integrity classification, collision reductions, trace accounting, and exact one-half fresh-tag acceptance. Its abstract state skeleton additionally checks first-writer-wins registration and bounded send advancement. The adaptive theorems assume only the named adversary losslessness conditions needed for probability equalities; the CTX pointwise layer assumes the named pending representation bridge; the active-quantum stateful witness assumes the two named recovery equations; and record integrity treats the combined authenticator as one hidden ideal table. The active interface remains a plaintext-returning decryption oracle without challenge exclusions. Neither computational suite is a production end-to-end proof, primitive reduction, implementation refinement, QPT theorem, or QROM theorem, and neither models the complete receive cache, rollback, persistence, arbitrary scheduling, or production widths.
...
8. **Complete:** pin rustc, hax, F*, Z3, ProVerif, Rocq, SSProve, MathComp, EasyCrypt, Why3, and CVC5 and run the applicable extraction plus proofs in CI.
...
11. **Complete infrastructure decision:** pin and check an EasyCrypt bounded-capstone suite, inventory its assumptions and hax-to-Lean/F*-to-EasyCrypt contract ledger, and select EasyCrypt as the primary computational-game development environment under a migration-cost-neutral comparison. Retain the independently encoded SSProve suite as a Rocq-kernel-rechecked cross-check.
...
`make verify` now regenerates or checks every configured backend in the revision-pinned proof shell. Its ProVerif result parser rejects timeouts, missing or substituted queries, unexpected true/false classifications, and every unproved or inconclusive security query. The aggregate ProVerif check runs its 17 `check-proverif-<scenario>` targets concurrently, including one passive-classical and one modeled-quantum observational-equivalence check over the five bounded plaintext challenges, and the corresponding `verify-proverif-<scenario>` and `check-generated-proverif-<scenario>` targets support isolated locked-shell and generated-drift checks. `make check-generated` covers the generated F*, ProVerif, and Lean artifacts, while EasyCrypt and SSProve sources are checked without placing generated objects in the inventoried proof tree.
...
The proof artifact must state the exact versions of rustc, hax, F*, Z3, ProVerif, Rocq, SSProve, MathComp, EasyCrypt, Why3, and CVC5 used to produce it.
