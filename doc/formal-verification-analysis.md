...
### VCVio computational-proof feasibility pilot

The imported [`VCVioFeasibility.lean`](../beaconcrypt-core/proofs/lean/BeaconcryptCore/Computational/VCVioFeasibility.lean) module checks that the Aeneas extraction and a pinned Lean game-based framework can coexist in one kernel. Its collision-layer theorems specialize generic bounded adaptive lazy-random-oracle commitment results to the extracted 229-byte transcript field and a 512-bit output, producing the tight `(q(q - 1) + 2) / (2 · 2^512)` ROM bound and the looser `(q + 2)(q + 1) / (2 · 2^512)` collision-resistance-chain bound; its state-handler theorems show exact successful consumption, a call from an assumed tombstone, two sequential calls returning the value once, and hidden-state public output for a simplified CKEY store. These are feasibility results, not complete protocol claims: the actual beaconcrypt CTX-game embedding, production transcript-injectivity/collision-witness bridge, deployed BLAKE2b justification, CTX privacy and ordinary integrity preservation, authentication/provenance, accepted two-slot result-and-final-heap certificate, arbitrary contexts, primitive composition, and extraction-linked end-to-end games remain open. The capability comparison and staged decision are recorded in [`vcvio-feasibility.md`](impl/vcvio-feasibility.md).
