...
The terminal typed KDF query syntax is preserved, but its responses are random-table values instead of the publicly computable fixed HKDF values. This is a distinguishable change even when the root is ignored. The fixed-HKDF term below must not be assumed negligible: the inequality is valid, while its production secrecy interpretation is unsupported. The separate `PqxdhSecretInputKdf` theorem corrects the one-secret-input interface but has not been composed with this challenge-blocked decapsulation context.
