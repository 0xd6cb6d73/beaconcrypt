...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations.
  Lean now mechanizes factor-one probability reductions from the general ideal-model CTX misattribution event and separately named wrong-sequence and wrong-sender record-opening games to BLAKE2b collision-resistance advantage, but the sender game is deliberately below the protocol's earlier pinned-sender check, PPT/runtime preservation and the extracted-production refinement remain unmechanized, and BLAKE2b-512 collision resistance remains assumed.
...
4. Lean machine-checks a factor-one reduction from the corresponding ideal-model misattribution advantage to BLAKE2b-512 collision advantage and preserves that bound for dedicated record-opening games in which an adversary changes the sequence number or sender identifier of an honestly sealed record; the ideal `beaconFinish` path rejects a mismatched sender before CTX, while PPT/runtime preservation, collision resistance itself, and the extracted-to-ideal production bridge are not mechanized.
...
The production commitment transcript helper and fixed-width pointwise collision implication are F*-proved, and Lean proves the factor-one probability reduction on the ideal-model surface for general misattribution and dedicated wrong-sequence and wrong-sender record-opening games; the sender game is a lower record-layer fact because the ideal beacon checks the pinned sender first, while PPT/runtime preservation, the representation bridge, the caller, BLAKE2b, parsing, and field provenance remain computational, adapter, or primitive obligations. The injectivity proof now applies one embedded-segment equality lemma at each of the six transcript offsets instead of repeating the same sequence-extensionality argument for every field; this is a proof-structure refactor and does not alter the transcript layout or injectivity statement.
...
> Lean machine-checks the corresponding factor-one ideal-model probability reduction to BLAKE2b-512 collision advantage and dedicated factor-one wrong-sequence and wrong-sender record-opening specializations; the latter is below the beacon's earlier pinned-sender check, while PPT/runtime preservation and the extracted-production bridge remain unmechanized.
