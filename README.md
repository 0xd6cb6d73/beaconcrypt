...
Before an IDE-managed coordinator spawns agents that may run formal proofs, prepare the shared content-keyed profile once:

```bash
make -C beaconcrypt-core prepare-proof-shell
```

Agents then use normal outer targets such as `make -C beaconcrypt-core verify`; those targets reuse the prepared profile and lazily prepare it if necessary. Agents must not invoke `*-in-shell` targets or `nix develop` directly. If sandboxing prevents access to the default profile root, set `BEACONCRYPT_PROOF_PROFILE_ROOT` to the same absolute writable path outside the checkout for the coordinator and every agent before preparation. Profile reuse is concurrency-safe, but proof targets mutate shared checkout and `target/` paths, so serialize them in one worktree or give agents separate worktrees. See the [core proof workflow](beaconcrypt-core/README.md#strict-haxfproverif-verification) for profile selection, direct wrapper commands, and safe retirement.

### Developer summary of the computational proof

The EasyCrypt and SSProve suites provide machine-checked attack accounting for deliberately small, bounded versions of beaconcrypt. Inside those models, a classical confidentiality or integrity attack has no unexplained protocol shortcut: a successful attacker must query an exact hidden oracle input, produce a collision, or guess a fresh authenticator. The games also reduce recovery of one message after a modeled ratchet erasure to querying the exact erased input, prove bounded binding and privacy properties for the modified CTX commitment, and confirm the expected active-quantum substitution break. Functionally, beaconcrypt does not provide active post-quantum authentication or security in this threat model.

This is evidence for the protocol design, not a security certificate for the shipped library. Production-sized probabilities, real primitive reductions, arbitrary sessions and schedules, the complete receive and persistence state, the pending Rust-to-game bridges, and true quantum programs or superposition hash queries remain outside the computational proof. In particular, the passive-quantum result is a dependency check with classical oracle queries, not a completed post-quantum security theorem. See [what the computational proofs mean in application terms](doc/formal-verification-analysis.md#what-the-computational-proofs-mean-in-application-terms) for the functional guarantees and boundaries scenario by scenario.

The proof suite includes an explicit state-neutral receive scenario. A correctly parsed, correctly sized future frame can trigger bounded private candidate derivation, but commitment or AEAD rejection publishes nothing and returns the exact entry state. The finite server-to-beacon ProVerif trace rejects a forged sequence-3 frame twice from the same symbolic state, accepts the honest sequence-3 ciphertext while publishing only skipped sequence 2, rejects replay, and later accepts the delayed cached ciphertext. A separate exact capacity leg advances from sequence 1 to sequence 52, retains skipped sequences 2 through 51, consumes target sequence 52 without caching it, rejects sequence 54 because sequence 53 would occupy a fifty-first cache slot, consumes cached sequence 51, and then accepts sequence 54 while retaining sequence 53. Lean proves that the extracted control driver refines the ideal `Ratchet.recvStep` for positive wire targets directly at its 50-skipped-key bound; sequence zero has a separate state-neutral non-delivery theorem. The checked F* artifacts describe the predecessor callback-based implementation and remain historical evidence until they are ported to the current effect API. Repeating an invalid boundary-distance frame can still force up to 51 private ratchet-KDF steps per attempt—50 skipped-key derivations plus the target—so deployments need external per-peer availability controls that do not mutate the ratchet on rejection.

This result is deliberately conditional. The private-state run concurrently allows attacker-owned beacon registration and confirms the attacker can read that beacon's routed canary, while the independently rooted legitimate receive canaries remain secret. Compromise immediately after rejection exposes exactly the unchanged pre-attempt live chain and no rejection-created cache material. That chain still derives future skipped and target material in the ideal compromise model, permits attacker forgery, and still leaves a trace in which the later honest target ciphertext is delivered successfully; the result is that rejection did not enlarge state or exposure, not that future secrets survive live-chain compromise. Cap'n Proto parsing, serialized byte lengths, end-to-end linkage of the standalone record root to registration, concrete cryptographic implementations, and arbitrary receive schedules remain outside the finite ProVerif trace. See [the formal-verification analysis](doc/formal-verification-analysis.md#state-neutral-receive-and-compromise) for the exact claim and limitations.
