...
These modules are not an end-to-end security theorem. The response is a one-bit XOR key-correctness harness with no nonce, associated data, AEAD, CTX, identifier binding, serialization, or active registration/replace/drop/replay/deliver/reveal network interface; the accepted root input and Boolean provenance are parameters rather than an intended-peer agreement proof; and the lifecycle snapshots are not reachability results from the executable package. `None` covers only the modeled false-provenance branch, not arbitrary protocol failure leakage. The CKEY interface is a custom consuming `PUT`/`TAKE` store rather than the state-separation paper's repeatable `GET`/`HON` interface, and the one-shot package is distinguishable from the stateless monolithic harness under a second call. Pinned SSProve 0.2.4 makes a separate reporting issue explicit: a capstone retaining generic package/raw sampler syntax makes `Print Assumptions` traverse a generic distribution proof that depends on the repository-rejected infinite-sum interchange axiom. The accepted result therefore consists of the locally checked exact raw-execution certificate, its proof-erased normal-form capstones, and a direct finite-source probability equality; it is not a stock `Pr`, `Pr_op`, package-validity, interchange, or unrestricted contextual-advantage theorem. A repeatable indexed-session relation or assumption-admissible contextual bridge, the production-left-half confidentiality reduction, and the four-corner bound remain open.
