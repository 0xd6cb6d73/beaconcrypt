...
| [`StateSeparatedComposition.v`](../../beaconcrypt-core/proofs/ssprove/StateSeparatedComposition.v) | A typed `CK || CD` package linked to a private consuming CKEY provider; one-shot role/session checks and cleared tombstones; correct `(L,R)/(R,L)` orientation; authenticated first-response use of the left half at sequence one; exact typed KDF uses; a pure single-run equality to the monolithic calculation; and explicit lifecycle scenario facts. | The record is only a one-bit XOR harness; provenance and accepted root input are parameters; lifecycle facts are snapshots rather than package reachability; and there is no package-semantic equality, four-corner bound, adversary-facing transcript, AEAD/CTX/AD composition, production-width representation, or implementation bridge. |
...
The linked SSProve package is executable and its public interface hides CKEY, raw payloads, setup, seal, and open, but the reported equality is deliberately only the pure calculation for one authenticated call. The CKEY-backed package remains tombstoned after that call while the monolithic calculation harness is stateless, so an unrestricted context can distinguish them by invoking the operation twice. Moreover, printing package-level validity, `Parable`, interchange, association, or stock advantage results in the pinned library exposes assumptions outside the repository allowlist, including the rejected infinite-sum interchange dependency. The current acceptance boundary therefore reports only closed pure capstones; a one-call interpreter theorem or repeatable indexed-session relation and an assumption-admissible probability bridge remain prerequisites for the computational hybrid.
...
### Structural lifecycle milestone: implemented single-run seam

`StateSeparatedComposition.v` implements the affine key-transfer and production-orientation nucleus needed by Phase 2. It constructs typed keying and keyed siblings around a private two-slot CKEY provider, checks one role- and session-bound `PUT`/destructive `TAKE` path with cleared tombstones, requires authenticated provenance on seal and open, steps the server's left initial half at sequence one, and proves a pure single-run equality to the monolithic key calculation. Separate closed snapshots record replay consumption before construction, failed construction without server publication, dropped-response asymmetry, accepted counters, and rejection that terminally aborts a pending beacon while leaving prior server state unchanged. This milestone does not complete Phase 1's production-width HKDF interface or Phase 2's adversary-facing experiment and computational hybrid.
...
SSProve is technically suitable for composing PQXDH with beaconcrypt's ratchet because its package algebra can represent the keying-plus-keyed structure, and the new single-run slice demonstrates a private consuming CKEY seam with production-correct role orientation and lifecycle snapshots. This does not by itself satisfy the stock paper theorem's repeatable CKEY or the repository's assumption policy for package-level advantage reasoning. The missing work is not a final triangle inequality: it is an admissible package-semantics bridge, the common production-shaped adversary experiment, a joint exact-label and prefix-consistent HKDF interface, contextual equivalences for the standalone hops, arbitrary bounded ratchet state, AEAD/CTX composition, production-width bounds, and deterministic representation bridges.
