...
## Server first-record proof

`initialize_send_refines` executes the extracted server initialization, proves the new zero counter cannot be exhausted, executes `begin_send` and the actual pending KDF `resume`, and finishes the extracted seal request with `recordSealReply`. The callback preserves the request's actual wire sequence and the complete encoded ciphertext. Its byte-model counterpart is exactly `Ratchet.sendStep`; the full kernel relation records the advanced sender and unchanged complementary empty receiver. `initialize_serverRecord_refines` specializes the same proof to the unchanged PQXDH `serverRecord`, preserving the identifier prefix `LE64(S.n + 1)`, nonabstracted application bytes, sequence one, server sender identifier, associated data, ciphertext, and both directional poststates. This is successful first-record correspondence; failed primitive sealing and candidate publication are covered separately by the server transaction workstream.

Local validation for these milestones used the locked Lean 4.31.0 toolchain and `lake build BeaconcryptCore.Refinement.ProtocolComposition`. The axiom audit of `completeBeacon_refines`, `initialize_receive_refines`, and `initialize_serverRecord_refines` reports only Lean's standard `propext`, `Classical.choice`, and `Quot.sound`. Repository-wide verification, inventory, and cleanup remain coordinated integration gates.

## Existing model property transferred

`completeBeacon_success_admits_record` applies the existing `Pqxdh.HonestRun.beaconRecordAdmitted_elim` theorem to an actually successful complete core registration. For a provisioned honest-run state but an arbitrary incoming frame, acceptance implies the pinned sender, a nonzero sequence, and successful `openRecord` on that exact ciphertext under the derived message material and exact record context. The proof obtains the ideal success from `completeBeacon_refines` and determinism of the actual evaluation, then invokes the existing model theorem; it does not assume ideal acceptance. The honest-run conditions and primitive laws remain explicit. This demonstrates authentication-property transfer beyond preservation of ratchet invariants.

## Server admission root provenance

`server_accept_root_input` starts from an actual successful extracted `server_accept` evaluation and proves that the returned pending registration contains the exact IKM built from that call's four DH contributions and KEM shared secret. It derives rather than assumes successful nonzero validation, and rules out replay/zero-DH branches from the observed accepted result. `accepted_root_refines` composes this fact with `RootKdfLaw`: deriving from that same pending root input gives the ideal root expression over the supplied primitive bytes. Local DH and KEM interpretation equations can then identify those bytes with the unchanged server model's primitive expressions. Server publication must prepare from the same pending value and initialize with `derive pending.root_key_input`; an arbitrary separately supplied root does not establish this provenance.
