...
- The extracted `RefinedRatchet` owns logical control state, both parametric chain values, and fixed slots of sealed `CachedReceiveKey<Material>` values, so normal-return tag association and mutation ordering no longer depend on a parallel production adapter. Its public send and receive operations lend the exact kernel-selected sequence/material pair and caller-supplied frame context to one opaque callback; the callback's `Some` or `None` result drives consumption or retention without a separately supplied authentication Boolean. Concrete HKDF and AEAD semantics, totality of the private fixed-signature primitives, final infallible conversion into external libsodium types, serde and persisted-pair cryptographic provenance, crash/concurrency atomicity, rollback prevention, external copies, and compiler correspondence remain assumptions. The reviewed private primitive implementation unconditionally selects `SYM_RATCHET_INFO`, so label selection is no longer caller-controlled.
...
- Refined constructors establish the combined logical/tagged-material invariant with arbitrary chain types. `refined_seal_next` relates the published counter and next send chain to the opaque step result, invokes the seal callback with that exact step material, allocated sequence, and supplied context, and returns the callback result only after the private token is consumed. Refined receive advancement relates the published receive counter, next receive chain, sequence, and sealed append-slot material to the opaque KDF-step result while preserving the stronger invariant.
...
- The kernel-private `refined_receive_key` returns material only through the current unique logical slot and rejects a populated slot whose sealed tag differs from the requested sequence. `refined_open_and_finish` passes only that selected material, its sequence, and the supplied frame context to the opaque open callback. The new F* lemmas prove that callback `None` returns the complete admitted state unchanged and callback `Some(plaintext)` returns that plaintext only with the state produced by consuming the same selected sequence. The extracted completion checks both target and old-last tags before mutation and returns `Missing` without mutation on a mismatch; successful completion moves the complete old-last tagged record into a non-last target, clears the old last slot, and publishes the matching logical removal in the same returned refined value.
...
The refined Rust send token and arbitrary-Boolean receive completion now remain private implementation details of the extracted kernel. F* proves that `refined_seal_next` calls its opaque callback with the exact allocated material and sequence and that `refined_open_and_finish` calls its opaque callback with the exact selected cached material and sequence, retains the admitted state on `None`, and publishes `Some(plaintext)` only alongside consumption of that sequence. F* still treats both callbacks as arbitrary pure functions and therefore does not prove that production's callbacks implement ChaCha20-Poly1305 and the commitment correctly. Serialization and deserialization, the cryptographic provenance of persisted sequence/material pairs, concrete cryptographic semantics, callback panics or side effects, crash/concurrency atomicity, rollback prevention, external copies, physical erasure, and compiled behavior remain outside F*.
...
| “Cached receive material belongs to its sequence; send keys are one use; replay is rejected.” | Each receive value is stored in a private `CachedReceiveKey` carrying its sequence. F* proves every active tag equals the logical entry in its slot, direct lookup rejects a disagreeing tag, valid transitions preserve the invariant, and successful refined receive removes the complete tagged target before replay lookup fails. The public lifecycle lemmas prove that seal/open callbacks receive the exact kernel-selected material, sequence, and context, that open `None` retains the admitted state, and that open `Some` returns plaintext only with same-sequence consumption. The high-level claim still requires correct concrete cryptographic callbacks, one authoritative state, no retained material copy or role-state fork, persisted-pair cryptographic provenance, and no rollback. |
...
> For the receive cache specifically, the extracted `RefinedRatchet` owns the logical cache and fixed slots of private sequence-tagged material, and production delegates directly to `refined_open_and_finish`. F* proves that extracted `derive_ratchet_step` passes the exact old 32-byte chain to a fixed `32 -> 76` opaque primitive and constructs the exact 32-byte key, 32-byte next-chain, and 12-byte nonce ranges; requires every active cached tag to equal its logical slot sequence; proves that all planned destinations are preflighted before the first abstract KDF call; and proves that the open callback receives the exact selected material, sequence, and context, with `None` retaining the admitted state and `Some` returning plaintext only alongside consumption of the same sequence. Lookup rejects a mismatched tag, completion validates the target and old-last tags before mutation, and the checked results cover full-state retention, whole-tagged-entry swap-removal, and sealed restoration. Production still has to provide the fixed-signature primitive's HKDF-SHA-512 semantics and totality, the seal/open callbacks' concrete commitment and AEAD semantics, correct external-type conversion and compiler correspondence, persisted-pair cryptographic provenance, faithful serialization, and one authoritative non-rollback state; callback or primitive panics, crash/concurrency atomicity, and physical erasure are not proved. The reviewed primitive body unconditionally selects the private domain label.
>
