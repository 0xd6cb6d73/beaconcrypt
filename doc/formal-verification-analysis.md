...
This document explains the formal-verification results in plain language. It audits the checked F*, Lean, ProVerif, EasyCrypt, and SSProve sources under [`beaconcrypt-core/proofs`](../beaconcrypt-core/proofs), distinguishes generated translation coverage from behavioral refinement, and records which concrete F* theorems still describe the predecessor executor/callback source snapshot.
...
- ProVerif proves secrecy and authentication properties for an active-classical symbolic model of registration and a fixed record-exchange schedule, proves the five selected secrets hidden in bounded honest-only passive-classical and modeled-quantum scenarios, proves observational equivalence for replacements of those five challenges in both passive theories, and demonstrates the expected exposure of cached and future keys after one precisely timed beacon-state compromise.
...
- SSProve now checks finite classical-random-oracle games for CTX binding/privacy and one ideal PQXDH establishment followed by one sequence-zero symmetric-ratchet record. The CTX binding game gives a bounded adaptive adversary `q` queries and proves that any successful distinct double opening yields an explicit collision within a trace of at most `q + 2` queries; the separate programming hop bounds the absolute true-real/fresh-ideal decision gap by a query for the secret key-containing transcript. The closed protocol game proves exact zero distinguishing advantage for active classical, passive classical, and a passive-quantum classical-query capability model and proves the expected active-quantum attack has advantage one. Its attacker-facing ROM extension reduces each positive-case advantage to the exact hidden-pad-query probability. Standalone protocol-core modules additionally prove a root-derived masked-bit hybrid bad-query bound, a one-step erasure-conditioned ratchet bad-query bound, same-run combined-authenticator collision extraction, and an exact `1/2` fresh-guess bound for the one-bit ideal table. No numerical production-width bound or end-to-end composition is proved.
- EasyCrypt is the primary environment for new computational-game development after a clean-slate comparison that assigns zero weight to migration cost and existing code. Its native probabilistic modules and relational logic fit the stateful reductions, and the compiler-green suite now has reviewed coverage counterparts for 30 selected bounded SSProve capstone areas. The manifest gate checks declaration existence and reviewed metadata, not cross-assistant proposition equivalence. SSProve remains an independently encoded Rocq-kernel-rechecked cross-check. Neither suite is a production end-to-end proof or a direct hax extraction target.
- The locked mainline EasyCrypt toolchain is classical. EasyPQC's surviving official upgrade branch is retrievable but does not build reproducibly in the locked environment and contains explicit QROM axioms and incomplete example proofs. No positive passive-QPT or QROM theorem is therefore part of the accepted proof suite.
...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations. The EasyCrypt and SSProve games use finite Boolean atoms and ideal primitives; production-width bad-query bounds, arbitrary protocol schedules, multi-session composition, the cross-prover representation bridge, concrete primitive reductions, and BLAKE2b-512 collision resistance remain open or assumed. The passive-quantum theorem is explicitly a classical-query capability result rather than a QPT or QROM theorem, and the current classical authentication makes the active-quantum game perfectly distinguishable as expected.

In one sentence: important protocol-control logic and an idealized network model have been verified, but “beaconcrypt as a whole is formally proven secure” would be an overstatement.

## What the computational proofs mean in application terms

The EasyCrypt and SSProve proofs check the structure of beaconcrypt's security arguments in deliberately small, bounded models. They show what an attacker would have to accomplish to break a modeled property; they do not yet prove the shipped library end to end. Applying these results requires production-shaped reductions that introduce explicit primitive-security assumptions and numerical bounds, cover the complete relevant state and adapter behavior, complete the pending Rust-to-game representation bridges, and state the deployment conditions that preserve the modeled behavior.

The most useful way to read these results is as machine-checked attack accounting. Inside the positive bounded classical, CTX, ratchet, and record games, an attacker has no unexplained way to win: a modeled confidentiality or integrity win is impossible or is reduced to an exact hidden-input query, a collision, or a fresh guess. The active-quantum negative control is deliberately different: after receiving the named classical-secret recovery capabilities, it wins. A bound saying that the attacker's advantage is at most the probability of querying a hidden input is not yet a numerical production estimate. It establishes that the modeled protocol gives the attacker no shortcut around finding that exact secret input; later production-width reductions must show that doing so is negligibly likely.

Here, **distinguishing advantage** measures the attacker's ability to tell which of two challenge values was protected: zero means its observations have the same distribution, while one means the modeled attacker tells them apart perfectly. A **random-oracle game** replaces a hash or derivation step with a consistent ideal hash-like lookup table; querying the hidden input means supplying the exact secret tuple to that table. A **collision** is two different inputs producing the same digest, and a **fresh guess** is a correct authenticator value for an input the attacker did not previously query. **QPT/QROM security** would cover genuine quantum computations and superposition queries to that hash-like oracle; the present computational games do neither.

| Developer question | What is machine-checked | Practical interpretation and boundary |
| --- | --- | --- |
| Can a classical network attacker learn the protected value by observing, forwarding, or replacing protocol data? | SSProve's closed model represents one ideal establishment and record with a multi-coin tape; the EasyCrypt coverage counterpart collapses the protected view to a challenge bit masked by one uniform bit. Both obtain a zero gap for their arbitrary distinguishers, and EasyCrypt's unsupported classical replacement produces a fixed failure view. In the adaptive classical random-oracle games, distinguishing advantage is bounded by each model's exact modeled hidden-pad query event. | These bounded result shapes expose no additional route to the selected value. The fixed failure is part of EasyCrypt's abstract replacement definition, not a proof that every production substitution is rejected. The games omit the actual registration transcript and are not cross-assistant proposition equivalence, a production-width probability bound, or a proof for every session, record schedule, receive path, or decryption-oracle interaction. |
| Does recorded traffic remain confidential if a quantum computer later breaks the modeled classical public-key components? | An abstract five-coordinate hybrid enumerates DH1, DH2, DH3, DH4, and ML-KEM. The passive-quantum-capability game designates ML-KEM as the remaining hidden coordinate and proves the challenge-gap-to-hidden-query result while giving the adversary only a bounded classical oracle interface. | This checks the intended dependency shape, not ML-KEM security or the production combiner and HKDF ordering. It is not a theorem about quantum programs or superposition oracle queries and must not be advertised as completed harvest-now-decrypt-later, QPT, or QROM security. |
| Is beaconcrypt secure against an active quantum attacker? | No. Under the named contracts that recover the classical Ed25519 and X25519 secrets and the explicit game-world validation rule, the stateful substitution model accepts an attacker-selected bundle and installs an attacker-known session secret. The separate closed recovery game has distinguishing advantage one. | The design is expected to fail against a quantum-capable man-in-the-middle because ML-KEM does not provide post-quantum authentication by itself. These are conditional model-level negative controls, not an implementation exploit or a proof of a quantum algorithm. |
| Does exposing the next modeled ratchet chain reveal the preceding message? | For one modeled hop after the predecessor chain and message-key coordinate have been erased, the old-message views have the same distribution. With adaptive classical oracle queries, any distinguishing advantage is bounded by the probability of querying the exact erased predecessor-chain input. | Learning the modeled next chain does not create another route to that one earlier message inside the game. The hop samples ideal Boolean outputs and does not prove the production KDF reduction, security after exposing the full production state or cache, physical memory erasure, arbitrary ratchet histories or compromise times, post-compromise recovery, snapshots, rollback, or the complete production receive/cache lifecycle. |
| What does the modified CTX commitment bind? | Two distinct accepted explanations of the same fixed protected payload imply a collision between distinct commitment inputs. The committed input contains the key, nonce, associated data, transmitted tag, sequence number, and sender identifier. | Assuming collision resistance and the pending injective encoding bridge, the same ciphertext, tag, and commitment cannot be validly reinterpreted with different committed fields or plaintext without yielding a hash collision. CTX is not encryption, does not include the ciphertext or plaintext in its own transcript, and does not replace AEAD authentication. |
| Does publishing the CTX commitment leak its secret-key-containing input? | In a tiny 64-point adaptive classical random-oracle game where each of the six transcript fields is one bit, the real CTX digest and a fresh ideal digest are indistinguishable except when the accepted query trace contains the exact hidden transcript. The hidden one-bit key is sampled inside the game and is not given to the adversary; the digest is also one bit. | The theorem checks the game hop, not that its right-hand probability is small: two accepted candidate-key queries can cover both possible keys in this tiny model. Production-width privacy, a numerical query bound, BLAKE2b behavior, complete AEAD-plus-CTX composition, and the Rust-to-game bridge remain separate obligations. |
| What is proved about accepting a modified or context-reused record? | Every accepted modified candidate in the bounded integrity model is classified as reuse of a previously queried authenticator input or a fresh accepted guess. Reusing the same payload across a different context or sequence yields a collision event. The deliberately one-bit fresh-tag game has exact acceptance probability one half. | This is exhaustive bookkeeping inside the model; EasyCrypt receives the trace from its adversary rather than generating it through a checked authentication oracle. Security still requires a production-shaped oracle game that separately bounds queried-input reuse and fresh guessing. The one-half result is not beaconcrypt's real forgery probability, and this game does not model the full production replay/cache state. |

The security conclusions are not assumed as axioms. EasyCrypt's seven inventoried assumptions are limited to termination of four abstract adversaries, the pending injective CTX-encoding bridge, and the two classical-secret recovery capabilities used only by the active-quantum negative control. Ideal primitive behavior and simplified failure behavior are still built into the game definitions. Cryptographic primitives remain idealized, and the EasyCrypt games are handwritten rather than extracted from Rust.

A defensible summary is: **machine-checked bounded models reduce their classical confidentiality, one-hop erased-key, and CTX privacy gaps to explicit hidden-query events; reduce distinct CTX openings and cross-context or cross-sequence record reuse to collisions; classify record attempts into previously queried-input or fresh-guess cases; and confirm the expected active-quantum authentication break.** This is supporting evidence for beaconcrypt's design, not an end-to-end certificate for the production library.
...
| EasyCrypt theorem | A classical probabilistic program, relational game hop, Hoare judgment, or reduction follows from the named modules and assumptions. | Primitive correctness, direct hax correspondence, or quantum-adversary soundness unless a separately qualified quantum extension establishes it. |
...
- **EasyCrypt** is a proof system for game-based cryptographic reductions over probabilistic programs with adversarial modules. The repository uses mainline EasyCrypt only for classical reasoning until a quantum-sound extension is separately pinned and audited.
...
The maintained 17-scenario suite now gives the threat modalities explicit targets. `active-classical` runs the full replicated Dolev–Yao baseline and depends on the seven-query reachability control; `passive-classical` and `passive-quantum` use one bounded honest-only session and check the five named secrets, with a shared active-scheduler control proving registration commit and all five exact record deliveries reachable in that same process. A separate five-choice biprocess proves observational equivalence in the passive classical theory and again in the modeled-quantum theory. The passive-quantum theory publicly recovers Ed25519 and X25519 secrets while leaving ML-KEM opaque, and paired ML-KEM controls require opacity with the honest KEM secret hidden and secrecy failure after the KEM secret is revealed. `active-quantum` is a bounded existential man-in-the-middle witness: it forges an honest-identity replacement bundle, substitutes attacker-owned DH and ML-KEM keys, derives the hax-modeled root input and associated data, recovers `INITIAL_SECRET`, and violates exact initiation agreement. These quantum rules are symbolic dependency controls, not QPT, QROM, or algorithmic quantum models.
...
The [computational lifting](ctx-commitment.md) now has independently checked EasyCrypt and SSProve bounded hidden-oracle verifiers that extract a collision from every successful double explanation. The production-width collision game, numerical primitive bound, representation bridge, and runtime theorem remain separate obligations.
...
The closed protocol capstones use a direct finite pushforward rather than SSProve's high-level general package advantage bridge because the pinned bridge pulls in an unaccepted infinite-sum interchange dependency. The executable package remains useful as the bounded operational presentation, but the checked equality theorem is presently stated for the direct deterministic pushforward. All attacker-facing extensions are sound reduction shapes rather than negligible production claims: their one-bit domains permit exhaustive queries or non-negligible guesses, and no production-width bad-event bounds are supplied. The original closed models fix one session, one record, sequence zero, public metadata, and a statically selected action; the new hybrid, one-step erasure, and integrity files remain standalone hops rather than an arbitrary-execution composition theorem.

### EasyCrypt bounded computational capstones

EasyCrypt is now the primary environment for new computational-game development under the stipulated zero migration cost, while the independently encoded SSProve results remain a Rocq-kernel-rechecked cross-check. This is a tooling recommendation, not a claim that either handwritten game is automatically connected to Rust: F* and Lean retain responsibility for implementation correctness, and hax-derived definitions and named refinement theorems remain the contract boundary because direct hax-to-EasyCrypt extraction is not usable.

The closed [`PqxdhGames.ec`](../beaconcrypt-core/proofs/easycrypt/PqxdhGames.ec) games prove exact zero advantage for active-classical forwarding or replacement, passive-classical forwarding, and the passive-quantum capability label restricted to a classical interface. The explicit active-quantum replacement/recovery distinguisher has advantage one. [`AdaptiveRom.ec`](../beaconcrypt-core/proofs/easycrypt/AdaptiveRom.ec) upgrades the positive cases to arbitrary lossless, fuel-bounded adaptive classical-ROM adversaries and bounds distinguishing advantage by the exact hidden-pad-query probability. [`HybridAdaptiveRom.ec`](../beaconcrypt-core/proofs/easycrypt/HybridAdaptiveRom.ec) proves the corresponding bad-query hop for the full ordered five-coordinate DH1/DH2/DH3/DH4/ML-KEM input. The passive-quantum labels in these ordinary EasyCrypt games describe recovery capabilities with classical queries; they are not QPT or QROM theorems.

[`RatchetAdaptiveRom.ec`](../beaconcrypt-core/proofs/easycrypt/RatchetAdaptiveRom.ec) proves the one-hop erasure-conditioned ratchet advantage bound, exact trace/count/fuel accounting, and the hidden predecessor-query classification. [`CtxRom.ec`](../beaconcrypt-core/proofs/easycrypt/CtxRom.ec) proves bounded randomized CTX binding extraction and verifier trace growth. [`CtxAdaptiveFullRom.ec`](../beaconcrypt-core/proofs/easycrypt/CtxAdaptiveFullRom.ec) uses the complete six-Boolean transcript domain, preserves arbitrary off-secret table answers, proves the uniform true-real/programmed-real representation by finite coin reindexing, and bounds the absolute true-real/fresh-ideal decision gap by the exact secret-transcript-query probability for an arbitrary lossless fuel-bounded adversary. [`RecordIntegrity.ec`](../beaconcrypt-core/proofs/easycrypt/RecordIntegrity.ec) gives distribution-level query-or-fresh-guess classification, cross-context and cross-sequence collision bounds, exact trace accounting, and exact `1/2` fresh one-bit tag acceptance for an arbitrary lossless randomized adversary.

The explicit assumptions are narrow and audited: four adaptive modules require their adversary's `run` procedure to be lossless when the supplied oracle is lossless; the exact one-bit record theorem requires the fresh-tag adversary to be lossless; `Ctx.ec` retains the pending abstract-to-production encoder-injectivity representation contract; and `ActiveQuantum.ec` retains the two explicit Ed25519/X25519 recovery equations. Finite programming swaps and game equivalences are proved rather than assumed. The stateful skeleton's active `deliver` operation is a plaintext-returning decryption oracle, its passive interface accepts adversarially chosen inputs, and corruption returns a non-evolving establishment secret. None of these results proves a production end-to-end execution, concrete primitive security or correctness, implementation refinement, full receive/cache/rollback behavior, multiple-session composition, QPT security, or QROM security.
...
The modified CTX justification has four deliberately separated layers. F* machine-checks `ctx_distinct_openings_imply_hash_collision`, which constructs a pointwise collision witness from two distinct accepted explanations of one fixed production-format `C || T || U` payload for arbitrary pure hash and AEAD-open functions. EasyCrypt and SSProve independently run bounded adaptive hidden-ROM verifiers and extract a valid collision from every successful abstract double explanation, with exact verifier-query trace accounting. Separate full six-Boolean programming games bound the absolute true-real/fresh-ideal CTX decision gap by a secret-transcript query, conditional on a hidden uniform key. ProVerif supplies an explicit ideal-hash negative control in which the base AEAD deliberately multi-opens and removing CTX reverses the query result. None of these layers proves BLAKE2b, the libsodium call, production field provenance, the cross-prover representation and production-width bridge, a numerical hidden-query or collision bound, full transformed-AEAD composition, hax/compiler correctness, or compiled machine code.
...
| “The modified CTX construction provides strong commitment.” | F* proves the production-format pointwise collision witness for arbitrary pure hash and AEAD-open functions, including unequal-key and unequal-context base-AEAD openings. EasyCrypt and SSProve independently supply bounded hidden-ROM adversaries, verifier-query accounting, extracted collision events, and full six-Boolean secret-query privacy hops over finite semantic atoms. ProVerif confirms the intended ideal-hash CTX/no-CTX differential. The production-width representation bridge, numerical ROM and BLAKE2b bounds, libsodium behavior, adapter provenance, compiler correspondence, ordinary-integrity preservation, and complete privacy composition remain assumptions or separate obligations. |
...
- a complete behavioral specification for every extracted function and error
  ...
  theorem;
- BLAKE2b-512 collision resistance, a concrete numerical bound for that primitive, a production-width CTX/hash-oracle reduction, or a mechanized runtime theorem; F* machine-checks the pointwise production transcript witness and injectivity, SSProve checks the finite hidden-ROM extractor and secret-query programming hop, and the representation bridge, opaque hash implementation, numerical bad-event bounds, production field provenance, and compiled caller retain their stated assumptions;
...
The result also trusts the correctness of rustc for the extracted core, hax and its Rust/F* models, the F* checker, Z3, ProVerif, Rocq, SSProve, MathComp, EasyCrypt, Why3, CVC5, their libraries, the build scripts, and the reviewed extraction selection. PQXDH's exact byte-range proofs specifically rely on three specification-only range-update contracts supplied by the pinned hax proof library; their stated prefix/range/suffix behavior is trusted rather than proved in this repository. The little-endian key-ID proof also relies on a specification-only right-shift lemma equating bounded shifts with division by the corresponding power of two ([inventory](../beaconcrypt-core/proofs/trusted-boundary.md#proof-library-and-tool-assumptions)). The handwritten AWK result classifier is likewise trusted to recognize every required ProVerif query and classify its output correctly. The repository prevents local proof shortcuts by rejecting `assume` and `admit` in repository-owned F* files and rejecting checker flags that would permit missing proofs ([policy gate](../beaconcrypt-core/Makefile#L135-L147)). This does not remove assumptions or trusted interfaces from external hax/F* libraries. The SSProve suite separately rejects repository admissions and the unsafe hax SSProve prelude, exact-diffs each module's reviewed foundational-assumption allowlist, confines objects to `target/formal-verification/ssprove`, and runs `coqchk`. Its accepted assumptions are propositional extensionality, dependent functional extensionality, constructive indefinite description, and an abstract `realType`; this audit discloses rather than proves those foundations. The EasyCrypt suite rejects proof-bypass commands, exact-diffs complete normalized axiom declarations, requires every axiom name to use the `assumption_*` prefix, and keeps its implementation bridges marked pending. EasyCrypt's proof engine and CVC5 answers are trusted and do not produce a Rocq-kernel-checkable proof artifact; the independently encoded SSProve capstones provide a separate kernel-rechecked assurance boundary.
...
4. SSProve gives a bounded adaptive hidden-ROM game, performs the two verifier queries, extracts a collision from any successful distinct explanation pair, and proves the `q + 2` trace bound. The F*/SSProve production representation bridge, numerical collision bound, and concrete collision-resistance assumption remain unmechanized.
...
The production commitment transcript helper and pointwise collision implication are F*-proved, and SSProve checks the finite hidden-ROM collision extractor, trace accounting, and separate secret-query privacy hop. The production-width F*/SSProve representation bridge, numerical bad-event bounds, caller, BLAKE2b, parsing, field provenance, and complete AEAD composition remain computational, adapter, or primitive obligations. The injectivity proof now applies one embedded-segment equality lemma at each of the six transcript offsets instead of repeating the same sequence-extensionality argument for every field; this is a proof-structure refactor and does not alter the transcript layout or injectivity statement.
...
It enters the locked Nix environment, checks exact tool identities, regenerates F*, ProVerif, and no-exclusion Lean extraction, enforces the local proof-bypass policies, strictly checks generated and handwritten F*, builds the imported Lean project, runs all ProVerif scenarios through separate targets, compiles and kernel-checks every SSProve game module against its exact assumption allowlist, and policy-checks and compiles every EasyCrypt bounded-capstone source. Each ProVerif scenario can also be checked directly with `make -C beaconcrypt-core check-proverif-<scenario>`, `make -C beaconcrypt-core verify-easycrypt` checks the primary computational development suite, `make -C beaconcrypt-core verify-ssprove` checks the independent Rocq-kernel cross-check, and `make -C beaconcrypt-core verify-lean` regenerates and builds Lean in the pinned environment. After intentional boundary diffs have been reviewed and their hashes refreshed, the separate `make -C beaconcrypt-core check-inventory` command checks the trust-boundary inventory. The historical F*/ProVerif tool bundle is recorded in the [Stage 8 document](impl/formal-verification-stage-8.md#locked-proof-bundle), while the current prover decision and limitations are recorded in the [computational-security integration evaluation](impl/computational-security-proof-integration.md).
...
- the shared weak-AEAD multi-opening query to be true (unreachable) with CTX and false (witnessed) without CTX, while all six symbolic CTX-constructor field-dependency tripwires are true in both scenarios;
...
- all five secrecy queries and the five-challenge observational-equivalence result to be true for passive classical and modeled quantum, with registration and every exact canary delivery witnessed under the shared active scheduler, both classical quantum-recovery canaries exposed, the ML-KEM control secret opaque without its recovery rule, and that same secret exposed when the recovery rule is loaded;
...
The generated check runs the extraction-backed suite and rejects tracked or untracked extraction drift; the SSProve target compiles every finite game module in the locked Rocq/SSProve shell, exact-checks each assumption report, and runs `coqchk`; the EasyCrypt target exact-checks its axiom and bridge policy before compiling every bounded-capstone source with the pinned Why3/CVC5 path; and the standalone inventory command checks monitored trust-boundary membership and fingerprints. The dedicated formal-verification workflow distributes the equivalent F*, SSProve, EasyCrypt, per-scenario ProVerif, and inventory checks across independent matrix jobs, then reports the established aggregate status only when every component passes. Together, those commands make the proof reproducible and prevent a query, monitored trust-boundary file, or generated file from silently disappearing or changing without an explicit baseline update. They do not prove that the handwritten model is faithful to production or that the reviewed assumptions hold; those conclusions still require substantive review beyond the mechanical Stage 9 gate.
...
> Complementary SSProve games check one ideal PQXDH establishment and one sequence-zero symmetric-ratchet record. The closed finite-tape game preserves the four-DH-plus-KEM root order, correlated Ed25519/X25519 compromise, and shared initial/step KDF prefix behavior; every deterministic Boolean distinguisher has advantage zero in the active-classical, passive-classical, and passive-quantum classical-query capability cases, while active quantum substitution has advantage one. A bounded adaptive classical-ROM extension fixes the DH atoms as guessable, hides only the honest ML-KEM atom and table, and reduces positive-case distinguishing advantage to querying the exact hidden pad input. No numerical production bound is proved, and the result fixes public metadata and does not cover general transcripts, sessions, record schedules, CTX composition, QPT adversaries, or the QROM.
>
> Independently encoded EasyCrypt and SSProve protocol-core hops additionally bound a root-derived masked-bit challenge by querying a modeled five-coordinate input containing one hidden contribution, bound one-step post-erasure ratchet confidentiality by querying the erased predecessor chain, extract ideal-table collisions from cross-context or cross-sequence payload reuse, and bound fresh one-bit combined-authenticator acceptance. EasyCrypt proves exact probability `1/2` for an arbitrary lossless randomized guesser. These are finite-table theorems, not production HKDF/tag-width bounds or an AEAD+CTX composition.
...
> EasyCrypt and SSProve independently supply finite hidden-ROM CTX adversaries and verifiers, extract collisions from accepted distinct pairs with exact trace accounting, and separately bound the absolute true-real/fresh-ideal privacy decision gap by a secret-transcript query over the full six-Boolean domain. The production-width representation bridges, numerical ROM and BLAKE2b-512 bounds, ordinary-integrity preservation, and complete AEAD privacy composition remain open or assumed.
