...
The Rust implementation keeps the existing flat `beaconcrypt_core::ratchet` API through scoped re-exports while placing ratchet definitions in four modules:
...
- [`src/ratchet.rs`](src/ratchet.rs) retains fixed-width KDF request, output, chain, key, nonce, and material values plus the exact output splitter and facade re-exports; hax generates `Beaconcrypt_core.Ratchet`.
- [`src/ratchet/concrete.rs`](src/ratchet/concrete.rs) owns executor-bearing chains, the concrete kernel, concrete seal/open operations, and concrete restoration; hax generates `Beaconcrypt_core.Ratchet.Concrete`.

PQXDH is split in the same way: [`src/pqxdh.rs`](src/pqxdh.rs) owns first-order protocol types and transitions and generates `Beaconcrypt_core.Pqxdh`, while [`src/pqxdh/concrete.rs`](src/pqxdh/concrete.rs) owns the initial executors and concrete role/candidate kernel constructors and generates `Beaconcrypt_core.Pqxdh.Concrete`.

The generated dependency graph is not a single linear chain: Refined consumes Control; Ratchet.Concrete consumes root Ratchet values and Refined operations; Pqxdh consumes root Ratchet values; and Pqxdh.Concrete consumes Pqxdh, root Ratchet, and Ratchet.Concrete. The Control and Refined `.fsti` files expose only declarations needed by their generated consumers, [`proofs/fstar/Beaconcrypt_core.Ratchet.fsti`](proofs/fstar/Beaconcrypt_core.Ratchet.fsti) exposes six root value declarations, and [`proofs/fstar/Beaconcrypt_core.Ratchet.Concrete.fsti`](proofs/fstar/Beaconcrypt_core.Ratchet.Concrete.fsti) exposes the concrete kernel type and constructor. The empty [`proofs/fstar/Beaconcrypt_core.Pqxdh.Concrete.fsti`](proofs/fstar/Beaconcrypt_core.Pqxdh.Concrete.fsti) keeps that implementation abstract to ordinary importers. The Ratchet and PQXDH lemma interfaces likewise export no declarations; their named implementations use explicit F* friend access where representation-dependent proofs need hidden definitions. Hax therefore produces seven reviewed F* implementation modules: Commitment, the four Ratchet modules, and the two PQXDH modules.
