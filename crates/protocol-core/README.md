...
The concrete cryptographic boundary is represented by two domain-specific opaque primitives: `fn(&[u8; 32]) -> [u8; 76]` for a later ratchet step and `fn(&[u8; 32]) -> [u8; 64]` for initial directional chains. Their production implementation privately fixes `SYM_RATCHET_INFO`; no production caller supplies a label, output length, or slice. Extracted `derive_ratchet_step` passes the exact old chain to the first primitive, partitions the result, and constructs owned fixed-width `RatchetChain`, `RatchetKey`, `RatchetNonce`, and `RatchetMaterial` values. Extracted `derive_initial_ratchet_chains` likewise passes the exact 32-byte root to the second primitive, splits both 32-byte halves, and selects complementary role-ordered send and receive chains. Production converts those fixed arrays into its role and libsodium types through infallible `From<[u8; N]>` implementations. The generic refined callback is consequently `fn(&Chain) -> RatchetStep<Chain, Material>` and has no caller-selected context. Production delegates receive-until directly to `refined_advance_receive_until`, which decides admission and preflights every planned fixed-array destination before its first callback. A valid admitted future plan then executes the exact bounded sequence of counters, append slots, tagged cache entries, and callback-produced materials in order and reaches the requested receive counter without an intermediate reported failure that could publish only a prefix; an old or current target performs zero steps and leaves availability to the subsequent lookup. Every reported rejection is state-neutral and invokes no callback. The kernel also resolves material by sequence and performs retention or swap-removal of the complete tagged entry internally after authentication. On send, it returns a non-clonable `RefinedSendKey<Material>` that keeps the allocated sequence paired with its material until consuming finish. The older logical APIs remain available for proof compatibility.

The refined restoration builder accepts each `(sequence, material)` pair in one checked operation, seals that sequence into the cached value, and reconstructs control and cached slots together. The persistence adapter retains the five-field format containing `send_key`, `recv_key`, `send_ctr`, `recv_past`, and `recv_ctr`; serialization reads active sequence/material pairs through the tag-checking kernel accessor, and import supplies sorted pairs to the refined builder. Imports with more than 50 outstanding receive keys are rejected, as are legacy six-field objects containing `send_past`; this is an intentional compatibility break. The unconditional builder lemmas prove only structural sequence/material association. The separate proof-only `reachable_restore` relation starts only when authenticated snapshot provenance supplies live chains equal to the canonical derivations at the imported counters, preserves that relation on each append only when the supplied material is the canonical receive derivation at its sequence, and makes `finish_refined_restore` publish a `reachable` state. The existing serde format does not authenticate this provenance, so ordinary deserialization remains outside the derivational-reachability claim even when all structural checks pass. See Step 3 of the [formal verification plan](../../doc/formal-verification.md) and the [Stage 3 implementation record](../../doc/formal-verification-stage-3.md).

The F* refinement lemmas are parametric in the chain and material types and in pure step callbacks. The adjacent-state lemmas may quantify over an arbitrary callback, but the proof-only `reachable(initial_send, initial_receive, send_step, receive_step, state)` relation fixes both initial directional chains and both abstract steps for the complete lifetime represented by one state. Its `chain_after` and `material_at` definitions require the live send and receive chains to be exactly the numbers of fixed-step iterations named by their counters and every populated cached entry to contain the receive-step material produced at its sealed sequence; it also includes the prior logical/tag validity invariant. `refined_new_is_reachable` establishes this relation from the supplied directional chains, while `refined_from_counters_is_reachable` is conditional on both supplied live chains already being the canonical derivations at the arbitrary counters. Preservation covers send advancement and public `refined_seal_next` on either callback result, one-step and bounded receive advancement, receive-until admission and rejection, failed-open retention, authenticated consumption and whole-entry swap-removal, and public `refined_open_and_finish`; reachable lookup can expose only `material_at` for the requested sequence. The exact primitive-input and fixed-width partition lemmas additionally prove that the concrete ratchet adapter invokes its opaque primitive on the exact old 32-byte chain, constructs the exact 76-byte key/next-chain/nonce partition, and that the initial adapter invokes its opaque primitive on the exact root and selects complementary fixed 32-byte role directions from the 64-byte result. The extracted completion code validates the target and old-last tags before mutation and returns `Missing` without mutation on a disagreement. These lemmas give derivational provenance relative to the fixed abstract steps, but deliberately do not prove cryptographic collision properties of their outputs, primitive panic behavior, or crash atomicity.

The correspondence claim covers high-level encryption and decryption traces that begin in a reachable initialization and continue under the same fixed production steps without state rollback. The remaining KDF assumptions include the cryptographic correctness and totality of the two private fixed-signature HKDF-SHA-512 primitives. Their reviewed implementation unconditionally selects the private `SYM_RATCHET_INFO` constant, so label selection is no longer an adapter argument or proof assumption. Input handoff, output lengths, per-step offsets, initial direction offsets, fixed-width construction, and derivation at the logical counters are proof conclusions; production's final infallible conversion into external libsodium types remains subject to ordinary Rust/compiler correspondence. A no-reuse conclusion for distinct logical record positions is conditional on noncollision of the relevant concrete key, nonce, or key/nonce projections of the fixed KDF outputs across those directional derivation positions and on one authoritative owner with no state fork, rollback, or restored stale snapshot. This condition does not forbid retrying the same retained receive material for the same sequence after failed authentication, and F* does not prove HKDF-output noncollision. Primitive panic behavior, authentication-result provenance, unauthenticated serde translation and snapshot provenance, crash atomicity, physical erasure and zeroization, direct low-level compatibility calls, pre-send state forks, and production peer-map selection remain explicit assumptions or exclusions. `Clone` is available only when all three refined types are cloneable, so preventing state forks and rollback still relies on the production ownership discipline.
...
The checked ratchet properties cover the exact 76-byte key/next-chain/nonce partition, send and receive counter monotonicity and exhaustion, receive-gap and cache bounds, equality of every active cached sequence tag with its logical slot, mismatched-tag lookup rejection and neutral missing outcomes, whole-plan destination preflight, neutral callback-free rejection, exact bounded receive sequence/slot/material execution to the target counter for admitted future plans, preservation of existing cache associations, zero-step old lookup, callback ordering, lookup soundness and completeness, retry retention, internal swap-removal of complete tagged entries, exact key consumption and replay rejection, consuming refined send tokens, compatibility with the older logical transitions, and non-selected peer isolation. The stronger `reachable` relation fixes the initial directional chains and abstract steps, identifies both live chains and all cached materials with their canonical counter-indexed derivations, is unconditional for fresh initialization but conditional for arbitrary-counter construction, and is preserved by public sealing and opening plus their internal send, advancement, retention, and consumption transitions. Structural restoration remains unconditional, while `reachable_restore` and its append/finish theorems require authenticated canonical chain/material premises that ordinary serde does not establish. The extracted runtime also validates target and old-last tags before completion mutation. The PQXDH properties cover exact type/role encodings and validation, the semantic registration ID, root and associated-data transcripts, conditional honest-role agreement, complementary ratchet initialization, authenticated key-ID correspondence, replay-status handling, and checked server transactions.
