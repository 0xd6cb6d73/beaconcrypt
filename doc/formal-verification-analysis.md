...
| “The modified CTX construction provides strong commitment.” | F* proves the production-format pointwise collision witness for arbitrary pure hash and AEAD-open functions, including unequal-key and unequal-context base-AEAD openings. Lean gives the adversary full control of two keys, nonces, associated-data contexts, sequence numbers, sender identifiers, and plaintexts, tests equality of the actual ideal `sealRecord` outputs, proves factor-one reductions for that direct full-commitment event and the stronger raw-payload game, and refines the current generated six-field builder to the ideal preimage. Its scheme-specific direct-game branch now reduces fresh retained-base acceptance with factor one to the defined modified nonce-AEAD INT-CTXT advantage, preserves every source `qE` sealing-query bound, and bounds the secret-prefix event by a modified key-probe IND$ advantage plus `qH / 2^256`. SSProve supplies a bounded hidden-ROM adversary, two-query verifier, extracted collision event, `q + 2` trace accounting, and a separate secret-query privacy hop over finite semantic atoms. ProVerif confirms the intended ideal-hash CTX/no-CTX differential. Production-width and cross-prover representation bridges, numerical ROM and BLAKE2b bounds, the key-probe-to-conventional-IND$ bridge, total byte-level uniform-query/PPT preservation, libsodium behavior, adapter hashing and field provenance, Hax/Rust semantics, compiler correspondence, ordinary-integrity preservation, and complete privacy composition remain assumptions or separate obligations. |
