...
The protocol-transition seam is now explicit in `Acceptance.lean`. `BeaconRecordAdmitted` classifies exactly successful beacon processing plus `badPlaintext` and `keyIdMismatch`, the two failures reachable only after record opening; it excludes identity, KEM, DH, sender, zero-sequence, and `badRecord` failures. `HonestRun.beaconRecordAdmitted_elim` proves that every classified outcome passed the pinned-sender and nonzero-sequence checks and contains an actual successful `openRecord` call under the exact receive-ratchet material and frame context. This deterministic eliminator is the checked premise for the next transition-level probability reductions; it is not itself a collision-resistance bound.

The native theorem closure is admission-free. `#print axioms` reports only `propext` and `Quot.sound` for collision extraction, the three pointwise field specializations, the field-game pointwise implications, and the beacon admission eliminator, and additionally `Classical.choice` for the five probability inequalities; no audited theorem reports `sorryAx`.
...
2. Lift the completed wrong-sequence, wrong-sender, and cross-session record-opening games through the completed `BeaconRecordAdmitted` eliminator: prove factor-one bounds for reordered and foreign-session `beaconFinish` records and a separate zero-advantage result for the earlier pinned-sender precheck.
