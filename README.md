...
The proof suite includes an explicit state-neutral receive scenario. A correctly parsed, correctly sized future frame can trigger bounded private candidate derivation, but commitment or AEAD rejection publishes nothing and returns the exact entry state. The finite server-to-beacon ProVerif trace rejects a forged sequence-3 frame twice from the same symbolic state, accepts the honest sequence-3 ciphertext while publishing only skipped sequence 2, rejects replay, and later accepts the delayed cached ciphertext. A separate exact capacity leg accepts sequence 51 from counter 1 with 49 skipped keys, rejects a distance-two future frame because `49 + 2 > 50`, consumes one cached key, and then accepts that future distance. F* separately proves general full-state rejection equality and reachability, non-vacuous exact preparation for every valid admitted future plan and valid cached target, exact success publication when the callback accepts, replay neutrality, equality of an arbitrary retry before and after any finite repetition of one fixed rejected operation, rejection capacity preservation, and the 49-entry maximum-gap result when the canonical target callback succeeds. Repeating an invalid boundary-distance frame can still force up to 50 private KDF steps per attempt, so deployments need external per-peer availability controls that do not mutate the ratchet on rejection.
