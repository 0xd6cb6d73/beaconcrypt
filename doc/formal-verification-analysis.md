...
| “Peer, counter, and ratchet publication commit atomically.” | F* proves the refined ratchet's normal-return transition over its counter, live chain, and fixed material slots, and separately proves the pure peer candidate's returned shape. Crash/concurrency atomicity, the surrounding Server peer-map mutation, persistent storage, and rollback prevention remain production obligations. |
