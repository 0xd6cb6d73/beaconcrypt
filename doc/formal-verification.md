...
The verified production trace remains restricted to high-level `Beacon::{encrypt_message,decrypt_message}` and `Server::{encrypt_message,decrypt_message}` operations starting from fresh or successfully validated role state without rollback. F* now proves the opaque primitive receives the exact old chain, fixes both output lengths by type, owns the per-step and initial directional partitions, constructs fixed-width result values, and routes the exact kernel-selected sequence/material pair and frame context through the callback-owned send/receive lifecycle. The remaining KDF assumption is the cryptographic correctness and totality of the two private HKDF-SHA-512 primitives. Their reviewed implementation unconditionally selects the private `SYM_RATCHET_INFO` constant, so label selection is no longer an adapter argument or separate assumption. Final conversion into external libsodium types, primitive and callback panic behavior, concrete seal/open callback semantics, serde translation and persisted-pair cryptographic provenance, hax/Rust/compiler correspondence, crash atomicity, zeroization and physical erasure, role-specific principal or peer selection, and one authoritative non-rollback owner remain assumptions. Conditional cloning or persistence rollback of the complete ratchet can still fork state.
