...
The native theorem closure is admission-free. `#print axioms` reports only `propext` and `Quot.sound` for collision extraction, the three pointwise field specializations, the field-game pointwise implications, the beacon admission eliminator, the reordered-transition collision implication, and wrong-sender impossibility, and additionally `Classical.choice` for the six probability inequalities and the exact-zero probability theorem; no audited theorem reports `sorryAx`.
...
2. Complete the remaining `beaconFinish` game through `BeaconRecordAdmitted`: the reordered-record factor-one bound and pinned-sender zero-advantage theorem are complete, while foreign-session admission still needs its factor-one collision bound.
