...
- One canonical 266-fact production transcript interface consumed by every scenario and checked against compiled core builders, mutable covered adapter/commitment/schema/concrete-ratchet sources, the exact Phase-1, Phase-2, and `CryptoFrame` schema ordinals and mappings, the covered Server and Beacon endpoint caller sites, the two normal-return ratchet message helpers, the finite receive-state fixtures and exact queries, named checked Lean interpretation anchors, and its ProVerif declaration, scoped process fixtures, active-quantum witness, and ideal frame-constructor sites.
...
The supported `Beacon` API creates one identity and ML-KEM keypair together, consumes its fresh state after one registration bundle, and has no same-identity ML-KEM rotation operation. The multi-epoch re-encapsulation fixture is therefore a non-production regression control for a hypothetical extension, not a claim that the current Beacon API performs rotation. Such an extension would need to preserve a justified public-key-binding property or cryptographically bind the selected KEM key into the authenticated handshake transcript, together with explicit authenticated epoch and replay semantics.
