...
Snapshot version 2 and its 50-entry receive-cache limit are unchanged. A trusted snapshot written by an older implementation may therefore contain a target retained by the former failed-receive behavior or all 50 cache entries, and restoration continues to accept such structurally valid state. An invalid retry against an imported cached target is neutral; an authentic retry consumes it. A full restored cache can still process the next no-skip future target because the target is consumed separately and never occupies a cache slot; a farther target that would require retaining another skipped key is rejected until an authentic cached frame frees capacity. A fresh maximum-gap success can now retain all 50 skipped keys without changing the snapshot format or cache capacity.
