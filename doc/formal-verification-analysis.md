...
- SSProve proves the first computational CTX milestone: within a joint subdistribution, the probability of the misattribution event is at most the probability of the collision event whenever the F*-backed pointwise witness contract holds. This is event inclusion, not yet a complete adversary-facing hash-oracle reduction or runtime theorem.
...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations. The CTX event-probability inequality is now mechanized, but its complete game construction, runtime theorem, cross-prover representation bridge, and BLAKE2b-512 collision-resistance premise remain open or assumed. The protocol still uses classical Ed25519 and X25519 and is not safe against an active quantum attacker; the new bounded symbolic witness recovers the initial task and violates honest registration-origin agreement as expected.
...
- **SSProve** is a Rocq framework for probabilistic cryptographic games and reductions. The current CTX theorem uses its subdistribution probability semantics but does not model quantum states or superposition oracle queries.
...
The [computational lifting](ctx-commitment.md) now uses SSProve to machine-check event-probability inclusion from the F*-backed pointwise witness; the complete adversary-facing collision game, oracle accounting, representation bridge, and runtime bound remain separate obligations.
...
The modified CTX justification has three deliberately separated layers. F* machine-checks `ctx_distinct_openings_imply_hash_collision`, which constructs a pointwise collision witness from two distinct accepted explanations of one fixed production-format `C || T || U` payload for arbitrary pure hash and AEAD-open functions. SSProve machine-checks that, in a joint subdistribution satisfying that event-inclusion contract, misattribution probability is at most collision probability. ProVerif supplies a supplementary explicit ideal-hash negative control in which the base AEAD deliberately multi-opens and removing CTX reverses the query result. None of these layers proves BLAKE2b, the libsodium call, production field provenance, the F*/SSProve representation bridge, hax/compiler correctness, a complete collision-adversary package and runtime theorem, or compiled machine code.
...
| “The modified CTX construction provides strong commitment.” | F* proves the pointwise collision witness for arbitrary pure hash and AEAD-open functions, including unequal-key and unequal-context base-AEAD openings; SSProve mechanizes the corresponding same-execution event-probability inequality. ProVerif separately confirms the intended ideal-hash CTX/no-CTX differential. The complete adversary-facing game, representation bridge, oracle/runtime accounting, BLAKE2b security, libsodium behavior, adapter provenance, compiler correspondence, and confidentiality/authenticity preservation remain assumptions or separate obligations. |
...
- a complete behavioral specification for every extracted function and error
  ...
  theorem;
- BLAKE2b-512 collision resistance, a concrete numerical bound for that primitive, a complete adversary-facing CTX/hash-oracle reduction, or a mechanized runtime theorem; F* machine-checks the pointwise collision witness and transcript injectivity, SSProve checks the same-execution event-probability inequality, and the representation bridge, opaque hash implementation, production field provenance, and compiled caller retain their stated assumptions;
...
assumptions or trusted interfaces from external hax/F* libraries. The SSProve CTX target separately rejects repository admissions and the unsafe hax SSProve prelude, exact-diffs its reviewed foundational-assumption allowlist, confines objects to `target/formal-verification/ssprove`, and runs `coqchk`. Its imported assumptions are propositional extensionality, dependent functional extensionality, constructive indefinite description, and an abstract `realType`; this audit discloses rather than proves those foundations.
...
4. SSProve mechanizes the same-execution event-probability inequality from misattribution to collision under that pointwise contract, but the complete adversary-facing game, F*/SSProve representation bridge, query/runtime accounting, and collision-resistance assumption are not mechanized.
...
The production commitment transcript helper and pointwise collision implication are F*-proved, and SSProve checks the conditional same-execution event-probability inequality. The complete adversary-facing game, F*/SSProve representation bridge, query/runtime accounting, caller, BLAKE2b, parsing, and field provenance remain computational, adapter, or primitive obligations. The injectivity proof now applies one embedded-segment equality lemma at each of the six transcript offsets instead of repeating the same sequence-extensionality argument for every field; this is a proof-structure refactor and does not alter the transcript layout or injectivity statement.
...
- the SSProve CTX theorem compiles with exactly the reviewed four foundational assumptions and passes `coqchk` without a repository admission, unsafe hax import, or admitted SSProve interchange dependency.
...
> SSProve mechanizes the same-execution probability inequality from CTX misattribution to hash collision under that pointwise contract; the complete adversary-facing game, F*/SSProve representation bridge, oracle/runtime accounting, and BLAKE2b-512 collision-resistance premise remain open or assumed.
