...
The closed protocol capstones use a direct finite pushforward rather than SSProve's high-level general package advantage bridge because the pinned bridge pulls in an unaccepted infinite-sum interchange dependency. The executable package remains useful as the bounded operational presentation, but the checked equality theorem is presently stated for the direct deterministic pushforward. The ROM extension is a sound reduction shape, not a negligible production confidentiality theorem: there is no numerical bound on its bad event, and its one-bit symmetric domain lets a two-query observer make that event certain. Both models fix one session, one record, sequence zero, public metadata, and a statically selected action and expose no general registration transcript or record-tampering/decryption interface. They are precise ideal-game milestones rather than arbitrary-execution active confidentiality theorems.
...
- the SSProve CTX binding game proves its hidden-ROM extractor and `q + 2` trace bounds, the CTX privacy game proves its distributional programming and bad-query hops, the three hidden-root protocol modalities have zero advantage, and the active-quantum witness has advantage one.
