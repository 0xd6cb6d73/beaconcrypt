...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations.
  Lean now mechanizes factor-one probability reductions from the direct one-shot equal-seal modified-CTX commitment game, the stronger general raw-payload misattribution event, and separately named wrong-sequence, wrong-sender, and cross-session record-opening games to BLAKE2b collision-resistance advantage. It proves a factor-one fixed-context retained-tag projection and, over an arbitrary well-formed designated history, an exhaustive same-view union bound that separates every accepted full-fresh CTX forgery into a fresh accepted base `C || T` projection or a context-alias replay under different outer fields. Under an extensional per-key nonce-consistency condition, Lean further proves that every alias replay targets an outer-hash input absent from all designated honest seal inputs and composes that fact into the full-forgery bound. A scheme-specific adaptive ROM game now samples one hidden 32-byte key, enforces 12-byte nonce non-reuse, runs the actual modeled base AEAD, retains `T`, and shares one lazy exact-byte oracle cache across honest seals, public queries, and final verification; every accepted alias replay is bounded by the alias-gated exact-target pre-verification cache-hit probability plus `2^-512`. Lean also extracts the exact opening from protocol admission, proves reordered-record and foreign-session transition bounds plus exact-zero wrong-sender admission, and equates the current generated 229-byte commitment builder with the ideal CTX preimage. The cache-hit term is not yet reduced to a public secret-key-prefix query and the fresh retained-base branch is not yet reduced to standard nonce-AEAD authenticity; the complete adaptive CAE game, raw-forgery normalization, IND$ key-query reduction, PPT/runtime preservation, and adapter hash, field-provenance, Hax/Rust, and compiler bridges remain unmechanized.
...
[`CtxRomAuth.lean`](../beaconcrypt-core/proofs/lean/BeaconcryptCore/Computational/CtxRomAuth.lean) begins that cryptographic step with an enforced, scheme-specific experiment rather than another arbitrary same-view attempt. It samples a hidden `List.Vector UInt8 32` key, exposes the public exact-byte ROM together with a CTX sealing oracle, rejects repeated `List.Vector UInt8 12` nonces, invokes the actual ideal `c.aeadSeal` on `ad.bytes`, retains the 16-byte `T`, and queries the shared ROM on the exact `Pqxdh.ctxPreimage`. The interface log records seal and public-ROM calls while honest internal hash calls bypass that log, and all calls still share the same lazy cache. The final verifier checks a full-fresh retained-base alias against the generated successful seal history, makes one private query on the exact target transcript, and performs the modeled base opening. `ctxAliasGameProbability_le_cacheHit_add_inv_512` proves `Pr[accepted alias] ≤ Pr[full-alias target transcript already cached before verification] + 2^-512`; gating the bad event by the alias shape prevents honest non-forgeries from making the bound vacuous. The inverse term follows from VCVio's fresh-query lemma and the checked cardinality of 64-byte responses. This is a classical ROM computational bound for the modified CTX alias game, but the cache-hit term deliberately still includes honest and public queries. The adversary returns a typed, well-formed `CtxRomRecord`; `decodeRecord_romRecord` proves its encoding parses, while the reverse normalization from every accepted arbitrary raw payload remains to be composed. The adversary surface has no separate private-coin oracle or `qE`/`qH` bound yet, and sampling one fixed hidden key proves a component game rather than ratchet key-distribution or multi-key composition. The next hop must prove that nonce enforcement rules out honest-query hits on an alias target and project the remaining hit to an adversarial query carrying the hidden key prefix.
...
These are computational reductions and exact probability projections for an ideal-model component, not a proof of BLAKE2b-512 collision resistance, transformed-AEAD privacy, standard nonce-AEAD authenticity, or end-to-end protocol security. VCVio's unkeyed `CRAdversary` does not carry a machine-checked PPT cost. The extracted-builder refinement now equates the current generated `build_commitment_transcript` result with `Pqxdh.ctxPreimage`, but no theorem yet connects ideal `Crypto.blake2b` to the adapter hash call or proves caller field provenance, Hax/Rust semantic preservation, or compiler correspondence; those complexity and production obligations remain explicit.
...
| “The modified CTX construction provides strong commitment.” | Lean gives the adversary full control of two keys, nonces, associated-data contexts, sequence numbers, sender identifiers, and plaintexts, tests equality of the actual ideal `sealRecord` outputs, and proves a factor-one reduction of that one-shot full-commitment advantage to BLAKE2b collision advantage; its stronger raw-payload game also permits a shared adversarial ciphertext outside the encoder image. F* proves the corresponding fixed-width extracted pointwise witness for arbitrary pure hash and AEAD-open functions, and Lean additionally refines the current generated six-field builder to the ideal preimage. ProVerif separately confirms the intended ideal-hash CTX/no-CTX differential. The complete adaptive CAE oracle/history game, PPT/runtime preservation, adapter hashing and field provenance, Hax/Rust semantics, BLAKE2b, libsodium, compiler correspondence, and confidentiality/authenticity preservation remain assumptions or separate obligations. |
...
- a complete behavioral specification for every extracted function and error
  ...
  theorem;
- BLAKE2b-512 collision resistance, a concrete numerical bound for that primitive, machine-checked PPT/runtime preservation, or the remaining production bridge for the modified CTX construction; F* machine-checks the fixed-width extracted witness, Lean machine-checks the direct full-commitment and stronger raw-misattribution probability reductions plus exact current generated-builder-to-ideal-preimage equality, while the opaque hash implementation, adapter invocation, production field provenance, Hax/Rust semantics, and compiled caller retain their stated assumptions;
...
The production commitment transcript helper and fixed-width pointwise collision implication are F*-proved, and Lean proves factor-one probability reductions on the ideal-model surface for the direct modified-CTX full-commitment game, general raw-payload misattribution, and dedicated wrong-sequence, wrong-sender, and cross-session record-opening games. Lean also extracts pinned-sender equality, nonzero sequence, and exact record-opening success from the ideal beacon's post-record outcomes, proves factor-one reordered-first-record and foreign-session admission bounds for a fixed honest run, proves exact zero advantage for protocol wrong-sender admission, and equates the current generated 229-byte builder with the ideal CTX preimage; the lower sender game remains a record-layer fact, while PPT/runtime preservation, the caller, BLAKE2b, parsing, session interpretation, field provenance, Hax/Rust semantics, and compiler correspondence remain computational, adapter, or primitive obligations. The injectivity proof now applies one embedded-segment equality lemma at each of the six transcript offsets instead of repeating the same sequence-extensionality argument for every field; this is a proof-structure refactor and does not alter the transcript layout or injectivity statement.
