...
- The proof does **not** establish general computational or post-quantum security of the primitive implementations.
  Lean now mechanizes factor-one probability reductions from the direct one-shot equal-seal modified-CTX commitment game, the stronger general raw-payload misattribution event, and separately named wrong-sequence, wrong-sender, and cross-session record-opening games to BLAKE2b collision-resistance advantage. It also proves a factor-one same-view projection from one accepted fresh retained-tag CTX opening to an accepted fresh base `C || T` opening when the material and complete record context are fixed, extracts the exact opening from protocol admission, proves reordered-record and foreign-session transition bounds plus exact-zero wrong-sender admission, and equates the current generated 229-byte commitment builder with the ideal CTX preimage. The authenticity projection is not a standard hidden-key nonce-AEAD game, and the result is not yet the paper's complete adaptive CAE oracle/history game; PPT/runtime preservation plus the adapter hash, field-provenance, Hax/Rust, and compiler bridges remain unmechanized, and BLAKE2b-512 collision resistance remains assumed.
