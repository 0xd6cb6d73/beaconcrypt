...
checks in the same locked shell, with the scenario targets running concurrently. From the host, each scenario is available independently as `make verify-proverif-<scenario>` or `make check-generated-proverif-<scenario>`; the raw `make check-proverif-<scenario>` form is only for callers already inside the proof shell. For example, `make verify-proverif-baseline` enters the locked shell, regenerates the extraction, and checks only the baseline model.
...
`make check-generated` reruns the complete claimed proof suite and additionally fails when extraction changes a tracked artifact or creates an untracked artifact. In the formal-verification workflow, each proof-bearing matrix worker acts as a coordinator: it selects one external profile root, runs `prepare-proof-shell` once, and then invokes its normal outer target so CI exercises profile reuse; the inventory-only worker skips real shell preparation. The workflow checks F*, Lean, and every ProVerif scenario through their configured jobs; each claimed component must regenerate and check its relevant extraction before acceptance. Run the separate `make check-inventory` tripwire after reviewing intentional production/proof boundary changes.
