...
| OR-12 | `RatchetManager::{default,recv_key,ratchet_recv,ratchet_recv_until,complete_recv_key,delete_recv_key,reset}` and the same-named `CryptoProvider` delegation helpers | Store one `RefinedRatchet` as the authoritative in-memory state and delegate construction/reset, one-step and receive-until advancement, lookup, and completion to its extracted operations with OR-07 as the sole opaque step. Logical sequences and opaque material share the kernel's fixed slots; lookup, retained outcomes, successful target/old-last swap-removal, and publication happen inside the extracted transition rather than in a parallel production array adapter. The public `delete_recv_key` wrapper routes to the same authenticated completion function. HKDF semantics, authentication-result provenance, peer selection, external material copies, panic/crash/concurrency atomicity, rollback, compiler correspondence, and physical erasure remain obligations. |
...
These statements connect opaque primitive results and the surrounding production trace to the extracted kernel. The refined ratchet now owns its control state, chain states, and fixed material slots, so its checked invariant and normal-return mutation order are proof conclusions rather than parallel-adapter assumptions; cryptographic callback behavior and the surrounding parser, authentication, persistence, peer-selection, compiler, and non-rollback obligations remain here.

| ID | Required refinement or precondition |
| --- | --- |
| AR-01 | Production stores and calls the extracted `RefinedRatchet` as its authoritative in-memory ratchet rather than maintaining a parallel logical control object and material array. F* proves that its active fixed slots pair each unique logical sequence with the material returned for that step, that inactive slots are empty, and that the bound is 50. The opaque material's cryptographic meaning still depends on AR-02. |
| AR-02 | The production step callbacks compute HKDF-SHA-512 with the intended chain state and `SYM_RATCHET_INFO`, split the exact output into the returned next chain, key, and nonce, and have no ratchet-state side effects beyond their return value. F* proves that rejection is callback-independent and state-neutral and relates each successful advance's published chain, counter, and material to the callback result; the extracted definition gates callback application after admission and retains an admitted future derivation if later authentication fails. HKDF implementation correctness, output slicing, callback side effects or panics, and compiler correspondence remain external. |
...
| AR-05 | High-level peer lookup selects one unique map entry matching the authenticated sender/target ID and leaves every other peer unchanged. Direct low-level receive mutators, pre-send state forks, and rollback are excluded. |
...
| --- | --- | --- |
| HB-01 | `proofs/fstar/Beaconcrypt_protocol_core.Ratchet.Lemmas.fst` | Logical cache view and strict counter, gap/capacity, failed-authentication retention, retry, consumption, replay, capacity release, send-key, and peer-isolation lemmas; bounded lookup and exact logical swap/removal/restoration facts; and refined-kernel validity, callback-result association, rejection neutrality, receive-until ordering, full-state retention, exact material swap-removal, lookup, send-token, and atomic restoration theorems for arbitrary chain and material types. Checked proof, not an axiom file. |
