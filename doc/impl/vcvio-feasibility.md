...
2. Extend beyond the completed one-transition CTX suite only where a maintained ideal component adds value: a `beaconRun` list lift is optional and mostly restates that only the first `InitSent` response can reach record admission, while transcript refinement and the next independent component reduction have higher priority.
