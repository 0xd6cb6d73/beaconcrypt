...
- F* proves useful, universal facts about selected deterministic Rust functions: exact PQXDH byte layouts and state transitions; the exact 32-byte key, 32-byte next-chain, and 12-byte nonce partition of a fixed 76-byte ratchet KDF output; ratchet bounds and checked counters; equality between every active cached material tag and its logical receive sequence; mismatch-aware lookup, retention, whole-entry swap-removal, and restoration behavior; and the CTX transcript and pointwise collision-witness properties described below.
- The extracted `RefinedRatchet` owns logical control state, both parametric chain values, and fixed slots of sealed `CachedReceiveKey<Material>` values, so normal-return tag association and mutation ordering no longer depend on a parallel production adapter. The HKDF implementation, intended old-chain input and label, faithful passage of its requested 76 bytes to the extracted splitter, faithful typed wrapping of the split result, authentication-result provenance, serde and persisted-pair cryptographic provenance, crash/concurrency atomicity, rollback prevention, external copies, and compiler correspondence remain assumptions.
...
| Ratchet direction | Beacon-send matches server-receive at offset 32, and beacon-receive matches server-send at offset 0, for the two halves of a 64-byte result ([theorems](../crates/protocol-core/proofs/fstar/Beaconcrypt_protocol_core.Pqxdh.Lemmas.fst#L525-L547)). | The actual HKDF operation and production application of these initial-chain offsets and storage are outside F*. This qualification concerns the initial 64-byte two-chain result; the later per-step 76-byte key/next-chain/nonce partition is extracted and proved separately. |
...
The extracted ratchet has a logical control layer and a generic refined layer. The logical `SequenceCache` says which receive sequences are available, while `RefinedRatchet<SendChain, ReceiveChain, Material>` owns that control value, both live chain values, and a fixed `[Option<CachedReceiveKey<Material>>; 50]` array. Each private cached record holds both `sequence` and `material`; derivation and restoration seal them together, lookup compares the tag with the requested logical sequence, and completion checks the target and old-last tags before mutation. F* treats chain and material values parametrically—it does not inspect secret bytes or assert that they came from HKDF—but proves that each active tag equals the logical sequence in the same unique slot. For example, after sequence 3 arrives first, sequences 2 and 3 and their opaque materials are cached as sealed records; successful authentication of sequence 3 removes its record while the sequence-2 record remains for delayed delivery.

The ratchet invariant says the active receive cache has at most 50 entries; every active logical entry is nonzero, no greater than the receive counter, and unique; every active concrete slot contains a cached record whose tag equals that logical entry; and every inactive concrete slot is empty ([ratchet lemmas](../crates/protocol-core/proofs/fstar/Beaconcrypt_protocol_core.Ratchet.Lemmas.fst)). Logical uniqueness plus tag equality makes a successful sequence lookup identify one current physical record even after an earlier swap-removal has moved entries.

The following properties are proved:

- Empty-cache constructors establish the invariant for arbitrary supplied counters.
- `split_ratchet_kdf_output` assigns borrowed views of bytes `0..32` to the message key, `32..64` to the next chain, and `64..76` to the nonce for every fixed 76-byte input, so it does not create another owned copy of the complete secret buffer. This proves the output layout, not how HKDF produced the input array or how the adapter later wraps the three named fields.
...
- Each successful receive advancement increments once, appends the exact new sequence, seals that sequence with the callback's material, preserves the send counter and tagged invariant, and reports the old cache length as the append slot. A full cache or exhausted counter is state-neutral.
- Production delegates the complete receive plan to `refined_advance_receive_until`. Before its first abstract KDF call, that operation preflights every fixed-array slot to which the plan could append. Every reported rejection therefore preserves all control, chain, and material fields, invokes no callback, and cannot return after publishing only a prefix of the plan.
- For every valid admitted future plan, the extracted executor performs exactly the bounded derivation count in order, appends exactly the consecutive sequences from the old counter plus one through the target to exactly the consecutive slots beginning at the old cache length, seals each sequence with the corresponding abstract callback result's material, preserves every old active tagged association, and finishes with the receive counter equal to the target. An old or current target performs zero steps and leaves the complete refined state unchanged before ordinary key lookup determines whether material is still available.
...
- Refined constructors establish the combined logical/tagged-material invariant with arbitrary chain types. Refined send advancement relates the published counter, next send chain, and token material to the opaque step result; rejection is state-neutral. Refined receive advancement relates the published receive counter, next receive chain, sequence, and sealed append-slot material to the opaque step result while preserving the stronger invariant.
...
- `refined_receive_key` returns material only through the current unique logical slot and rejects a populated slot whose sealed tag differs from the requested sequence. Missing and failed-authentication completion preserve every control, chain, and cached-record field. The extracted completion checks both target and old-last tags before mutation and returns `Missing` without mutation on a mismatch; successful completion moves the complete old-last tagged record into a non-last target, clears the old last slot, and publishes the matching logical removal in the same returned refined value.
- `refined_restore_receive_key` either rejects without changing its complete builder or seals and appends the supplied sequence and opaque material at the next packed slot. `finish_refined_restore` publishes the same control, chains, and slots as one valid refined state. This structural guarantee does not establish that persisted material was cryptographically derived for the supplied sequence.
...
These facts remove the former production invariant between a copyable `RatchetState` and an independently mutated `recv_slots` array. `RatchetManager` now stores one `RefinedRatchet`; `ratchet_recv_until` delegates directly to the extracted whole-plan transaction, and receive lookup, completion, and restoration call that same kernel. The kernel owns destination preflight, private sequence tags, and every logical and opaque-material mutation, so production neither chooses the append slots nor maintains an unverified association by occupancy alone, and a reported rejection cannot expose a partially executed prefix.

The proof still cannot inspect `KeyMaterial` or give a cryptographic meaning to the callback. Production must implement HKDF-SHA-512 over the intended old chain and `SYM_RATCHET_INFO`, request and faithfully pass its 76 output bytes to `split_ratchet_kdf_output`, faithfully wrap the helper's named next-chain, key, and nonce fields, and avoid relevant side effects or panics. F* proves the helper's exact byte ranges, callback-independent reported rejection, and the exact ordered sealed association of every pure callback result in a valid admitted plan; HKDF behavior and input/label selection, adapter passage/wrapping, callback panic behavior, crash atomicity, concrete callback invocation traces, and compiler behavior remain external.

The refined Rust send API returns a non-`Copy`, non-`Clone` token containing the associated material, but F* does not prove Rust ownership or global non-forking. Production borrows that token for one stack-local encryption attempt, consumes it with `refined_finish_send` on every recoverable post-allocation path, does not persist an independent copy, and retains one authoritative non-rollback role instance. The proof also accepts `authenticated` as an input, so production must derive it from the intended commitment and AEAD checks. Serialization and deserialization, the cryptographic provenance of persisted sequence/material pairs, authentication-result provenance, crash/concurrency atomicity, rollback prevention, external copies, physical erasure, and compiled behavior remain outside F*.
...
| “A ratchet step splits its 76-byte HKDF output into the intended key, next chain, and nonce.” | Production passes a fixed 76-byte array to extracted `split_ratchet_kdf_output`, and F* proves exact ranges `0..32`, `32..64`, and `64..76`. The HKDF implementation, intended old-chain input and `SYM_RATCHET_INFO` label, faithful passage of the requested bytes, faithful typed wrapping of the named result fields, hax/compiler correspondence, and machine code remain outside the theorem. |
...
| “Cached receive material belongs to its sequence; send keys are one use; replay is rejected.” | Each receive value is stored in a private `CachedReceiveKey` carrying its sequence. F* proves every active tag equals the logical entry in its slot, direct lookup rejects a disagreeing tag, valid transitions preserve the invariant, and successful refined receive removes the complete tagged target before replay lookup fails. The extracted completion also validates target and old-last tags before mutation. F* separately proves that a refined send step pairs callback material and its logical capability in one non-copyable token. The high-level claim still requires truthful authentication, one authoritative state, no retained material copy or role-state fork, correct token use, correct HKDF callback behavior, persisted-pair cryptographic provenance, and no rollback. |
...
- nonce uniqueness or the cryptographic correctness, secrecy, and physical deletion of actual message-key bytes; F* treats the step callback's chain and material outputs parametrically while proving the exact fixed 76-byte key/next-chain/nonce partition and where the returned typed values are sealed, moved, retained, and removed;
...
- concrete HKDF call behavior, the old-chain input and `SYM_RATCHET_INFO` label, faithful passage of the requested 76 bytes to the extracted splitter, faithful typed wrapping of its named fields, AEAD result provenance, callback panic behavior, crash/concurrency atomicity, compiler correspondence, retained copies outside the authoritative refined state, or rollback; the extracted kernel now proves the split offsets, sealed cached-tag association, callback-free reported rejection, and exact normal-return whole-plan ordering for the chain and material values returned by its opaque callback;
- cryptographic provenance of imported receive sequence/material pairs; the extracted restoration builder validates structural bounds and ordering and seals each supplied pair, but cannot prove that persisted bytes were originally derived for the supplied sequence;
...
- security after retaining or reusing a copy of an available send capability, forking pre-send role state, or rolling counters and replay history backward; or
...
- Ed25519-to-X25519 conversion and all primitive calls have the documented
  semantics;
- HKDF is deterministic and supplies the needed PRF and one-way properties. Production uses `PQXDH_INFO` for root derivation and `SYM_RATCHET_INFO` for both initial-chain derivation and per-step ratcheting; its two directions are separated by taking different halves of the initial 64-byte output, which the adapter must preserve. For each later step, F* proves the fixed 76-byte output partition, while faithful passage of the requested bytes into that helper and faithful typed wrapping of its result remain adapter/compiler obligations;
...
The extracted `RefinedRatchet` owns the logical control value, both live chains, and fixed slots containing sealed `CachedReceiveKey<Material>` records. Its F* invariant proves that every active cached tag equals the unique logical sequence in the same slot and every inactive slot is empty. Its whole-plan theorem proves destination preflight before callback application, callback-free complete-state neutrality for every reported rejection, the exact bounded ordered sequence/slot/material trace and target counter for a valid admitted future plan, preservation of every old tagged association, and a zero-step unchanged result for an old or current target before lookup. Receive lookup checks the tag before exposing material, missing and retained completion preserve the complete state, extracted completion validates target and old-last tags before mutation, successful completion performs exact whole-entry swap-removal with its logical update, and restoration seals each supplied sequence/material pair atomically. The extracted splitter and its F* theorem also own the exact 32/32/12-byte interpretation of a fixed 76-byte ratchet output. The production connection still assumes:
...
- the adapter passes the exact F*-verified root input and labels to HKDF, uses the exact returned associated data, and applies the complementary role offsets to the initial 64-byte two-chain result correctly;
- the production ratchet-step callback passes the intended old chain and `SYM_RATCHET_INFO` to HKDF-SHA-512, requests exactly 76 bytes, faithfully passes those bytes to `split_ratchet_kdf_output`, faithfully wraps its named next-chain, key, and nonce fields, and has no relevant stateful effect beyond returning those values; F* proves the split ranges, callback-independent reported rejection, and exact whole-plan normal-return sealed association, while the extracted definition preflights all destinations and applies each pure callback in admitted sequence order with the matching counter and slot; HKDF behavior, callback panic, crash behavior, and compiler correspondence remain external;
...
- restoration requires the exact five-field ratchet schema, rejects legacy objects containing `send_past`, validates bounds and uniqueness, sorts the exact imported receive sequence/material pairs, rejects malformed or oversized state, passes each pair to `refined_restore_receive_key`, serializes only tag-checked sequence/material pairs from authoritative refined entries, and supplies material that was cryptographically derived for the persisted sequence; and
- server state and replay history have one owner and are not rolled back.

The extracted shared kernel now supplies the machine-checked in-memory ratchet refinement and per-step output partition that the former parallel adapter lacked. Compile-time size assertions, private Rust fields, consuming APIs, and regression tests support the remaining primitive, HKDF invocation, split-result passage/wrapping, parser, authentication, role-specific principal or peer selection, persisted-pair provenance, compiler, crash/concurrency, and non-rollback assumptions but do not discharge them.
...
6. Separately, F* proves the exact 32-byte key, 32-byte next-chain, and 12-byte nonce ranges in every fixed 76-byte input to the extracted ratchet splitter.
7. F* also proves that every valid active cached tag equals its logical sequence, successful refined receive removes exactly the complete tagged target record, the old-last tagged record moves intact when necessary, and a retry is rejected without changing the complete refined state. The extracted lookup and completion operations reject tag disagreements at runtime rather than exposing or moving mismatched material.
8. Production must invoke HKDF with the intended old chain and label, faithfully pass the requested 76 bytes to the splitter, faithfully wrap its named fields, and preserve one authoritative, non-rollback refined state without retaining independent material copies.
9. Production frame parsing, sealing, opening, commitment hashing, and sender lookup must match the models.
...
1. F* proves that refined receive consumption removes the complete sealed record whose private tag and material are paired with the target logical sequence in the authoritative fixed slots; production must still avoid rollback and retained copies outside that state.
...
2. Production delegates the whole plan to `refined_advance_receive_until`. F* proves that the extracted operation checks admission and every planned fixed-array destination before the first callback, so every reported rejection is callback-free, preserves the complete refined state, and cannot publish only an executed prefix. For a valid admitted future plan it proves the exact bounded ordered sequence/slot/material trace with each result sealed under its sequence, preservation of all old active tagged associations, and a final receive counter equal to the target; an old or current target is a zero-step unchanged result before lookup. The admitted post-transaction state is then retained if authentication fails. F* does not prove production callback side effects or panics, crash behavior, role-specific principal or peer selection, or the cryptographic meaning of callback outputs.
3. F* proves the exact per-step 76-byte key/next-chain/nonce partition, cached-tag equality for valid states, mismatched-tag lookup rejection, full-state neutrality for missing and retained outcomes, exact whole-record target/old-last swap-removal on success, and atomic sealed restoration of each supplied pair. The extracted completion validates both relevant tags and rejects a mismatch without mutation. HKDF behavior and input/label selection, faithful byte passage and typed wrapping, authentication-result provenance, persisted-pair cryptographic provenance, serde correctness, compiler correspondence, retained external copies, crash/concurrency atomicity, and rollback remain outside those theorems.
...
> For the receive cache specifically, the extracted `RefinedRatchet` owns the logical cache and fixed slots of private sequence-tagged material, and production delegates directly to its `refined_advance_receive_until` operation. F* proves the exact 32-byte key, 32-byte next-chain, and 12-byte nonce ranges of the extracted 76-byte splitter; requires every active cached tag to equal its logical slot sequence; and proves that all planned destinations are preflighted before the first abstract KDF call, every reported rejection is callback-free and preserves the complete state, every valid admitted future plan produces the exact bounded ordered sequence/slot/material trace while preserving old tagged associations, and an old or current target performs zero steps before lookup. Lookup rejects a mismatched tag, completion validates the target and old-last tags before mutation, and the checked results cover full-state retention, whole-tagged-entry swap-removal, and sealed restoration. Production still has to invoke HKDF with the intended old chain and label, faithfully pass the requested 76 bytes to the splitter, faithfully wrap its named fields, supply truthful authentication, establish persisted-pair cryptographic provenance, serialize faithfully, preserve one authoritative non-rollback state, and rely on correct hax/compiler translation; callback panics, crash/concurrency atomicity, and physical erasure are not proved.
>
