...
| OR-09 | `CryptoProvider::encrypt_message`, `encrypt_message_with_ratchet`, and `<BeaconCryptPqxdh as ProviderServer>::{encrypt_and_update,encrypt_and_update_json}` | The trait method selects one target peer and associated data. The helper calls `advance_send`, requires the returned sequence, capability, and advanced counter to agree, performs one concrete KDF step, and keeps the capability and concrete key local to that call. It then calls ChaCha20-Poly1305 `encrypt_detached`, builds the CTX commitment, serializes the frame, passes the capability to `finish_send` on every recoverable post-allocation outcome, requires the result to report consumption and unavailability, and drops the concrete key without storing either value in `RatchetManager`. The update wrappers snapshot or JSON-serialize the resulting five-field state. |
...
| OR-12 | `RatchetManager::{default,recv_key,ratchet_recv,ratchet_recv_until,complete_recv_key,delete_recv_key,receive_slots_match_control,reset}` and the same-named `CryptoProvider` delegation helpers | Validate the returned sequence, slot, logical mapping, concrete bounds, and emptiness before performing one opaque HKDF step after each successful `advance_receive`; store its result in the returned append slot; resolve existing receive sequences only through `lookup_receive_key`; and, on `Consumed`, validate and apply exactly the target/old-last swap/take returned by `finish_receive_with_removal` before publishing its state. The extracted core proves the logical slot decisions, while HKDF behavior, correct `KeyMaterial` association, concrete array operations, authentication-result provenance, panic/crash atomicity, and physical erasure remain adapter obligations. The public `delete_recv_key` wrapper routes to the same receive completion function; direct low-level use remains outside the claimed high-level trace. |
...
| OR-14 | `<BeaconCryptPqxdh as ProviderServer>::{export_state,try_from_state}`, `ProviderServer::from_state`, `RatchetManager` serialization/deserialization, `serialize_server_state`, `deserialize_server_state`, and the complete serde impl blocks in `src/ser.rs`/`src/deser.rs` | Serde/JSON persistence is not extracted. Ratchet export emits exactly `send_key`, `recv_key`, `send_ctr`, `recv_past`, and `recv_ctr`; no concrete send-message key or logical send capability is persisted. Import requires that exact five-field shape, so legacy objects containing `send_past` are rejected. It validates typed key roles plus receive sequence/counter bounds and cache capacity, sorts receive sequence/material map entries, rebuilds only through `start_restore`/`restore_receive_key_with_slot`/`finish_restore`, and places each material value in its returned slot. Export maps active receive slots back to logical sequences and rejects both missing active material and populated inactive slots. Physical slots are not persisted, and duplicate raw JSON map keys are not claimed to be rejected before ordinary `HashMap` deserialization. The remaining server-state checks cover replay-history shape plus server/peer ID bounds, and the JSON update methods additionally use the `StateUpdate` serde impl. |
...
| --- | --- |
| AR-01 | For `n = RatchetState::receive_cache_len`, each physical `recv_slots` entry below `n` contains exactly the concrete key and nonce for the logical sequence in the same core slot, every slot at or above `n` is empty, and capacity is 50. The core proves logical lookup/removal/restoration positions; the secret-byte association itself remains an adapter refinement. |
| AR-02 | Each successful `advance_receive` causes exactly one concrete HKDF step and stores its result in the returned sequence's append slot; rejected advancement performs no HKDF step or concrete mutation. Admitted future derivation occurs before frame authentication and is retained if authentication fails. |
| AR-03 | Each successful `advance_send` is paired with exactly one concrete key in the same stack-local high-level operation. The adapter validates that the sequence, capability, and advanced counter agree, does not persist the capability or reuse a copy, leaves no reusable copy after the call, passes the capability to `finish_send` after the one permitted encryption attempt even if AEAD or serialization fails, requires the result to be consumed and unavailable, and drops the concrete key without storing either value in manager or persistent state. |
| AR-04 | Production obtains the candidate's current slot from `lookup_receive_key`. Failed receive authentication returns no removal and retains the complete post-admission logical/concrete state, including every newly derived skipped key and the exact candidate key; repeating that failed target derives nothing further. Success validates and applies exactly the target/old-last swap-removal plan returned by `finish_receive_with_removal` before publishing its state, so replay cannot find the target and the freed packed-last slot is available to a later admitted receive. |
...
| AR-06 | Persistence import requires the exact five-field ratchet schema, rejects legacy `send_past` objects, checks receive sequence bounds, materializes and sorts receive sequence/material pairs, and rebuilds only through `start_restore`/`restore_receive_key_with_slot`/`finish_restore`, placing material at each returned slot and rejecting oversized, duplicate-slot, or structurally invalid logical state. Serialization emits no concrete send-message key, logical send capability, or physical receive slot, and rejects both missing active receive material and populated inactive slots. Duplicate raw JSON map keys may be resolved by ordinary serde `HashMap` parsing before these checks. Serde correctness, persistence confidentiality, storage atomicity, and rollback prevention remain external. |
...
| --- | --- | --- |
| HB-01 | `proofs/fstar/Beaconcrypt_protocol_core.Ratchet.Lemmas.fst` | Logical cache view and strict counter, gap/capacity, one-step failed-authentication retention, zero-cost retry, consumption, replay, full-cache rejection, post-consumption capacity release, send-key, and peer-isolation lemmas, extended with bounded-lookup soundness and absence/completeness characterization, unique-slot reasoning, detailed/compatibility finish equivalence, missing/retained no-removal neutrality, exact successful target/old-last swap shape and moved-entry preservation, detailed/compatibility restore equivalence, and restoration append-slot validity. Checked proof, not an axiom file. |
