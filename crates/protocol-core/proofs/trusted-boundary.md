...
- no `hax_lib::opaque` or `hax_lib::opaque_type` annotation in the protocol
  core;
- two generated F* modules and two strictly checked handwritten F* lemma
  modules;
- one generated ProVerif library, exactly three production-backed replacement
  fragments, and five handwritten ProVerif model/query libraries plus two
...
| --- | --- | --- |
| OR-01 | `<BeaconCryptPqxdh as CryptoProvider>::{new,default}` | Initializes libsodium; generates an Ed25519 identity or derives the server identity from a supplied seed; validates an expected server Ed25519 key; and, for a beacon, generates its X25519 prekey and ML-KEM keypair. `ensure_init`, `crypto_sign::KeyPair::{generate,from_seed}`, `crypto_kx::KeyPair::generate`, and `crypto_kem::mlkem768::KeyPair::generate` are outside the proof. Persistence restoration is OR-14, not part of `new`. |
...
| OR-03 | `<BeaconCryptPqxdh as ProviderBeacon>::finish_registration` | Parses the response, converts Ed25519 identity keys to X25519, decapsulates ML-KEM, computes ordered DH1--DH4, derives the root and initial chains, opens the first record, authenticates its assigned-ID prefix, and publishes the staged ratchet. |
...
| OR-10 | `CryptoProvider::decrypt_message`, `encrypted_frame_sender`, `decrypt_message_with_ratchet`, and `<BeaconCryptPqxdh as ProviderServer>::{decrypt_and_update,decrypt_and_update_json}` | The trait method parses the sender ID and selects its peer/associated data. The helper rechecks that expected sender, performs receive admission and exactly the planned KDF steps, checks the CTX commitment, calls ChaCha20-Poly1305 `decrypt`, and retains or consumes the same candidate key according to authentication. The update wrappers snapshot or JSON-serialize the resulting state. |
| OR-11 | `build_commitment` | Calls libsodium BLAKE2b `crypto_generichash::generichash` over the fixed production tuple `(key, nonce, associated data, AEAD tag, LE64 sequence, LE64 sender ID)`. `libsodium_rs::utils::memcmp` performs concrete secret/commitment comparison. |
| OR-12 | `RatchetManager::{default,ratchet_send,send_key,recv_key,ratchet_recv,ratchet_recv_until,consume_send_key,complete_recv_key,receive_key_slot,send_cache_matches_control,receive_cache_matches_control,reset,restored_send_capability}` and the same-named `CryptoProvider` delegation helpers | Pair the concrete `HashMap` keys with core capabilities, derive exactly the admitted number of keys, refine completion to the core disposition, and reconstruct the logical/concrete relation. The public `delete_send_key`/`delete_recv_key` wrappers route to the same completion functions; direct low-level use remains outside the claimed high-level trace. |
...
| PL-07 | ChaCha20-Poly1305 open inverts an honest seal only for the same key, nonce, plaintext/ciphertext, and associated data, and a successful open supplies authentication provenance. The symbolic frame rule additionally binds the exact sequence and sender through the CTX commitment. |
| PL-08 | The fixed-size CTX BLAKE2b commitment is binding/collision resistant for its exact six-field input. ProVerif idealizes it as a collision-free constructor. No general hash injectivity assumption is made by the F* proof. |
...
| PL-10 | ProVerif's fresh assigned-key-ID names and constructor-generated `first_sequence`/`next_sequence` terms model only the checked, non-exhausted, collision-free bounded prefix of the production `u64` spaces. They do not prove arbitrary finite-counter executions or wraparound behavior; those are covered by F*. |
...
| --- | --- |
| AR-01 | `keys(recv_past)` equals the active logical receive sequences in `RatchetState`; the concrete cache has at most 50 entries and no duplicate sequence. |
| AR-02 | Each successful `advance_receive` causes exactly one concrete HKDF step and associates its result with the returned sequence and slot. Rejected admission is state neutral. |
...
| AR-04 | Failed receive authentication retains the exact logical and concrete candidate key; success removes exactly that key from both representations, so a later replay cannot find it. |
...
| AR-10 | The expected server identity, parsed response identity, and accepting server binding refer to the same authenticated key. The eight assigned-ID bytes supplied to the beacon typestate are exactly the prefix returned by a successful initial AEAD open. |
...
| LA-02 | PQXDH's exact slice proofs use the val-only contracts for `Rust_primitives.Hax.Monomorphized_update_at.{update_at_range,update_at_range_to,update_at_range_from}` from the pinned hax proof libraries. Their reviewed contracts replace precisely the selected range and preserve its prefix/suffix. |
| LA-03 | The little-endian key-ID proof calls val-only `Rust_primitives.Integers.shift_right_lemma` seven times for shifts 8 through 56. Its exact contract is `Lemma (v (shift_right a b) == v a / pow2 (v b))` with an SMT pattern for the shift; the underlying pinned `shift_right` is `opaque_to_smt`. |
| LA-04 | `update_at_usize`, `repeat`, `array_of_list`, slice copying, Option/Result, the other selected finite integer arithmetic/casts, equality, and `Core_models.Num.impl_u64__MAX` are transparent definitions in the locked hax/Core_models libraries. Ratchet extraction uses only the transparent single-index update. |
...
is exactly the PQXDH and ratchet modules.
