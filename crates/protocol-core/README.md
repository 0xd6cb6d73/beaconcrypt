...
The correspondence claim covers high-level encryption and decryption traces that begin in a concrete reachable initialization and continue without state rollback. Equal concrete roots still depend on correct PQXDH root-derivation semantics for the exact verified transcript and `PQXDH_INFO`; F* proves equality only under the same fixed pure root function. The request executor must faithfully and totally implement HKDF-SHA-512 for each core-owned input and `SYM_RATCHET_INFO`, and the production callbacks must faithfully implement the intended commitment and ChaCha20-Poly1305 operations. Core-owned `RatchetChain`, `RatchetKey`, `RatchetNonce`, `RatchetMaterial`, and request arrays now implement zeroization on `Drop`, but physical erasure, compiler optimization behavior, and retained external copies remain outside F*. Production's libsodium key/nonce conversions are ephemeral inside callbacks rather than persistent state conversions, yet their byte fidelity, libsodium behavior, callback result provenance, panic behavior, and Rust/hax/compiler correspondence remain assumptions. A no-reuse conclusion for distinct logical record positions remains conditional on noncollision of the relevant concrete KDF projections and one authoritative owner with no fork, rollback, or stale restored snapshot. Persistence fidelity and authenticated canonical snapshot provenance, crash atomicity, role-specific peer selection, and rollback prevention are likewise external obligations.
...
The checked ratchet properties cover the core-owned request's exact 32-byte input and `SYM_RATCHET_INFO`, the exact 76-byte key/next-chain/nonce partition, preservation of one executor through every concrete-chain step, send and receive counter monotonicity and exhaustion, receive-gap and cache bounds, exact cached sequence/material association, whole-plan preflight and bounded execution, lookup, retry retention, consumption, replay rejection, restoration structure, and non-selected peer isolation. `concrete_reachable` fixes the concrete initial directional chains and the sole core-selected step for a kernel's lifetime; fresh concrete construction establishes it, and `concrete_seal_next_preserves_reachability` plus `concrete_open_and_finish_preserves_reachability` preserve it. The PQXDH lemmas additionally cover authenticated common-root derivation under a fixed pure root function, complementary role-bound concrete kernels, reachability of both initial kernels, both directions of material equality at every sequence, and preservation of the paired concrete session through beacon-seal/server-open and server-seal/beacon-open. Structural restoration remains unconditional, while the generic `reachable_restore` append/finish theorems require authenticated canonical chain/material premises that ordinary serde does not establish.
